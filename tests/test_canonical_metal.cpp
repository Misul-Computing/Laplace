...
#ifndef LAPLACE_HAS_QUALIFICATION_GGUF
#define LAPLACE_HAS_QUALIFICATION_GGUF 0
#endif

#ifndef LAPLACE_QUALIFICATION_GGUF
#define LAPLACE_QUALIFICATION_GGUF ""
#endif
...
std::shared_ptr<const RuntimePackage> synthetic_f16_dense_package() {
    ...
    for (uint32_t token = 0; token != vocabulary; ++token) {
        const uint16_t value = token == 0 ? 0x2800u : token == 1 ? 0x2c00u : 0x2e00u;
    ...
    const auto f16 = [&](uint32_t id, TensorRole role, uint32_t columns) {
        ...
        tensor.dimensions = {{DimensionKind::Constant, columns},
                             {DimensionKind::Constant, hidden}};
    ...
    return std::make_shared<RuntimePackage>(std::move(model), package, package.digest(),
...
void test_two_token_prefill_uses_registered_f16_batch() {
    if (!metal_device_present() || !metal_available()) {
        std::fprintf(stderr, "SKIP: Metal device is unavailable\n");
        return;
    }
    const auto package = synthetic_f16_dense_package();
    CHECK(package != nullptr);
    if (!package) return;
    auto created = create_canonical_metal_program(package, 4);
    ...
    auto stepped_created = create_canonical_metal_program(package, 4);
    ...
    auto rejected_created = create_canonical_metal_program(package, 4);
...
std::shared_ptr<const RuntimePackage> load_package_with_semantic_mutation(
    ...
    return std::make_shared<RuntimePackage>(std::move(semantics), std::get<PackageView>(view),
...
void test_dense_f16_prefill_uses_one_completed_metal_transaction() {
    ...
    auto created = create_canonical_metal_program(package, 8);
    CHECK(std::holds_alternative<CanonicalMetalProgram>(created));
    if (!std::holds_alternative<CanonicalMetalProgram>(created)) return;
    CanonicalMetalProgram program = std::get<CanonicalMetalProgram>(std::move(created));
...
void test_interleaved_programs_do_not_share_kv_or_weight_resources() {
    ...
    CHECK(package != nullptr);
    if (!package) return;
    auto first_created = create_canonical_metal_program(package, 8);
    auto second_created = create_canonical_metal_program(package, 8);
...
void test_planner_selects_only_the_exact_dense_metal_pattern() {
    ...
    auto created = create_canonical_metal_program(extra, 8);
    CHECK(std::holds_alternative<CompatibilityReport>(created));
    if (const auto* report = std::get_if<CompatibilityReport>(&created)) {
        CHECK(report->code == CompatibilityError::KERNEL_UNAVAILABLE);
    }
...
void test_canonical_metal_binds_the_exact_query_gate_pattern() {
    ...
    if (!malformed) return;
    created = create_canonical_metal_program(malformed, 8);
    CHECK(std::holds_alternative<CompatibilityReport>(created));
    if (const auto* report = std::get_if<CompatibilityReport>(&created)) {
        CHECK(report->code == CompatibilityError::KERNEL_UNAVAILABLE);
    }
...
void test_metal_prefill_and_incremental_decode_match_prefixes_one_through_eight() {
    ...
    for (uint32_t prefix = 1; prefix <= 8; ++prefix) {
        ...
        auto one_shot_created = create_canonical_metal_program(package, 8);
        auto incremental_created = create_canonical_metal_program(package, 8);
...
int test_generic_metal_artifact(const char* path, uint32_t decode_count = 1, uint32_t maximum_context = 128) {
    ...
    auto created = create_canonical_metal_program(std::get<ValidatedPackage>(loaded).runtime_package(), maximum_context);
    if (const auto* report = std::get_if<CompatibilityReport>(&created)) {
    ...
    CanonicalMetalProgram program = std::get<CanonicalMetalProgram>(std::move(created));
    const uint32_t token = std::get<ValidatedPackage>(loaded).runtime_package()->semantics().bos_id;
    for (uint32_t index = 0; index != decode_count; ++index) {
        ...
        if (const auto* output = std::get_if<CanonicalMetalOutput>(&executed)) {
            ...
            CHECK(output->projection_weight_bytes > 0);
            if (decode_count > 1) {
                fprintf(stderr, "generic canonical Metal decode %u: wall_ms=%.3f gpu_ms=%.3f cpu_wait_ms=%.3f ops=%u projections=%u q4k=%u q6k=%u weight_bytes=%llu\n",
                        index + 1, wall_ms, output->gpu_time_ms, output->cpu_wait_ms, output->operator_count,
                        output->projection_dispatches, output->q4k_projection_dispatches,
                        output->q6k_projection_dispatches,
                        static_cast<unsigned long long>(output->projection_weight_bytes));
            }
            if (output->profiled) {
...
int test_generic_metal_profile(const char* path) {
    ...
    auto run_sample = [&](bool profile, Sample& sample) {
        ...
        auto created = create_canonical_metal_program(package, 1);
        if (!std::holds_alternative<CanonicalMetalProgram>(created)) {
            unsetenv("LAPLACE_CANONICAL_METAL_PROFILE");
            CHECK(false);
            return false;
...
int test_generic_metal_recurrent_preflight(const char* path) {
    ...
    CHECK(std::holds_alternative<ValidatedPackage>(loaded));
    if (!std::holds_alternative<ValidatedPackage>(loaded)) return test_summary("test_canonical_metal");
    auto created = create_canonical_metal_program(std::get<ValidatedPackage>(loaded).runtime_package(), 128);
    CHECK(std::holds_alternative<CanonicalMetalProgram>(created));
    if (!std::holds_alternative<CanonicalMetalProgram>(created)) return test_summary("test_canonical_metal");
...
int test_generic_metal_error_propagation(const char* path) {
    ...
    if (!std::holds_alternative<ValidatedPackage>(loaded)) return test_summary("test_canonical_metal");
    const auto package = std::get<ValidatedPackage>(loaded).runtime_package();
    auto created = create_canonical_metal_program(package, 128);
    CHECK(std::holds_alternative<CanonicalMetalProgram>(created));
    if (!std::holds_alternative<CanonicalMetalProgram>(created)) return test_summary("test_canonical_metal");
    CanonicalMetalProgram program = std::get<CanonicalMetalProgram>(std::move(created));
    canonical_metal_fail_after_completed_submission_for_testing(program);
...
int test_generic_metal_derived_q2_ownership(const char* path, bool streamed_only = false,
    ...
    if (!retained_source_only) {
        auto control_created = create_canonical_metal_program(package, 128);
    ...
    auto created = create_canonical_metal_program(package, 128, policy);
    ...
    if (retained_source_only) {
        ...
        auto paired_control_created = create_canonical_metal_program(package, 128);
        ...
        for (uint32_t trial = 0; trial != 5; ++trial) {
            ...
            CHECK(last_derived.projection_weight_bytes == last_control.projection_weight_bytes -
    ...
    if (const auto* output = std::get_if<CanonicalMetalOutput>(&executed)) {
        ...
        CHECK(output->projection_weight_bytes == control_output.projection_weight_bytes -
                                                   expected_source_bytes + expected_derived_bytes);
        ...
        std::fprintf(stderr,
                     ...
                     static_cast<unsigned long long>(control_output.projection_weight_bytes),
                     static_cast<unsigned long long>(output->projection_weight_bytes),
...
int test_generic_metal_ffn_calibration(const char* path) {
    ...
    auto created = create_canonical_metal_program(package, 128, {}, {}, {}, calibration);
    if (const auto* report = std::get_if<CompatibilityReport>(&created))
...
int test_generic_metal_ffn_calibration_cache(const char* path, const char* corpus_path,
    ...
    for (size_t window_index = 0; window_index != window_lengths.size(); ++window_index) {
        ...
        auto created = create_canonical_metal_program(package, window_length, {}, {}, {}, calibration);
        CHECK(std::holds_alternative<CanonicalMetalProgram>(created));
        if (!std::holds_alternative<CanonicalMetalProgram>(created)) return test_summary("test_canonical_metal");
        CanonicalMetalProgram program = std::get<CanonicalMetalProgram>(std::move(created));
...
int test_generic_metal_sparse_ffn_calibrated(
    ...
    if (mode == CalibratedSparseMode::VariableWidthProbe) {
        auto created = create_canonical_metal_program(package, 1, {}, policy);
        CHECK(std::holds_alternative<CanonicalMetalProgram>(created));
        if (!std::holds_alternative<CanonicalMetalProgram>(created)) {
            if (const auto* report = std::get_if<CompatibilityReport>(&created))
    ...
    if (mode == CalibratedSparseMode::MarginalAllocate) {
        ...
        {
            auto baseline_created = create_canonical_metal_program(
        ...
        for (size_t layer = 0; layer != rankings.size(); ++layer) {
            for (size_t level = 0; level + 1 != retention_levels.size(); ++level) {
                ...
                auto candidate_created = create_canonical_metal_program(
        ...
        std::vector<uint32_t> reference_top1;
        {
            auto reference_created = create_canonical_metal_program(
                package, 4);
        ...
        for (size_t layer = 0; layer != rankings.size(); ++layer) {
            ...
            for (uint32_t count : sequential_levels) {
                ...
                auto trial_created = create_canonical_metal_program(
                    package, static_cast<uint32_t>(calibration_inputs.size()), {},
                    trial_policy);
    ...
    uint64_t control_projection_bytes = 0;
    {
        ...
        for (size_t step = 0; step != 17; ++step) {
            ...
            control_projection_bytes = output.projection_weight_bytes;
    ...
    for (size_t step = 0; step != 17; ++step) {
        ...
        CHECK(candidate_output.projection_weight_bytes ==
              control_projection_bytes - expected_source_bytes + expected_requested_bytes);
        candidate_projection_bytes = candidate_output.projection_weight_bytes;
...
int test_generic_metal_iq2_xxs_ffn_quality(const char* path) {
    ...
    uint64_t control_peak_session_bytes = 0;
    {
        ...
        for (size_t step = 0; step != 17; ++step) {
            ...
            CanonicalMetalOutput output = std::get<CanonicalMetalOutput>(std::move(result));
            CHECK(output.command_buffers == 1);
            CHECK(output.projection_weight_bytes == baseline_intermediate_projection_bytes);
            control_gpu_ms += output.gpu_time_ms;
    ...
    auto candidate_created = create_canonical_metal_program(package, 128, {}, {}, policy);
    const double construction_ms = std::chrono::duration<double, std::milli>(
    ...
    for (size_t step = 0; step != 17; ++step) {
        ...
        CHECK(iq.projection_weight_bytes == candidate_intermediate_projection_bytes);
...
int test_generic_metal_sparse_ffn_ownership(const char* path, bool all_blocks,
    ...
    if (device_selector || dense_oracle_selector) {
        ...
        for (size_t step = 0; step != 17; ++step) {
            ...
            if (!dense_oracle_selector)
                CHECK(selected_output.projection_weight_bytes ==
                      control_output.projection_weight_bytes - expected_source_bytes + expected_requested_bytes);
    ...
    for (size_t repeat = 0; repeat != repetitions; ++repeat) {
        ...
        CHECK(sparse_output.projection_weight_bytes == control_output.projection_weight_bytes -
    ...
    std::fprintf(stderr,
                 ...
                 static_cast<unsigned long long>(control_output.projection_weight_bytes),
                 static_cast<unsigned long long>(sparse_output.projection_weight_bytes),

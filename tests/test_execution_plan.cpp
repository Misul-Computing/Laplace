...
void test_product_planner_provenance_shape() {

    // The legacy semantic overload must not accidentally produce a product
    // codec map. Product provenance is stamped only by the package overload.
    ...
    const PlanResult result = plan_canonical_metal(
        q4k_dense_model(), request(), metal, builtin_canonical_metal_registry());
    if (const auto* plan = std::get_if<ExecutionPlan>(&result)) {
        CHECK(!plan->codec_aware);
        CHECK(plan->package_fingerprint == Sha256Digest{});
        CHECK(plan->dispatch_programs.empty());
        CHECK(plan->dispatch_pipeline_slots.empty());
    }
}
...
SemanticTensor column_grouped_u2_tensor(uint32_t id, TensorRole role,
    ...
    tensor.role = role;
    tensor.logical_type = ScalarType::F32;
    tensor.dimensions = {{DimensionKind::Constant, logical_k},
                         {DimensionKind::Constant, logical_n}};
    ...
    tensor.layout.strides = {1, stride, 0, 0, 0, 0, 0, 0};
    tensor.layout.block_elements = 256;
    ...
    const uint64_t elements = static_cast<uint64_t>(logical_k) * logical_n;
    const uint64_t values_bytes = elements / 4;
    const uint64_t plane_bytes = elements / 256 * 2;
    const auto aligned = [](uint64_t value) { return (value + 127u) / 128u * 128u; };
    const uint64_t scales_offset = aligned(values_bytes);
    const uint64_t biases_offset = aligned(scales_offset + plane_bytes);
    tensor.planes = {
        {PlaneKind::Values, ScalarType::U8, ArtifactId{0}, 0, values_bytes, 128, 0},
        {PlaneKind::Scales, ScalarType::F16, ArtifactId{0}, scales_offset, plane_bytes, 128, 0},
        {PlaneKind::Biases, ScalarType::F16, ArtifactId{0}, biases_offset, plane_bytes, 128, 0},
    };
    return tensor;
}

SemanticModel column_grouped_u2_recurrent_model(uint64_t gate_up_stride,
                                                uint64_t down_stride) {
    SemanticModel model = recurrent_q4k_q6k_model();
    model.values[16].dimensions[1].constant_or_symbol = 512;
    model.values[17].dimensions[1].constant_or_symbol = 512;
    model.values[18].dimensions[1].constant_or_symbol = 512;
    model.tensors[11] = column_grouped_u2_tensor(
        11, TensorRole::FfnGateWeight, 256, 512, gate_up_stride);
    model.tensors[12] = column_grouped_u2_tensor(
        12, TensorRole::FfnUpWeight, 256, 512, gate_up_stride);
    model.tensors[13] = column_grouped_u2_tensor(
        13, TensorRole::FfnDownWeight, 512, 256, down_stride);
    return model;
}
...
SemanticTensor target_native_bit_plane_tensor(uint32_t id = 77) {
    SemanticTensor tensor;
    tensor.id = id;
    tensor.role = TensorRole::QueryWeight;
    tensor.logical_type = ScalarType::F32;
    tensor.dimensions = {{DimensionKind::Constant, 256}, {DimensionKind::Constant, 128}};
    ...
    tensor.quantization.block_bytes = 64;
    tensor.quantization.group_size = 256;
    tensor.quantization.required_plane_mask = 0x7;
    tensor.planes = {
        {PlaneKind::Values, ScalarType::U8, ArtifactId{4}, 0, 8192, 128, 0},
        {PlaneKind::Scales, ScalarType::F16, ArtifactId{4}, 8192, 256, 128, 0},
        {PlaneKind::Biases, ScalarType::F16, ArtifactId{4}, 8448, 256, 128, 0},
    };
    return tensor;
}

std::array<uint8_t, 32> codec_digest(uint8_t value) {
    std::array<uint8_t, 32> result{};
    result.fill(value);
    return result;
}

std::array<uint8_t, 32> codec_digest_index(uint32_t value) {
    std::array<uint8_t, 32> result{};
...
PhysicalCodecIdentity target_native_bit_plane_identity(const SemanticTensor& tensor,
                                                       uint8_t arithmetic,
                                                       uint8_t codebook = 0) {
    const auto identity = physical_codec_identity(
        tensor, 1, codec_digest(arithmetic), codec_digest(codebook));
    CHECK(identity.has_value());
    return identity.value_or(PhysicalCodecIdentity{});
}

PhysicalCodecRegistry target_native_bit_plane_registry(const SemanticTensor& first,
                                                       const SemanticTensor* second = nullptr) {
    PhysicalCodecRegistry result;
    const PhysicalCodecIdentity first_identity = target_native_bit_plane_identity(first, 1);
    result.codecs.push_back({first_identity});
    result.tensors.push_back({first.id, first_identity});
    if (second) {
        const PhysicalCodecIdentity second_identity = target_native_bit_plane_identity(*second, 2);
        result.codecs.push_back({second_identity});
        result.tensors.push_back({second->id, second_identity});
    }
    return result;
}

SemanticModel target_native_bit_plane_model(bool two_tensors = false) {
    SemanticModel model;
    model.maximum_context = 8;
    model.values = {
        {0, ScalarType::F32, {{DimensionKind::Symbol, 1}, {DimensionKind::Constant, 256}}, 0},
        {1, ScalarType::F32, {{DimensionKind::Symbol, 1}, {DimensionKind::Constant, 128}}, 0},
    };
    SemanticOperator linear;
    linear.id = 0;
    linear.kind = OperatorKind::Linear;
    linear.inputs = {0};
    linear.outputs = {1};
    linear.tensors = {0};
    linear.payload = LinearPayload{};
    model.tensors = {target_native_bit_plane_tensor()};
    if (two_tensors) {
        SemanticTensor second = target_native_bit_plane_tensor(88);
        second.role = TensorRole::QueryBias;
        second.dimensions = {{DimensionKind::Constant, 512}, {DimensionKind::Constant, 128}};
        second.layout.strides[1] = 512;
        second.planes[0].artifact_id = ArtifactId{9};
        second.planes[0].offset = 4096;
        second.planes[0].length = 16384;
        second.planes[1].artifact_id = ArtifactId{9};
        second.planes[1].offset = 20480;
        second.planes[1].length = 512;
        second.planes[2].artifact_id = ArtifactId{9};
        second.planes[2].offset = 20992;
        second.planes[2].length = 512;
        model.tensors.push_back(std::move(second));
        linear.tensors = {0, 1};
    }
    model.operators = {linear};
    return model;
}

KernelDescriptor target_native_bit_plane_descriptor(ExecutionPhase phase,
                                                    std::vector<PhysicalCodecIdentity> codecs) {
    KernelDescriptor result;
    result.id = phase == ExecutionPhase::Decode ? 7001 : 7002;
    result.implementation = KernelImplementation::ScalarLinear;
    result.pattern.operation = exact(OperatorKind::Linear);
    result.pattern.semantic_version = {1, 1};
    result.pattern.phase = exact(phase);
    result.pattern.logical_type = exact(ScalarType::F32);
    result.pattern.storage_type = exact(ScalarType::U8);
    result.pattern.layout = exact(PhysicalLayoutKind::GroupedAffine);
    result.pattern.quantization = exact(QuantizationKind::BlockedAffine);
    result.pattern.state_kind = any<StateKind>();
    result.pattern.state_format = any<StateFormatKind>();
    result.pattern.rank = {2, 2};
    result.pattern.alignment = {128, 128};
    result.pattern.required_physical_codecs = std::move(codecs);
    result.capability_predicate = &requires_scalar_fp32;
    result.numerical_class = NumericalClass::ExactFp32;
    result.transactional = true;
    result.effects = {BackendWriter::Metal, NumericalClass::ExactFp32,
...
void test_physical_codec_identity_is_source_neutral_and_complete() {
    const SemanticTensor source = target_native_bit_plane_tensor();
    const PhysicalCodecIdentity source_identity = target_native_bit_plane_identity(source, 1);
    CHECK(source_identity.planes.size() == 3);
    if (source_identity.planes.size() == 3) {
        CHECK(source_identity.planes[0].logical_elements_covered == 256);
        CHECK(source_identity.planes[0].bytes_per_block == 64);
        CHECK(source_identity.planes[1].logical_elements_covered == 256);
        CHECK(source_identity.planes[1].bytes_per_block == 2);
    }

    SemanticTensor different_identity = source;
    different_identity.id = 88;
    different_identity.role = TensorRole::FfnDownWeight;
    different_identity.dimensions = {{DimensionKind::Constant, 1024}, {DimensionKind::Constant, 64}};
    different_identity.layout.strides[1] = 1024;
    different_identity.planes[0] = {PlaneKind::Values, ScalarType::U8, ArtifactId{99}, 8192, 16384, 128, 0};
    different_identity.planes[1] = {PlaneKind::Scales, ScalarType::F16, ArtifactId{99}, 24576, 512, 128, 0};
    different_identity.planes[2] = {PlaneKind::Biases, ScalarType::F16, ArtifactId{99}, 25088, 512, 128, 0};
    CHECK(target_native_bit_plane_identity(different_identity, 1) == source_identity);

    SemanticTensor different_alignment = source;
    for (TensorPlane& plane : different_alignment.planes) plane.alignment = 64;
    CHECK(target_native_bit_plane_identity(different_alignment, 1) == source_identity);

    CHECK(target_native_bit_plane_identity(source, 2) != source_identity);

    SemanticTensor codebook = source;
    codebook.layout.kind = PhysicalLayoutKind::GgufBlocked;
    codebook.layout.packing = PackingKind::Gguf;
    codebook.quantization.kind = QuantizationKind::Codebook;
    const PhysicalCodecIdentity first_codebook = target_native_bit_plane_identity(codebook, 1, 7);
    const PhysicalCodecIdentity second_codebook = target_native_bit_plane_identity(codebook, 1, 8);
    CHECK(first_codebook != second_codebook);

    SemanticTensor malformed = source;
    malformed.quantization.group_size = 0;
    CHECK(!physical_codec_identity(malformed, 1, codec_digest(1), {}).has_value());

    malformed = source;
    malformed.planes[1].storage_type = ScalarType::F32;
    CHECK(target_native_bit_plane_identity(malformed, 1) != source_identity);

    CHECK(!physical_codec_identity(source, 0, codec_digest(1), {}).has_value());
    CHECK(!physical_codec_identity(source, 1, {}, {}).has_value());
    CHECK(!physical_codec_identity(source, 1, codec_digest(1), codec_digest(9)).has_value());
    CHECK(!physical_codec_identity(codebook, 1, codec_digest(1), {}).has_value());

    malformed = source;
    malformed.dimensions[0].kind = DimensionKind::Symbol;
    CHECK(!physical_codec_identity(malformed, 1, codec_digest(1), {}).has_value());
}

void test_unquantized_scalar_planes_have_exact_schema_width() {
    struct ScalarWidth {
        ScalarType type;
        uint32_t bytes;
    };
    constexpr std::array<ScalarWidth, 5> widths = {{
        {ScalarType::F32, 4}, {ScalarType::F16, 2}, {ScalarType::U32, 4},
        {ScalarType::I32, 4}, {ScalarType::U8, 1},
    }};
    for (const ScalarWidth width : widths) {
        PhysicalCodecIdentity invalid_elements;
        invalid_elements.arithmetic_version = 1;
        invalid_elements.arithmetic_digest = codec_digest(1);
        invalid_elements.layout.rank = 1;
        invalid_elements.layout.axis_order = {0, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff};
        invalid_elements.planes = {{PlaneKind::Values, width.type, 2, width.bytes, 0}};
        CHECK(!valid_physical_codec_identity(invalid_elements));

        PhysicalCodecIdentity invalid_bytes = invalid_elements;
        invalid_bytes.planes[0].logical_elements_covered = 1;
        invalid_bytes.planes[0].bytes_per_block = width.bytes + 1;
        CHECK(!valid_physical_codec_identity(invalid_bytes));

        SemanticTensor tensor;
        tensor.id = 1;
        tensor.role = TensorRole::QueryWeight;
        tensor.logical_type = width.type;
        tensor.dimensions = {{DimensionKind::Constant, 2}};
        tensor.layout.rank = 1;
        tensor.layout.axis_order = {0, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff};
        tensor.layout.strides[0] = 1;
        tensor.planes = {{PlaneKind::Values, width.type, ArtifactId{0}, 0,
                          static_cast<uint64_t>(2) * width.bytes, 1, 0}};
        CHECK(physical_codec_identity(tensor, 1, codec_digest(2), {}).has_value());
        tensor.planes[0].length = width.type == ScalarType::U8 ? 4 : 2;
        CHECK(!physical_codec_identity(tensor, 1, codec_digest(2), {}).has_value());
    }
}

void test_physical_codec_registry_wire_is_exact() {
    const SemanticModel model = target_native_bit_plane_model(true);
    const PhysicalCodecRegistry registry = target_native_bit_plane_registry(
        model.tensors[0], &model.tensors[1]);
    CHECK(validate_physical_codec_registry(registry, true));
    CHECK(physical_codec_registry_is_canonical(registry));
    CHECK(physical_codec_registry_covers_model(registry, model));

    const auto encoded_result = encode_physical_codec_registry(registry);
    CHECK(std::holds_alternative<std::vector<uint8_t>>(encoded_result));
    if (!std::holds_alternative<std::vector<uint8_t>>(encoded_result)) return;
    const std::vector<uint8_t>& encoded = std::get<std::vector<uint8_t>>(encoded_result);
    const auto decoded = decode_physical_codec_registry(encoded);
    CHECK(std::holds_alternative<PhysicalCodecRegistry>(decoded));
    if (const auto* roundtrip = std::get_if<PhysicalCodecRegistry>(&decoded)) {
        CHECK(*roundtrip == registry);
    }

    auto expect_decode_rejected = [](std::vector<uint8_t> bytes) {
        CHECK(std::holds_alternative<CompatibilityReport>(
            decode_physical_codec_registry(bytes)));
    };
    auto trailing = encoded;
    trailing.push_back(0);
    expect_decode_rejected(std::move(trailing));
    expect_decode_rejected({1, 0, 0, 0, 0, 0, 0, 0});

    PhysicalCodecRegistry single;
    single.codecs.push_back(registry.codecs.front());
    single.tensors.push_back(registry.tensors.front());
    const auto single_encoded_result = encode_physical_codec_registry(single);
    CHECK(std::holds_alternative<std::vector<uint8_t>>(single_encoded_result));
    if (!std::holds_alternative<std::vector<uint8_t>>(single_encoded_result)) return;
    const size_t identity_bytes =
        (std::get<std::vector<uint8_t>>(single_encoded_result).size() - 12) / 2;
    CHECK(identity_bytes != 0);
    if (identity_bytes == 0 || encoded.size() < 4 + identity_bytes * 2) return;

    auto reordered = encoded;
    for (size_t index = 0; index != identity_bytes; ++index) {
        std::swap(reordered[4 + index], reordered[4 + identity_bytes + index]);
    }
    expect_decode_rejected(std::move(reordered));

    auto duplicate = encoded;
    std::copy(duplicate.begin() + 4,
              duplicate.begin() + 4 + identity_bytes,
              duplicate.begin() + 4 + identity_bytes);
    expect_decode_rejected(std::move(duplicate));

    auto tampered = encoded;
    tampered[4 + 4] ^= 1; // arithmetic digest byte in the first identity
    expect_decode_rejected(std::move(tampered));

    auto missing = encoded;
    const size_t tensor_count_offset = 4 + identity_bytes * 2;
    missing[tensor_count_offset] = 1;
    missing[tensor_count_offset + 1] = 0;
    missing[tensor_count_offset + 2] = 0;
    missing[tensor_count_offset + 3] = 0;
    expect_decode_rejected(std::move(missing));

    PhysicalCodecRegistry reordered_registry = registry;
    std::reverse(reordered_registry.codecs.begin(), reordered_registry.codecs.end());
    CHECK(std::holds_alternative<CompatibilityReport>(
        encode_physical_codec_registry(reordered_registry)));
    PhysicalCodecRegistry duplicate_registry = registry;
    duplicate_registry.tensors.push_back(duplicate_registry.tensors.front());
    CHECK(std::holds_alternative<CompatibilityReport>(
        encode_physical_codec_registry(duplicate_registry)));
}

void test_empty_physical_codec_registry_is_a_valid_diagnostic_case() {
    const PhysicalCodecRegistry empty;
    const SemanticModel model;
    CHECK(validate_physical_codec_registry(empty));
    CHECK(physical_codec_registry_is_canonical(empty));
    CHECK(physical_codec_registry_covers_model(empty, model));
    const auto encoded = encode_physical_codec_registry(empty);
    CHECK(std::holds_alternative<std::vector<uint8_t>>(encoded));
    if (const auto* bytes = std::get_if<std::vector<uint8_t>>(&encoded)) {
        const auto decoded = decode_physical_codec_registry(*bytes);
        CHECK(std::holds_alternative<PhysicalCodecRegistry>(decoded));
        if (const auto* registry = std::get_if<PhysicalCodecRegistry>(&decoded)) {
            CHECK(registry->codecs.empty());
            CHECK(registry->tensors.empty());
        }
    }
}

void test_codec_aware_plan_binds_every_tensor_and_fails_closed() {
    const SemanticModel model = target_native_bit_plane_model(true);
    const PhysicalCodecRegistry codecs = target_native_bit_plane_registry(model.tensors[0], &model.tensors[1]);
    auto plan_request = request();
    plan_request.enable_prefill = false;
    plan_request.enable_decode = true;
    const std::vector<PhysicalCodecIdentity> required = {
        codecs.codecs[0].identity, codecs.codecs[1].identity};
    const std::vector<KernelDescriptor> kernels = {
        target_native_bit_plane_descriptor(ExecutionPhase::Decode, required),
        target_native_bit_plane_descriptor(ExecutionPhase::Output, required),
    };
    auto planned = plan_session(model, plan_request, cpu_capabilities(), kernels, codecs);
    ...
    if (const auto* plan = std::get_if<ExecutionPlan>(&planned)) {
        ...
        CHECK(plan->entries.front().codec_bindings.size() == 2);
        if (plan->entries.front().codec_bindings.size() == 2) {
            const PhysicalTensorCodecBinding& first = plan->entries.front().codec_bindings[0];
            const PhysicalTensorCodecBinding& second = plan->entries.front().codec_bindings[1];
            CHECK(plan->entries.front().codec_aware);
            CHECK(first.tensor_id == model.tensors[0].id);
            CHECK(first.identity == required[0]);
            CHECK(first.dimensions == model.tensors[0].dimensions);
            CHECK(first.strides == model.tensors[0].layout.strides);
            CHECK(first.planes.size() == model.tensors[0].planes.size());
            CHECK(second.tensor_id == model.tensors[1].id);
            CHECK(second.identity == required[1]);
            CHECK(second.dimensions == model.tensors[1].dimensions);
            CHECK(second.planes.size() == model.tensors[1].planes.size());
            if (!second.planes.empty()) {
                CHECK(second.planes[0].artifact_id == ArtifactId{9});
                CHECK(second.planes[0].offset == 4096);
                CHECK(second.planes[0].length == 16384);
            }
        }
        CHECK(plan_entry_matches(model, plan->entries.front()));
        PlanEntry tampered = plan->entries.front();
        tampered.codec_bindings[0].dimensions[0].constant_or_symbol++;
        CHECK(!plan_entry_matches(model, tampered));
        tampered = plan->entries.front();
        tampered.codec_bindings[0].planes[0].length++;
        CHECK(!plan_entry_matches(model, tampered));
        tampered = plan->entries.front();
        tampered.codec_bindings.clear();
        tampered.descriptor.pattern.required_physical_codecs.clear();
        CHECK(!plan_entry_matches(model, tampered));
        tampered = plan->entries.front();
        tampered.descriptor.pattern.required_physical_codecs[0].arithmetic_digest = codec_digest(77);
        CHECK(!plan_entry_matches(model, tampered));
    }

    PhysicalCodecRegistry reordered = codecs;
    std::reverse(reordered.codecs.begin(), reordered.codecs.end());
    const auto reordered_plan = plan_session(model, plan_request, cpu_capabilities(),
                                             kernels, reordered);
    CHECK(std::holds_alternative<CompatibilityReport>(reordered_plan));
    if (const auto* report = std::get_if<CompatibilityReport>(&reordered_plan)) {
        CHECK(report->code == CompatibilityError::KERNEL_UNAVAILABLE);
    }

    SemanticModel with_unreferenced_tensor = model;
    SemanticTensor unreferenced = with_unreferenced_tensor.tensors.front();
    unreferenced.id = 999;
    with_unreferenced_tensor.tensors.push_back(std::move(unreferenced));
    CHECK(physical_codec_registry_covers_model(codecs, with_unreferenced_tensor));
    const auto unreferenced_plan = plan_session(
        with_unreferenced_tensor, plan_request, cpu_capabilities(), kernels, codecs);
    CHECK(std::holds_alternative<CompatibilityReport>(unreferenced_plan));
    if (const auto* report = std::get_if<CompatibilityReport>(&unreferenced_plan)) {
        CHECK(report->code == CompatibilityError::KERNEL_UNAVAILABLE);
    }

    auto expect_rejected = [&](PhysicalCodecRegistry rejected) {
        const auto result = plan_session(model, plan_request, cpu_capabilities(), kernels, rejected);
    ...
    PhysicalCodecRegistry rejected = codecs;
    rejected.tensors.pop_back();
    expect_rejected(rejected);

    rejected = codecs;
    rejected.tensors.push_back(rejected.tensors.front());
    expect_rejected(rejected);

    rejected = codecs;
    rejected.tensors[1].identity = target_native_bit_plane_identity(model.tensors[1], 99);
    expect_rejected(rejected);

    rejected = codecs;
    rejected.codecs[1].identity.arithmetic_digest = {};
    expect_rejected(rejected);

    std::vector<KernelDescriptor> codec_agnostic = kernels;
    codec_agnostic[0].pattern.required_physical_codecs.clear();
    codec_agnostic[1].pattern.required_physical_codecs.clear();
    const auto ignored = plan_session(model, plan_request, cpu_capabilities(),
                                      codec_agnostic, codecs);
    CHECK(std::holds_alternative<CompatibilityReport>(ignored));

    SemanticModel aliased_tensor_ids = model;
    aliased_tensor_ids.tensors[1].id = aliased_tensor_ids.tensors[0].id;
    const PhysicalCodecIdentity aliased_identity =
        target_native_bit_plane_identity(aliased_tensor_ids.tensors[0], 1);
    PhysicalCodecRegistry aliased_registry;
    aliased_registry.codecs = {{aliased_identity}};
    aliased_registry.tensors = {{aliased_tensor_ids.tensors[0].id, aliased_identity}};
    const std::vector<KernelDescriptor> aliased_kernels = {
        target_native_bit_plane_descriptor(ExecutionPhase::Decode,
                                           {aliased_identity, aliased_identity}),
        target_native_bit_plane_descriptor(ExecutionPhase::Output,
                                           {aliased_identity, aliased_identity}),
    };
    const auto aliased = plan_session(aliased_tensor_ids, plan_request,
                                      cpu_capabilities(), aliased_kernels,
                                      aliased_registry);
    CHECK(std::holds_alternative<CompatibilityReport>(aliased));

    SemanticModel malformed = model;
    malformed.tensors[1].planes[0].offset = UINT64_MAX - 7;
    malformed.tensors[1].planes[0].length = 16;
    planned = plan_session(malformed, plan_request, cpu_capabilities(), kernels, codecs);
    CHECK(std::holds_alternative<CompatibilityReport>(planned));
}

void test_physical_codec_contract_rejects_malformed_storage() {
    const SemanticTensor source = target_native_bit_plane_tensor();
    auto identity_of = [&](const SemanticTensor& tensor) {
        return physical_codec_identity(tensor, 1, codec_digest(1), {});
    };

    SemanticTensor malformed = source;
    malformed.layout.packing = static_cast<PackingKind>(99);
    CHECK(!identity_of(malformed).has_value());
    malformed = source;
    malformed.layout.version = 2;
    CHECK(!identity_of(malformed).has_value());
    malformed = source;
    malformed.layout.flags = 1;
    CHECK(!identity_of(malformed).has_value());
    malformed = source;
    malformed.quantization.version = 2;
    CHECK(!identity_of(malformed).has_value());
    malformed = source;
    malformed.quantization.flags = 1;
    CHECK(!identity_of(malformed).has_value());
    malformed = source;
    malformed.quantization.kind = QuantizationKind::None;
    CHECK(!identity_of(malformed).has_value());
    malformed = source;
    malformed.planes[1].offset = 8064;
    CHECK(!identity_of(malformed).has_value());

    const SemanticModel model = target_native_bit_plane_model();
    const PhysicalCodecRegistry codecs = target_native_bit_plane_registry(model.tensors[0]);
    auto plan_request = request();
    plan_request.enable_prefill = false;
    plan_request.enable_decode = true;
    const std::vector<KernelDescriptor> kernels = {
    ...
    auto expect_rejected = [&](SemanticModel rejected) {
        const auto result = plan_session(rejected, plan_request, cpu_capabilities(),
                                         kernels, codecs);
        CHECK(std::holds_alternative<CompatibilityReport>(result));
    };

    SemanticModel rejected = model;
    rejected.tensors[0].layout.strides[1] = UINT64_MAX;
    expect_rejected(rejected);

    rejected = model;
    rejected.tensors[0].planes[1].offset = 8064;
    expect_rejected(rejected);

    rejected = model;
    rejected.tensors[0].planes.pop_back();
    expect_rejected(rejected);

    rejected = model;
    rejected.tensors[0].planes.push_back(
        {PlaneKind::Zeros, ScalarType::F16, ArtifactId{5}, 0, 256, 128, 0});
    expect_rejected(rejected);
}

void test_physical_codec_registry_is_not_a_32_bit_format_mask() {
    SemanticModel model = target_native_bit_plane_model();
    model.tensors.clear();
    model.operators[0].tensors.clear();
    PhysicalCodecRegistry codecs;
    std::vector<PhysicalCodecIdentity> required;
    for (uint32_t index = 0; index != 40; ++index) {
        SemanticTensor tensor = target_native_bit_plane_tensor(100 + index);
        for (TensorPlane& plane : tensor.planes) {
            plane.artifact_id = ArtifactId{100 + index};
        }
        const auto identity = physical_codec_identity(
            tensor, 1, codec_digest_index(index + 1), {});
        CHECK(identity.has_value());
        if (!identity) continue;
        model.operators[0].tensors.push_back(index);
        model.tensors.push_back(std::move(tensor));
        codecs.codecs.push_back({*identity});
        codecs.tensors.push_back({100 + index, *identity});
    ...
    auto planned = plan_session(model, plan_request,
                                cpu_capabilities(), kernels, codecs);
    CHECK(std::holds_alternative<ExecutionPlan>(planned));
    if (const auto* plan = std::get_if<ExecutionPlan>(&planned)) {
        CHECK(plan->entries.front().codec_bindings.size() == 40);
        CHECK(plan->entries.front().descriptor.pattern.required_physical_codecs == required);
    }

    auto expect_rejected = [&](PhysicalCodecRegistry rejected) {
        const auto result = plan_session(model, plan_request, cpu_capabilities(), kernels,
                                         rejected);
        CHECK(std::holds_alternative<CompatibilityReport>(result));
    };

    PhysicalCodecRegistry incomplete = codecs;
    incomplete.tensors.push_back({9999, incomplete.codecs.front().identity});
    expect_rejected(incomplete);

    incomplete = codecs;
    PhysicalCodecIdentity unused = incomplete.codecs.back().identity;
    unused.arithmetic_digest = codec_digest_index(9999);
    incomplete.codecs.push_back({unused});
    expect_rejected(incomplete);

    CHECK(kPhysicalCodecRegistryMaximumCodecs > 32);
    incomplete = codecs;
    for (uint32_t index = static_cast<uint32_t>(incomplete.codecs.size());
         index <= kPhysicalCodecRegistryMaximumCodecs; ++index) {
        PhysicalCodecIdentity identity = incomplete.codecs.front().identity;
        identity.arithmetic_digest = codec_digest_index(index + 10000);
        incomplete.codecs.push_back({std::move(identity)});
    }
    expect_rejected(std::move(incomplete));
}
...
void test_column_grouped_u2_recurrent_axis_contract() {
    RuntimeCapabilities metal;
    metal.global_fp32_kv = true;
    metal.transactional_state = true;
    metal.metal_device = true;
    metal.metal_library = true;
    metal.metal_pipeline = true;
    metal.metal_column_grouped_affine_u2_skip_256 = true;

    const SemanticModel model = column_grouped_u2_recurrent_model(256, 512);
    const auto planned = plan_canonical_metal_recurrent(
        model, request(), metal, builtin_canonical_metal_registry());
    CHECK_MSG(std::holds_alternative<ExecutionPlan>(planned),
              "U2 axis contract red: gate/up [256,512] and down [512,256] use physical row strides [256,512]");
    if (const auto* plan = std::get_if<ExecutionPlan>(&planned)) {
        CHECK(plan->entries.size() == 2);
    ...
    const auto reject = [&](uint32_t gate_up_stride, uint32_t down_stride) {
        const auto rejected = plan_canonical_metal_recurrent(
            column_grouped_u2_recurrent_model(gate_up_stride, down_stride),
            request(), metal, builtin_canonical_metal_registry());
...
void test_canonical_planner_rejects_invalid_rope_and_rms_contracts() {
    ...
    SemanticModel grouped_query_key = with_query_key_norms();
    grouped_query_key.schema_major = 8;
    grouped_query_key.opset_major = 8;
    for (SemanticOperator& op : grouped_query_key.operators) op.semantic_version = 8;
    for (SemanticState& state : grouped_query_key.states) state.semantic_version = 8;
    for (SemanticTensor& tensor : grouped_query_key.tensors) {
        if (tensor.role != TensorRole::AttentionQueryNormWeight &&
            tensor.role != TensorRole::AttentionKeyNormWeight) continue;
        tensor.dimensions[0].constant_or_symbol = 64;
        tensor.planes[0].length = 64 * sizeof(float);
    }
    for (SemanticOperator& op : grouped_query_key.operators) {
        if (auto* payload = std::get_if<RmsNormPayload>(&op.payload)) {
            const SemanticTensor* tensor = op.tensors.empty() ? nullptr :
                &grouped_query_key.tensors[op.tensors[0]];
            if (tensor && (tensor->role == TensorRole::AttentionQueryNormWeight ||
                           tensor->role == TensorRole::AttentionKeyNormWeight)) {
                payload->affine_geometry = RmsNormAffineGeometry::SharedAcrossGroups;
                payload->reduction_extent = 64;
            }
        } else if (auto* payload = std::get_if<CausalAttentionPayload>(&op.payload)) {
            payload->value_source_value = op.inputs[2];
        }
    }
    DenseGraphWitness grouped_witness;
    CHECK(match_canonical_dense_operator_edges(
        grouped_query_key, grouped_query_key.layers.front(), grouped_witness));
    const PlanResult grouped_plan = plan_canonical_metal(
        grouped_query_key, request(), metal, registry);
    CHECK(std::holds_alternative<ExecutionPlan>(grouped_plan));

    SemanticModel invalid_grouped_extent = grouped_query_key;
    for (SemanticOperator& op : invalid_grouped_extent.operators) {
        if (auto* payload = std::get_if<RmsNormPayload>(&op.payload);
            payload && payload->affine_geometry == RmsNormAffineGeometry::SharedAcrossGroups) {
            payload->reduction_extent = 63;
            break;
        }
    }
    DenseGraphWitness invalid_grouped_witness;
    CHECK(!match_canonical_dense_operator_edges(
        invalid_grouped_extent, invalid_grouped_extent.layers.front(), invalid_grouped_witness));
...
SemanticModel generic_moe_planner_model(uint32_t down_block_bytes = 22, bool q8_down = false,
    ...
    model.tensors.push_back(dense_tensor(12, TensorRole::RouterScaleWeight,
    ...
    model.tensors.push_back(dense_tensor(16, TensorRole::ReduceScaleWeight,
                                         {{DimensionKind::Constant, experts}}, ScalarType::F32));
    model.tensors.push_back(dense_tensor(17, TensorRole::ExpertNormWeight,
                                         {{DimensionKind::Constant, hidden}}, ScalarType::F32));
    const auto rows = [](uint32_t width) {
        return std::vector<Dimension>{{DimensionKind::Symbol, 1}, {DimensionKind::Constant, width}};
    };
...
PhysicalCodecIdentity moe_worklist_codec(uint8_t tag) {
    PhysicalCodecIdentity result;
    result.arithmetic_version = 1;
    result.arithmetic_digest[0] = tag;
    result.layout.kind = PhysicalLayoutKind::GgufBlocked;
    result.layout.packing = PackingKind::Gguf;
    result.layout.rank = 3;
    result.layout.block_rank = 1;
    result.layout.axis_order = {1, 2, 0, 0xff, 0xff, 0xff, 0xff, 0xff};
    result.layout.block_elements = 256;
    result.layout.block_bytes = 144;
    result.quantization.kind = QuantizationKind::BlockedAffine;
    result.quantization.accumulation_type = ScalarType::F32;
    result.quantization.block_elements = 256;
    result.quantization.block_bytes = 144;
    result.quantization.group_size = 256;
    result.quantization.required_plane_mask = 1;
    result.planes = {{PlaneKind::Values, ScalarType::U8, 256, 144, 0}};
    return result;
}

MoeWorklistTensorContract moe_worklist_tensor(uint32_t input, uint32_t output,
                                              const PhysicalCodecIdentity& codec) {
    MoeWorklistTensorContract result;
    result.input_width = input;
    result.output_width = output;
    result.expert_count = 4;
    result.dimensions = {{DimensionKind::Constant, 4},
                         {DimensionKind::Constant, input},
                         {DimensionKind::Constant, output}};
    result.strides = {1, output, static_cast<uint64_t>(input) * output, 0, 0, 0, 0, 0};
    result.layout = codec.layout;
    result.quantization = codec.quantization;
    result.planes = codec.planes;
    result.expert_stride = static_cast<uint64_t>(input) * output / 256 * codec.layout.block_bytes;
    return result;
}

MoeWorklistDescriptor moe_worklist_descriptor() {
    MoeWorklistDescriptor result;
    result.phase = ExecutionPhase::Decode;
    result.semantic_operator_ids = {20, 21, 22, 23, 24};
    result.expert_count = 4;
    result.selected_count = 2;
    result.hidden = 512;
    result.intermediate = 1024;
    result.activation = ActivationKind::Silu;
    result.scale_source = ExpertScaleSource::PerExpertTensor;
    result.gate_up = moe_worklist_tensor(512, 2048, moe_worklist_codec(1));
    result.down = moe_worklist_tensor(1024, 512, moe_worklist_codec(3));
    result.physical_codecs = {moe_worklist_codec(1), moe_worklist_codec(2), moe_worklist_codec(3)};
    result.router_scale_tensor_id = 25;
    result.expert_norm_tensor_id = 26;
    result.reduce_scale_tensor_id = 27;
    result.post_norm_tensor_id = 28;
    result.output_scale_tensor_id = 29;
    return result;
}

void test_moe_worklist_descriptor_is_typed_and_model_independent() {
    const MoeWorklistDescriptor descriptor = moe_worklist_descriptor();
    CHECK(valid_moe_worklist_descriptor(descriptor));
    CHECK(descriptor.semantic_operator_ids[0] == 20);
    CHECK(descriptor.gate_up.layout.kind == PhysicalLayoutKind::GgufBlocked);
    CHECK(descriptor.gate_up.planes.size() == 1);
    CHECK(descriptor.physical_codecs.size() == 3);
    CHECK(moe_worklist_matches(descriptor, descriptor));

    auto layout_mismatch = descriptor;
    layout_mismatch.down.layout.kind = PhysicalLayoutKind::GroupedAffine;
    CHECK(!valid_moe_worklist_descriptor(layout_mismatch));
    auto codec_mismatch = descriptor;
    codec_mismatch.physical_codecs[1].arithmetic_digest[0] = 99;
    CHECK(!moe_worklist_matches(codec_mismatch, descriptor));
    auto bad_counts = descriptor;
    bad_counts.selected_count = 5;
    CHECK(!valid_moe_worklist_descriptor(bad_counts));
}

void test_moe_worklist_selection_requires_device_worklist_capability() {
    const MoeWorklistDescriptor query = moe_worklist_descriptor();
    const std::vector<MoeWorklistDescriptor> registry = {query};
    RuntimeCapabilities missing;
    const auto unavailable = select_moe_worklist(query, missing, registry);
    CHECK(std::holds_alternative<CompatibilityReport>(unavailable));
    if (const auto* report = std::get_if<CompatibilityReport>(&unavailable))
        CHECK(report->code == CompatibilityError::CAPABILITY_MISSING);

    RuntimeCapabilities capable;
    capable.metal_moe_worklist = true;
    const auto selected = select_moe_worklist(query, capable, registry);
    CHECK(std::holds_alternative<MoeWorklistDescriptor>(selected));
    auto unsupported_codec = query;
    unsupported_codec.physical_codecs = {};
    const auto rejected = select_moe_worklist(unsupported_codec, capable, registry);
    CHECK(std::holds_alternative<CompatibilityReport>(rejected));
}
...
void test_moe_planner_rejects_ambiguous_and_nonmatching_graphs() {
    ...
    // Auxiliary routed tensors have distinct semantic roles.  Reusing a
    // legacy Nextn role must not make an otherwise similar graph admissible.
    for (uint32_t tensor_id : {12u, 16u, 17u}) {
        SemanticModel wrong_role = generic_moe_planner_model();
        wrong_role.tensors[tensor_id].role = TensorRole::NextnProjectionWeight;
        const PlanResult rejected = plan_canonical_metal(
            wrong_role, request(), capabilities, registry);
        CHECK(std::holds_alternative<CompatibilityReport>(rejected));
    }
}
...
int main(int argc, char** argv) {
    if (argc == 2 && std::string(argv[1]) == "--physical-codec-identity") {
        test_physical_codec_identity_is_source_neutral_and_complete();
        test_unquantized_scalar_planes_have_exact_schema_width();
        test_codec_aware_plan_binds_every_tensor_and_fails_closed();
        return test_summary("test_execution_plan");
    }
    if (argc == 2 && std::string(argv[1]) == "--physical-codec-contract") {
        test_physical_codec_contract_rejects_malformed_storage();
        return test_summary("test_execution_plan");
    }
    if (argc == 2 && std::string(argv[1]) == "--physical-codec-completeness") {
        test_physical_codec_registry_is_not_a_32_bit_format_mask();
        return test_summary("test_execution_plan");
    }
    if (argc == 2 && std::string(argv[1]) == "--u2-axis") {
        test_column_grouped_u2_recurrent_axis_contract();
        return test_summary("test_execution_plan");
    }
    test_physical_codec_identity_is_source_neutral_and_complete();
    test_unquantized_scalar_planes_have_exact_schema_width();
    test_physical_codec_registry_wire_is_exact();
    test_empty_physical_codec_registry_is_a_valid_diagnostic_case();
    test_codec_aware_plan_binds_every_tensor_and_fails_closed();
    test_physical_codec_contract_rejects_malformed_storage();
    test_physical_codec_registry_is_not_a_32_bit_format_mask();
    test_pattern_matrix();
    ...
    test_moe_worklist_descriptor_is_typed_and_model_independent();
    test_moe_worklist_selection_requires_device_worklist_capability();
    test_dense_graph_witness_closes_the_executed_dag();
    test_product_planner_provenance_shape();
    return test_summary("test_execution_plan");
}

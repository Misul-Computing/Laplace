...
void test_semantic_operator_arity() {
    for (uint16_t schema = 1; schema <= 8; ++schema) {
...
void test_semantic_wire_v8_grouped_rms_norm() {
    SemanticModel model = operator_arity_model(8);
    model.values = {
        {0, ScalarType::F32, {{DimensionKind::Constant, 4}}, 0},
        {1, ScalarType::F32, {{DimensionKind::Constant, 4}}, 0},
    };
    model.tensors[0].role = TensorRole::AttentionQueryNormWeight;
    model.tensors[0].dimensions = {{DimensionKind::Constant, 2}};
    model.tensors[0].planes[0].length = 8;
    model.operators = {
        {0, OperatorKind::RmsNorm, 8, {0}, {1}, {0}, {},
         RmsNormPayload{float_bits(1.0e-6f), -1, 1,
                        RmsNormAffineGeometry::SharedAcrossGroups, 2}},
    };
    ...
    if (!std::holds_alternative<std::vector<uint8_t>>(encoded)) return;
    const auto& bytes = std::get<std::vector<uint8_t>>(encoded);
    CHECK(std::memcmp(bytes.data(), "LAPIR008", 8) == 0);
    auto decoded = decode_semantic_model(bytes);
    CHECK(std::holds_alternative<SemanticModel>(decoded));
    if (const auto* roundtrip = std::get_if<SemanticModel>(&decoded)) {
        const auto* payload = std::get_if<RmsNormPayload>(&roundtrip->operators[0].payload);
        CHECK(payload != nullptr);
        if (payload) {
            CHECK(payload->affine_geometry == RmsNormAffineGeometry::SharedAcrossGroups);
            CHECK(payload->reduction_extent == 2);
        }
    }

    auto zero_extent = model;
    std::get<RmsNormPayload>(zero_extent.operators[0].payload).reduction_extent = 0;
    CHECK(std::holds_alternative<CompatibilityReport>(encode_semantic_model(zero_extent)));

    auto full_with_extent = model;
    auto& full_payload = std::get<RmsNormPayload>(full_with_extent.operators[0].payload);
    full_payload.affine_geometry = RmsNormAffineGeometry::FullWidth;
    CHECK(std::holds_alternative<CompatibilityReport>(encode_semantic_model(full_with_extent)));
}

void test_semantic_moe_tensor_roles_roundtrip() {
    const std::array<TensorRole, 5> roles = {
        TensorRole::RouterScaleWeight,
        TensorRole::ExpertNormWeight,
        TensorRole::ReduceScaleWeight,
        TensorRole::PostNormWeight,
        TensorRole::OutputScaleWeight,
    };
    for (TensorRole role : roles) {
        SemanticModel model = operator_arity_model(8);
        model.tensors[0].role = role;
        const auto encoded = encode_semantic_model(model);
        CHECK(std::holds_alternative<std::vector<uint8_t>>(encoded));
        if (!std::holds_alternative<std::vector<uint8_t>>(encoded)) continue;
        const auto decoded = decode_semantic_model(std::get<std::vector<uint8_t>>(encoded));
        CHECK(std::holds_alternative<SemanticModel>(decoded));
        if (const auto* roundtrip = std::get_if<SemanticModel>(&decoded))
            CHECK(roundtrip->tensors[0].role == role);
    }
}
...
int main() {
    ...
    test_semantic_wire_v8_grouped_rms_norm();
    test_semantic_moe_tensor_roles_roundtrip();

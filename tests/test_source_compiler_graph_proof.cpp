...
void test_rejects_stale_grouped_normalization_geometry() {
    auto model = compact_decoder();
    model.operators[1].semantic_version = 8;
    model.operators[1].payload = RmsNormPayload{
        0x358637bdu, -1, 1, RmsNormAffineGeometry::SharedAcrossGroups, 1};
    const auto result = prove_source_candidate_graph(model);
    CHECK(std::holds_alternative<SourceCompilerGraphProof>(result));
    if (const auto* proof = std::get_if<SourceCompilerGraphProof>(&result)) {
        auto changed = model;
        std::get<RmsNormPayload>(changed.operators[1].payload).reduction_extent = 2;
        CHECK(!source_compiler_graph_proof_matches(changed, *proof));
    }
}
...
int main() {
    ...
    test_rejects_stale_grouped_normalization_geometry();

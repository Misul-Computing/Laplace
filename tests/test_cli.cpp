...
void test_raw_gguf_with_incomplete_tokenizer_contract_fails_closed() {
    ...
    CHECK(status != 0);
    CHECK(output.find("no closed data-only semantic schema matches this GGUF source") !=
          std::string::npos);
    std::remove(path);
}

void test_self_built_manifest_cannot_enter_default_generation() {
    ...
    CHECK(status != 0);
    CHECK(default_mode.find("code=48") != std::string::npos);
    CHECK(default_mode.find("verified trust certificate") != std::string::npos);
    ...
    const std::string benchmark = run_cli(
        ...
        &status);
    CHECK(status != 0);
    CHECK(benchmark.find("verified trust certificate") != std::string::npos);
    ...
    const std::string no_decode = run_cli(
        ...
        &status);
    CHECK(status != 0);
    CHECK(no_decode.find("verified trust certificate") != std::string::npos);
...
void test_untrusted_package_cannot_supply_a_context_limit() {
    ...
    CHECK(status != 0);
    CHECK(output.find("verified trust certificate") != std::string::npos);
...
int main() {
    ...
    test_self_built_manifest_cannot_enter_default_generation();

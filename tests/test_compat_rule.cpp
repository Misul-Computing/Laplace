...
void test_bundled_first_dense_rule() {
    const std::span<const uint8_t> record = bundled_compatibility_rule_record();
    CHECK(record.size() == 110264);
    CHECK(record.size() >= 8 && std::memcmp(record.data(), "LAPRUL10", 8) == 0);
    std::vector<uint8_t> record_copy(record.begin(), record.end());
    auto record_decoded = decode_compatibility_rule(record_copy);
    CHECK(std::holds_alternative<CompatibilityRule>(record_decoded));
    if (auto* decoded = std::get_if<CompatibilityRule>(&record_decoded)) {
        CHECK(rule_fingerprint(*decoded).hex() == "da0ecff3a0e97b3f61338fa301416da841e4401b853f5785dfe56c33baca4263");
        auto reencoded = encode_compatibility_rule(*decoded);
        CHECK(std::holds_alternative<std::vector<uint8_t>>(reencoded));
        if (auto* bytes = std::get_if<std::vector<uint8_t>>(&reencoded)) CHECK(*bytes == record_copy);
    }
    const auto& rules = bundled_compatibility_rules();
    CHECK(rules.size() == 1);
    if (rules.size() == 1) {
        CHECK(rules[0].qualification_state == RuleQualificationState::CanonicalRuntimePassed);
        CHECK(rules[0].semantic_template.maximum_context == 32768);
        CHECK(rules[0].semantic_template.tensors.size() == 291);
        CHECK(rules[0].tensors.size() == 14);
        CHECK(rules[0].semantic_template.values.size() == 364);
        CHECK(rules[0].semantic_template.operators.size() == 339);
        CHECK(rules[0].semantic_template.layers.size() == 24);
        CHECK(rules[0].semantic_template.states.size() == 48);
        CHECK(rules[0].capabilities == rules[0].semantic_template.capabilities);
        CHECK(rules[0].fallbacks == rules[0].semantic_template.fallbacks);
        CHECK(rules[0].semantic_template_digest != Sha256Digest{});
        CHECK(rules[0].semantic_template.tensors.back().role == TensorRole::OutputWeight);
    }
}

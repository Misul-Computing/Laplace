...
bool rms_affine_contract(const SemanticModel& model, const SemanticOperator& op,
                         TensorRole role, uint32_t logical_width,
                         uint32_t shared_extent) {
    const auto* payload = std::get_if<RmsNormPayload>(&op.payload);
    const SemanticTensor* tensor = tensor_with_role(model, op, role);
    if (!payload || payload->weight_mode != 1 || !tensor ||
        tensor->dimensions.size() != 1 ||
        tensor->dimensions[0].kind != DimensionKind::Constant) return false;
    uint32_t affine_width = 0;
    if (payload->affine_geometry == RmsNormAffineGeometry::FullWidth) {
        if (payload->reduction_extent != 0 || shared_extent != 0) return false;
        affine_width = logical_width;
    } else if (payload->affine_geometry == RmsNormAffineGeometry::SharedAcrossGroups) {
        if (op.semantic_version < 8 || shared_extent == 0 ||
            payload->reduction_extent != shared_extent ||
            logical_width % shared_extent != 0) return false;
        affine_width = shared_extent;
    } else {
        return false;
    }
    return tensor->dimensions[0].constant_or_symbol == affine_width;
}
...
bool owns_fp32_global_kv(const SemanticModel& model, const SemanticOperator& attention,
    ...
    const bool valid = key && value && key->id != value->id &&
           ...
           (key->semantic_version == 1 || key->semantic_version == 4 ||
            key->semantic_version == 5 || key->semantic_version == 6 ||
            key->semantic_version == 7 || key->semantic_version == 8) &&
...
struct CanonicalMetalProgram::Impl {
    ...
    bool product_pipeline_lease = false;
    std::shared_ptr<const ProductMetalExecutionContract> product_contract;
    uint32_t prefill_state_program = UINT32_MAX;
    uint32_t prefill_logits_program = UINT32_MAX;
...
bool product_structural_contract_matches_lowering(
    const ProductMetalExecutionContract& contract,
    const CanonicalProgramWitness& lowering,
    const std::vector<std::variant<DenseLayer, RecurrentLayer>>& layers,
    bool& prefill_batch, std::string& detail) {
    const auto semantics = contract.semantic_programs();
    const auto structural = contract.structural_programs();
    if (semantics.empty() || semantics.size() != structural.size()) {
        detail = "product Metal semantic and structural program counts differ";
        return false;
    }
    prefill_batch = false;
    ...
    for (size_t program_index = 0; program_index != structural.size();
        ...
        const StructuralMetalProgramBundle& program = structural[program_index];
        const ProductMetalProgramKind kind = [&]() {
            if (semantic.request.phase == ExecutionPhase::Decode)
                return semantic.request.include_greedy_sampler
                    ? ProductMetalProgramKind::DecodeGreedySample
        ...
        const MetalTokProgramRange* range =
            binding ? contract.range(binding->id) : nullptr;
        if (!binding || binding->semantic_program_index != program_index ||
            !range || !semantic.output_binding ||
            range->batch_rows != semantic.request.batch_rows ||
            range->selected_output_row !=
                semantic.output_binding->selected_row ||
            range->semantic_program_digest != semantic.program_digest.bytes ||
            program.phase() != semantic.request.phase ||
            program.batch_rows() != semantic.request.batch_rows ||
            program.program_digest() != semantic.program_digest) {
            detail = "product Metal program identity is not aligned with its structural bundle";
            return false;
        }
        ...
        const size_t expected_groups =
            layers.size() + 2u +
            static_cast<size_t>(semantic.request.include_greedy_sampler);
        if (groups.size() != expected_groups ||
            groups.front().kind() != StructuralMetalBundleGroupKind::Graph ||
            groups.front().shape() !=
                StructuralMetalExecutionShape::GraphEmbedding ||
            std::find(groups.front().covered_operator_ids().begin(),
                      groups.front().covered_operator_ids().end(),
                      lowering.embedding_operator_id) ==
                groups.front().covered_operator_ids().end()) {
            detail = "product Metal structural graph does not match the lowered embedding boundary";
            return false;
        }
        for (size_t layer_index = 0; layer_index != layers.size(); ++layer_index) {
            const StructuralMetalBundleGroup& group = groups[layer_index + 1];
            const uint32_t lowered_operator = std::visit(
                [](const auto& layer) { return layer.operator_id; },
                layers[layer_index]);
            const StructuralMetalExecutionShape expected_shape = std::visit(
                [](const auto& layer) {
                    using Layer = std::decay_t<decltype(layer)>;
                    if constexpr (std::is_same_v<Layer, RecurrentLayer>) {
                        return StructuralMetalExecutionShape::RecurrentDelta;
                    } else {
                        return layer.metal.moe_gate
                            ? StructuralMetalExecutionShape::MoeRoutedFeedForward
                            : StructuralMetalExecutionShape::DenseAttention;
                    }
                }, layers[layer_index]);
            if (group.kind() != StructuralMetalBundleGroupKind::Layer ||
                group.shape() != expected_shape ||
                group.phase() != semantic.request.phase ||
                group.batch_rows() != semantic.request.batch_rows ||
                std::find(group.covered_operator_ids().begin(),
                          group.covered_operator_ids().end(), lowered_operator) ==
                    group.covered_operator_ids().end()) {
                detail = "product Metal structural layer does not match the lowered executor shape";
                return false;
            }
            if (prefill_batch &&
                expected_shape != StructuralMetalExecutionShape::DenseAttention) {
                detail = "product Metal batched prefill requires an exact dense-attention program";
                return false;
            }
        }
        const size_t final_index = layers.size() + 1;
        const StructuralMetalBundleGroup& final = groups[final_index];
        const auto covers_final = [&](uint32_t operator_id) {
            return std::find(final.covered_operator_ids().begin(),
                             final.covered_operator_ids().end(), operator_id) !=
                   final.covered_operator_ids().end();
        };
        if (final.kind() != StructuralMetalBundleGroupKind::Final ||
            final.shape() != StructuralMetalExecutionShape::FinalOutput ||
            !covers_final(lowering.final_norm_operator_id) ||
            !covers_final(lowering.output_operator_id)) {
            detail = "product Metal structural graph does not match the lowered final-output boundary";
            return false;
        }
        if (semantic.request.include_greedy_sampler) {
            const StructuralMetalBundleGroup& sampler = groups.back();
            if (sampler.kind() != StructuralMetalBundleGroupKind::Sampler ||
                sampler.shape() != StructuralMetalExecutionShape::GreedySampler) {
                detail = "product Metal structural graph does not match the device sampler boundary";
                return false;
            }
        }
    }
...
CanonicalMetalCreateResult create_canonical_metal_program_internal(
    ...
    ProductMetalPreparedSession* prepared_product_session) {
    ...
    for (const SemanticLayer& layer : model.layers) {
        ...
        const uint64_t query_width_u64 = static_cast<uint64_t>(attention_payload->query_heads) *
                                         attention_payload->head_dimension;
        const uint64_t key_width_u64 = static_cast<uint64_t>(attention_payload->kv_heads) *
                                       attention_payload->head_dimension;
        if (query_width_u64 == 0 || query_width_u64 > UINT32_MAX ||
            key_width_u64 == 0 || key_width_u64 > UINT32_MAX ||
            !rms_affine_contract(model, *attn_norm, TensorRole::AttentionNormWeight,
                                 impl->hidden, 0) ||
            !rms_affine_contract(model, *ffn_norm, TensorRole::FfnNormWeight,
                                 impl->hidden, 0)) {
            return metal_error(CompatibilityError::KERNEL_UNAVAILABLE,
                               "canonical dense RMS geometry is unsupported");
        }
        if (query_norm) {
            const auto grouped_or_single_head = [&](const SemanticOperator& op,
                                                    TensorRole role, uint32_t heads,
                                                    uint32_t logical_width) {
                const auto* payload = std::get_if<RmsNormPayload>(&op.payload);
                if (!payload) return false;
                if (payload->affine_geometry == RmsNormAffineGeometry::SharedAcrossGroups) {
                    return rms_affine_contract(model, op, role, logical_width,
                                               attention_payload->head_dimension);
                }
                return heads == 1 && rms_affine_contract(model, op, role,
                                                         logical_width, 0);
            };
            if (!grouped_or_single_head(*query_norm,
                                        TensorRole::AttentionQueryNormWeight,
                                        attention_payload->query_heads,
                                        static_cast<uint32_t>(query_width_u64)) ||
                !grouped_or_single_head(*key_norm,
                                        TensorRole::AttentionKeyNormWeight,
                                        attention_payload->kv_heads,
                                        static_cast<uint32_t>(key_width_u64))) {
                return metal_error(CompatibilityError::KERNEL_UNAVAILABLE,
                                   "canonical dense grouped Q/K RMS geometry is unsupported");
            }
        }
        ...
        if (is_moe &&
            (!assign_single(*moe_router, dense.moe_gate) ||
             !assign(*moe_edges.router_scale, TensorRole::RouterScaleWeight,
                     dense.moe_gate_scale, true, false) ||
             !assign(*moe_norm, TensorRole::ExpertNormWeight, dense.moe_pre_norm, true, false) ||
             ...
             !assign(*moe_edges.expert_reduce, TensorRole::ReduceScaleWeight,
                     dense.moe_down_scale, true, false) ||
             (moe_edges.dense_post_norm &&
              !assign(*moe_edges.dense_post_norm, TensorRole::PostNormWeight,
                      dense.post_ffw_1, true, false)) ||
             (moe_edges.moe_post_norm &&
              !assign(*moe_edges.moe_post_norm, TensorRole::PostNormWeight,
                      dense.post_ffw_2, true, false)) ||
             (moe_edges.output_post_norm &&
              !assign(*moe_edges.output_post_norm, TensorRole::PostNormWeight,
                      dense.post_ffw, true, false)) ||
             (moe_edges.output_scale &&
              !assign(*moe_edges.output_scale, TensorRole::OutputScaleWeight,
        ...
        if (is_moe) {
            ...
            if (dense.out_scale.data) {
                if (dense.out_scale.data_bytes < sizeof(uint32_t)) {
                    return metal_error(
                        CompatibilityError::IR_REFERENCE_INVALID,
                        "canonical Metal MoE output scale storage is truncated");
                }
                std::memcpy(&dense.metal.moe_output_scale_bits,
                            dense.out_scale.data, sizeof(uint32_t));
                const float output_scale =
                    std::bit_cast<float>(dense.metal.moe_output_scale_bits);
                if (!std::isfinite(output_scale)) {
                    return metal_error(
                        CompatibilityError::IR_REFERENCE_INVALID,
                        "canonical Metal MoE output scale is not finite");
                }
            }
    ...
    if (prepared_product_session) {
        const ProductMetalExecutionContract* product_contract =
            prepared_product_session->contract();
        impl->product_pipeline_lease = prepared_product_session->valid();
        if (!impl->product_pipeline_lease || !product_contract) {
            return metal_error(
                CompatibilityError::SESSION_CONSTRUCTION_FAILED,
                "canonical Metal product preparation omitted its immutable execution contract");
        }
        impl->prefill_state_program = prepared_product_session->program_id(
            ProductMetalProgramKind::PrefillStateOnly);
        impl->prefill_logits_program = prepared_product_session->program_id(
            ProductMetalProgramKind::PrefillLogits);
        impl->prefill_sample_program = prepared_product_session->program_id(
            ProductMetalProgramKind::PrefillGreedySample);
        impl->prefill_batch2_logits_program =
        ...
        impl->decode_sample_program = prepared_product_session->program_id(
            ProductMetalProgramKind::DecodeGreedySample);
        impl->product_contract = prepared_product_session->take_contract();
        impl->metal_session = prepared_product_session->take_session();
    } else {
...
#endif
    if (!impl->product_pipeline_lease &&
        !metal_tok_session_dense_ready(*impl->metal_session)) {
        return metal_error(CompatibilityError::CAPABILITY_MISSING,
                           "canonical Metal required pipeline capabilities are unavailable");
    }
    if (!impl->product_pipeline_lease && impl->has_recurrent &&
        !metal_tok_session_recurrent_ready(*impl->metal_session)) {
        return metal_error(CompatibilityError::SESSION_CONSTRUCTION_FAILED,
    ...
    if (impl->product_pipeline_lease) {
        ...
        if (!impl->product_contract ||
            !product_structural_contract_matches_lowering(
                *impl->product_contract, lowering_program, impl->layers,
                impl->prefill_batch, contract_detail)) {
            return metal_error(
                CompatibilityError::IR_REFERENCE_INVALID,
                contract_detail.empty()
                    ? "canonical Metal product execution contract is unavailable"
                    : std::move(contract_detail));
        }
        impl->plan.codec_aware = true;
        impl->plan.package_fingerprint = impl->package->package_fingerprint();
        impl->plan.program = lowering_program;
        impl->plan.layer_chain = lowering_program.layers;
        impl->plan.dispatch_programs.assign(
            impl->product_contract->semantic_programs().begin(),
            impl->product_contract->semantic_programs().end());
        impl->plan.dispatch_pipeline_slots.resize(
            impl->plan.dispatch_programs.size());
        ...
        capabilities.metal_affine_u2_256 =
            metal_tok_session_affine_u2_256_ready(*impl->metal_session);
        capabilities.metal_column_grouped_affine_u2_skip_256 =
            metal_tok_session_column_grouped_affine_u2_skip_256_ready(
                *impl->metal_session);
        ...
        PlanResult planned = plan_canonical_metal(
            execution_model, request, capabilities,
        ...
        impl->prefill_batch =
            impl->sparse_ffn_layers == 0 && !selected_plan.entries.empty() &&
            std::all_of(selected_plan.entries.begin(),
                        selected_plan.entries.end(),
                        [](const PlanEntry& entry) {
                            return entry.phase != ExecutionPhase::Prefill ||
                                   entry.descriptor.implementation ==
                                       KernelImplementation::MetalDensePrefillBatch;
                        });
        if (impl->prefill_batch) {
            ...
            const PlanResult token_planned = plan_canonical_metal(
                execution_model, token_request, capabilities,
                builtin_canonical_metal_registry());
            if (const auto* report =
                    std::get_if<CompatibilityReport>(&token_planned))
                return *report;
...
CanonicalMetalCreateResult create_canonical_metal_program(
    ...
    ProductMetalPreparedSessionResult prepared_result =
        prepare_product_metal_session(*package, request);
    if (const auto* report =
            std::get_if<CompatibilityReport>(&prepared_result)) {
        return *report;
    }
    ProductMetalPreparedSession prepared =
        std::get<ProductMetalPreparedSession>(std::move(prepared_result));
    return create_canonical_metal_program_internal(std::move(package), request, derived_q2,
                                                   sparse_ffn, derived_iq2_xxs, calibration,
                                                   derived_column_u2, &prepared);
}
...
CanonicalMetalRunResult CanonicalMetalProgram::run(std::span<const uint32_t> token_ids,
    ...
    if (impl_->product_pipeline_lease) {
        ...
        if (phase == ExecutionPhase::Prefill) {
            if (output_mode == OutputMode::None) {
                ...
                product_program = produce_sample
                    ...
                    : impl_->prefill_logits_program;
            }
        } else if (phase == ExecutionPhase::Decode && output_mode != OutputMode::None) {
            product_program = produce_sample ? impl_->decode_sample_program
                                             : impl_->decode_logits_program;
        }
        if (product_program == UINT32_MAX ||
            !metal_tok_session_select_program(*impl_->metal_session,
                                              product_program)) {
            return metal_error(
                CompatibilityError::KERNEL_UNAVAILABLE,
                "canonical Metal has no exact leased program for the requested phase and output mode");
        }
        if (output_mode != OutputMode::None) {
            const MetalTokProgramRange* range =
                impl_->product_contract
                    ? impl_->product_contract->range(product_program)
                    : nullptr;
            if (!range || range->selected_output_row >= token_ids.size()) {
                return metal_error(
                    CompatibilityError::KERNEL_UNAVAILABLE,
                    "canonical Metal product program has no valid semantic output row");
            }
            selected_output_row = range->selected_output_row;
        }
    }
    ...
    if (prefill_batch_transaction) {
        ...
        if (!tokens_valid ||
            ...
            const char* failure =
                metal_tok_session_last_failure(*impl_->metal_session);
            std::string detail =
                "canonical Metal F16 prefill batch submission failed";
            if (failure && failure[0] != '\0') detail += ": " + std::string(failure);
        ...
        for (const auto& program_layer : impl_->layers) {
            ...
            const bool selected = impl_->product_pipeline_lease ||
                (planned != impl_->plan.entries.end() &&
                 planned->descriptor.implementation ==
                     KernelImplementation::MetalDensePrefillBatch);
            if (!selected ||
    ...
    for (size_t index = 0; index != token_ids.size(); ++index) {
        ...
        for (const auto& program_layer : impl_->layers) {
            ...
            if (!impl_->product_pipeline_lease) {
                if (planned == active_plan.entries.end() ||
                    planned->kernel_id != planned->descriptor.id) {
                    metal_tok_session_abort(*impl_->metal_session);
                    return metal_error(
                        CompatibilityError::KERNEL_UNAVAILABLE,
                        "canonical Metal has no selected descriptor for a layer");
                }
                const bool expected_dense =
                    std::holds_alternative<DenseLayer>(program_layer);
                const KernelImplementation expected_implementation =
                    expected_dense ? KernelImplementation::MetalDenseToken
                                   : KernelImplementation::MetalRecurrentToken;
                if (planned->descriptor.implementation !=
    ...
    output.command_buffers = metrics.command_buffers;

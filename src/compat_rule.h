...
public:
    ...
    // Expose the immutable proof carried by the manifest.
    ...
    bool product_authoritative() const noexcept {
        return authority_kind_ == PackageAuthorityKind::ClosedV1Compiled;
    }
...
// Structurally validates an immutable carried manifest. A carrier is diagnostic
// until a separate trust certificate exists; it cannot grant product authority.

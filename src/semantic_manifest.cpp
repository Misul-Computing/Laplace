...
struct SemanticManifest::Data {
    ...
    Sha256Digest physical_codec_registry_digest{};
...
// Body version 5 additionally carries one canonical, typed token authority
// descriptor and the exact physical codec registry after the transport
// payload. A complete LAPIR payload remains transport data, not graph identity.
constexpr uint32_t kBodyVersion = 5;
...
bool tensor_binding_matches(const SemanticTensor& semantic, const ArtifactTensorRecord& physical) {
    ...
    if (semantic.expert_axis.kind == ExpertAxisKind::ExpertBank) {
        // The semantic expert axis may be a normalized view of a source
        // axis.  Resolve the source bank through the declared axis map while
        // retaining the artifact's physical dimensions and strides.
        if (physical.coordinate.bank_axis >= physical.logical_dimensions.size() ||
            semantic.expert_axis.expert_axis >= semantic.layout.rank ||
            semantic.layout.axis_order[0] == 0xff) return false;
        uint8_t source_axis = UINT8_MAX;
        for (uint8_t position = 0; position != semantic.layout.rank; ++position) {
            if (semantic.layout.axis_order[position] == semantic.expert_axis.expert_axis) {
                source_axis = physical.layout.axis_order[position];
                break;
            }
        }
        if (source_axis == UINT8_MAX || physical.coordinate.bank_axis != source_axis) return false;
    }
...
bool physical_bindings_match(const ArtifactIndex& physical, const SemanticModel& model) {
    ...
    for (const SemanticTensor& tensor : model.tensors) {
        ...
        if (!found || !used.insert(tensor.id).second || !tensor_binding_matches(tensor, *found))
            return false;
    }
...
bool model_contract_supported(const SemanticModel& model) {
    if (model.schema_minor != 0 || model.opset_minor != 0 ||
        ...
        model.schema_major > 8 || model.entry_kind != EntryKind::TokenIds) {
        return false;
    }
...
bool codec_certificates_are_authoritative(const PhysicalCodecRegistry& registry) {
    if (registry.codecs.empty()) return registry.tensors.empty();
    for (const PhysicalCodecSpec& spec : registry.codecs) {
        ...
        if (!certificate ||
            certificate->identity().abi_version != spec.identity.arithmetic_version ||
            certificate->identity().digest != spec.identity.arithmetic_digest ||
            certificate->canonical_bytes().size() != spec.certificate_bytes.size() ||
            !std::equal(certificate->canonical_bytes().begin(),
                        certificate->canonical_bytes().end(),
                        spec.certificate_bytes.begin())) {
            return false;
        }
    }
    return true;
...
Sha256Digest compute_package_digest(const ArtifactIndex& physical, const Sha256Digest& graph,
                                    ...
                                    const Sha256Digest& codec_registry_digest) {
...
std::vector<uint8_t> make_body(const ArtifactIndex& physical, const TokenContract& contract,
                               ...
                               const Sha256Digest& codec_registry_digest_value,
                               const std::vector<uint8_t>& semantic_bytes,
                               const std::vector<uint8_t>& codec_registry_bytes) {
    ...
    append_digest(body, codec_registry_digest_value);
    ...
    append_u64(body, codec_registry_bytes.size());
    body.insert(body.end(), codec_registry_bytes.begin(), codec_registry_bytes.end());
    return body;
}
...
std::shared_ptr<const SemanticManifest::Data> make_data(const ArtifactIndex& physical,
    ...
    data->codec_registry = std::move(codec_registry);
    ...
    data->physical_codec_registry_digest = codec_registry_digest;
...
SemanticManifestResult SemanticManifest::build(const ArtifactIndex& physical,
                                               const SemanticModel& model,
                                               const TokenContract& contract) {
    return build(physical, model, contract, PhysicalCodecRegistry{});
}
...
SemanticManifestResult SemanticManifest::build(const ArtifactIndex& physical,
    ...
    if ((!codec_registry.codecs.empty() || !codec_registry.tensors.empty()) &&
        (!validate_physical_codec_registry(codec_registry) ||
         !physical_codec_registry_is_canonical(codec_registry) ||
         !physical_codec_registry_matches_model(codec_registry, model) ||
         !codec_certificates_are_authoritative(codec_registry))) {
        return manifest_error(CompatibilityError::IR_QUANTIZATION_UNSUPPORTED,
                              "physical codec registry is incomplete, invalid, or not bound to every semantic tensor");
    }
    auto encoded_codecs = encode_physical_codec_registry(codec_registry);
    if (const auto* report = std::get_if<CompatibilityReport>(&encoded_codecs)) return *report;
    const std::vector<uint8_t> codec_registry_bytes =
        std::get<std::vector<uint8_t>>(std::move(encoded_codecs));
    const Sha256Digest codec_digest = ::Laplace::physical_codec_registry_digest(codec_registry, model);
    if ((!codec_registry.codecs.empty() || !codec_registry.tensors.empty()) && codec_digest == Sha256Digest{}) {
        return manifest_error(CompatibilityError::IR_QUANTIZATION_UNSUPPORTED,
                              "physical codec registry digest could not be established");
    }
    ...
    const Sha256Digest binding = compute_binding_digest(physical, model);
    const Sha256Digest interaction = compute_interaction_digest(contract);
    const Sha256Digest package = compute_package_digest(physical, graph, model, contract, codec_digest);
    const std::vector<uint8_t> body = make_body(physical, contract, graph, binding, interaction, package,
                                                codec_digest, semantic_bytes, codec_registry_bytes);
    ...
    return SemanticManifest(make_data(physical, model, contract, semantic_bytes, std::move(envelope),
                                      graph, binding, interaction, package, codec_registry,
                                      codec_digest, std::move(graph_proof)));
...
SemanticManifestResult SemanticManifest::decode(const ArtifactIndex& physical,
    ...
    Sha256Digest encoded_codec_registry_digest;
    std::span<const uint8_t> graph_digest_span, binding_digest_span, interaction_digest_span,
        package_digest_span, codec_registry_digest_span;
    if (!body.take(32, graph_digest_span) || !body.take(32, binding_digest_span) ||
        ...
        !body.take(32, codec_registry_digest_span)) {
    ...
    std::copy(codec_registry_digest_span.begin(), codec_registry_digest_span.end(),
              encoded_codec_registry_digest.bytes.begin());
    ...
    if (!body.take(static_cast<size_t>(token_authority_length), token_authority_span)) {
        return manifest_error(CompatibilityError::PACKAGE_BOUNDS_INVALID,
                              "manifest token authority descriptor is truncated");
    }
    uint64_t codec_registry_length = 0;
    if (!body.u64(codec_registry_length) || codec_registry_length > body.remaining() ||
        !fits_size(codec_registry_length)) {
        return manifest_error(CompatibilityError::IR_QUANTIZATION_UNSUPPORTED,
                              "manifest physical codec registry is missing or out of bounds");
    }
    std::span<const uint8_t> codec_registry_span;
    if (!body.take(static_cast<size_t>(codec_registry_length), codec_registry_span) || body.remaining() != 0) {
        return manifest_error(CompatibilityError::IR_VERSION_UNSUPPORTED,
    ...
    auto decoded_codecs = decode_physical_codec_registry(codec_registry_span);
    if (const auto* report = std::get_if<CompatibilityReport>(&decoded_codecs)) return *report;
    PhysicalCodecRegistry codec_registry =
        std::get<PhysicalCodecRegistry>(std::move(decoded_codecs));
    if ((!codec_registry.codecs.empty() || !codec_registry.tensors.empty()) &&
        (!physical_codec_registry_matches_model(codec_registry, model) ||
         !codec_certificates_are_authoritative(codec_registry))) {
        return manifest_error(CompatibilityError::IR_QUANTIZATION_UNSUPPORTED,
                              "manifest physical codec registry is incomplete or does not bind every semantic tensor");
    }
    const Sha256Digest codec_digest = ::Laplace::physical_codec_registry_digest(codec_registry, model);
    if (encoded_codec_registry_digest != codec_digest) {
        return manifest_error(CompatibilityError::PACKAGE_CHECKSUM_MISMATCH,
                              "manifest physical codec registry digest is invalid");
    }
    const SemanticModel transport_model = payload_model(model);
    ...
    return SemanticManifest(make_data(physical, model, contract, std::move(semantic_bytes), std::move(envelope),
                                      graph, binding, interaction, package, std::move(codec_registry),
                                      codec_digest, std::move(graph_proof)));
    } catch (const std::bad_alloc&) {
        return manifest_error(CompatibilityError::IR_CONSTRAINT_FAILED, "SemanticManifest allocation exceeds its resource budget");
    }
...
const PhysicalCodecRegistry& SemanticManifest::physical_codec_registry() const noexcept {
    return data_->codec_registry;
}
const Sha256Digest& SemanticManifest::physical_codec_registry_digest() const noexcept {
    return data_->physical_codec_registry_digest;
}
...
bool SemanticManifest::has_physical_codec_authority() const {
    if (!data_ || data_->codec_registry.codecs.empty() || data_->codec_registry.tensors.empty()) return false;
    const Sha256Digest expected = ::Laplace::physical_codec_registry_digest(
        data_->codec_registry, data_->model);
    return codec_certificates_are_authoritative(data_->codec_registry) &&
           expected != Sha256Digest{} &&
           expected == data_->physical_codec_registry_digest;
}

...
void advise_product_tensor_ranges(const ArtifactIndex& physical,
                                  const SemanticModel& model) {
    struct ArtifactRanges {
        const PackageView* artifact = nullptr;
        std::vector<MappedReadRange> ranges;
    };
    std::vector<ArtifactRanges> by_artifact;
    by_artifact.reserve(physical.artifacts().size());
    for (const PackageView& artifact : physical.artifacts()) {
        by_artifact.push_back({&artifact, {}});
    }
    ...
    for (const SemanticTensor& tensor : model.tensors) {
        ...
        for (const TensorPlane& plane : tensor.planes) {
            const auto artifact = std::find_if(
                by_artifact.begin(), by_artifact.end(), [&](const ArtifactRanges& entry) {
                    return entry.artifact->artifact_id() == plane.artifact_id;
                });
            if (artifact == by_artifact.end() ||
                plane.offset > artifact->artifact->bytes().size() ||
                plane.length > artifact->artifact->bytes().size() -
                                   static_cast<size_t>(plane.offset)) {
                continue;
            }
            artifact->ranges.push_back({static_cast<size_t>(plane.offset),
                                        static_cast<size_t>(plane.length)});
        }
    }

    for (ArtifactRanges& entry : by_artifact) {
        if (entry.ranges.empty()) continue;
        std::sort(entry.ranges.begin(), entry.ranges.end(),
                  [](const MappedReadRange& left, const MappedReadRange& right) {
                      if (left.offset != right.offset) return left.offset < right.offset;
                      return left.length < right.length;
                  });
        std::vector<MappedReadRange> merged;
        merged.reserve(entry.ranges.size());
        for (const MappedReadRange& range : entry.ranges) {
            if (range.length == 0) continue;
            if (merged.empty()) {
                merged.push_back(range);
                continue;
            }
            MappedReadRange& tail = merged.back();
            const size_t tail_end = tail.offset + tail.length;
            if (range.offset <= tail_end) {
                const size_t range_end = range.offset + range.length;
                if (range_end > tail_end) tail.length = range_end - tail.offset;
            } else {
                merged.push_back(range);
            }
        }
        entry.artifact->advise_read_ranges(merged);
    }
}
...
ProductPackageLoadResult ProductPackage::finish_closed_v1(
    ...
    advise_product_tensor_ranges(compilation.manifest.physical_index(),
                                 compilation.manifest.semantic_model());
...
ProductPackageLoadResult ProductPackage::finish_product_package(ArtifactIndex physical,
                                                                PackageView carrier,
                                                                const PackageView* physical_package_carrier) {
    ...
    if (!runtime || !runtime->manifest().has_physical_codec_authority()) {
        return package_report(CompatibilityError::IR_QUANTIZATION_UNSUPPORTED,
                              "product manifest physical codec registry is incomplete");
    }
    advise_product_tensor_ranges(runtime->physical_index(), runtime->semantics());
    if (runtime->manifest().token_contract().tokenizer_algorithm !=
    ...
    if (physical_package_carrier != nullptr) {
        auto loaded_physical = load_physical_program_package_wire(
            runtime->physical_index(), runtime->manifest(),
            physical_package_carrier->bytes());
        if (const auto* report = std::get_if<CompatibilityReport>(&loaded_physical))
            return *report;
        physical_package = std::make_shared<const VerifiedPhysicalProgramPackage>(
            std::get<VerifiedPhysicalProgramPackage>(std::move(loaded_physical)));
    }
    auto authoritative = std::shared_ptr<const RuntimePackage>(new RuntimePackage(
        runtime->manifest(), Sha256Digest{}, 0, RuleQualificationState::Draft,
        PackageAuthorityKind::CarriedManifest, {}, physical_package));
    if (!authoritative->product_authoritative() ||
        authoritative->authority_kind() != PackageAuthorityKind::CarriedManifest) {
        return package_report(CompatibilityError::AUTHORITY_INVALID,
                              "carried package authority was not granted by the complete product closure");
    }
    return ProductPackage(std::move(authoritative), std::move(program),
...
ProductPackageLoadResult load_product_package(std::string_view package_path) {
    ...
    try {
        ...
        const std::string physical_package_path = primary_path + ".lappkg";
        struct stat physical_status {};
        const bool has_physical_package =
            lstat(physical_package_path.c_str(), &physical_status) == 0;
        const std::array<ArtifactSource, 4> sources = {
            ...
            ArtifactSource{physical_package_path, ArtifactRole::Sidecar, ArtifactId{3}},
        };
        if (!has_physical_package) {
            const std::array<ArtifactSource, 3> required_sources = {
                ArtifactSource{primary_path, ArtifactRole::Primary, ArtifactId{0}},
                ArtifactSource{carrier_path, ArtifactRole::Sidecar, ArtifactId{1}},
                ArtifactSource{token_path, ArtifactRole::Shard, ArtifactId{2}},
            };
            auto graph = ArtifactSet::load_graph(required_sources);
            ...
            if (!std::holds_alternative<PackageView>(primary) ||
                !std::holds_alternative<PackageView>(carrier) ||
                !std::holds_alternative<PackageView>(token_program))
                return package_report(CompatibilityError::PACKAGE_GRAPH_UNSUPPORTED,
                                      "product package members could not be opened");
            ...
            auto augmented = add_token_program_artifact(
                std::get<ArtifactIndex>(std::move(physical)), std::get<PackageView>(token_program));
            if (const auto* report = std::get_if<CompatibilityReport>(&augmented)) return *report;
            return ProductPackage::finish_product_package(
                std::get<ArtifactIndex>(std::move(augmented)), std::get<PackageView>(carrier));
        }
        ...
        auto physical_package = artifacts.view(ArtifactId{3});
        if (const auto* report = std::get_if<CompatibilityReport>(&physical_package)) return *report;
        auto physical = build_gguf_artifact_index(std::get<PackageView>(primary));
        if (const auto* report = std::get_if<CompatibilityReport>(&physical)) return *report;
        auto augmented = add_token_program_artifact(
        ...
        return ProductPackage::finish_product_package(
            ...
            &std::get<PackageView>(physical_package));

...
RuleEvaluationResult resolve_gguf_semantics(const PackageEvidence& package) {
    ...
    bool grouped_attention_norms = false;
    for (const PackageTensorEvidence& tensor : package.tensors) {
        ...
        grouped_attention_norms = grouped_attention_norms ||
            member == "attn_q_norm.weight" || member == "attn_k_norm.weight";
    }
    ...
    SemanticModel model;
    model.schema_major = grouped_attention_norms ? 8 : nextn_layers != 0 ? 6 : multi_rope_sections ? 5 : 4;
    ...
    const RmsNormPayload grouped_attention_rms{
        rms_epsilon, -1, 1, RmsNormAffineGeometry::SharedAcrossGroups,
        head_dimension};
    ...
    for (uint32_t layer = 0; layer != trunk_block_count; ++layer) {
        ...
        if (attention[layer]) {
            ...
            CausalAttentionPayload attention_payload{
                static_cast<uint32_t>(*attention_heads), layer_kv_heads,
                head_dimension, f32_bits(attention_scale), AttentionMask::Causal,
                CachePolicy::Global};
            if (model.schema_major >= 7) attention_payload.value_source_value = v;
    ...
    if (nextn_layers != 0) {
        ...
        add_operator(model, OperatorKind::RmsNorm, {q}, {normalized_q}, {query_norm}, {}, grouped_attention_rms);
        add_operator(model, OperatorKind::RmsNorm, {k}, {normalized_k}, {key_norm}, {}, grouped_attention_rms);

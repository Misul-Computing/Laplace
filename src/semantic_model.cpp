...
bool valid_tensor(const SemanticTensor& tensor, bool supports_expert_axis,
    ...
    if (!enum_between(tensor.role, 1, 36) || !valid_scalar_type(tensor.logical_type) ||
...
bool valid_payload(const SemanticOperator& op) {
    ...
    if (op.semantic_version >= 2 && op.semantic_version <= 8) {
        switch (op.kind) {
        ...
        case OperatorKind::RmsNorm: {
            ...
            if (op.semantic_version < 8) {
                if (payload->affine_geometry != RmsNormAffineGeometry::FullWidth ||
                    payload->reduction_extent != 0) return false;
            } else if ((payload->affine_geometry == RmsNormAffineGeometry::FullWidth &&
                        payload->reduction_extent != 0) ||
                       (payload->affine_geometry == RmsNormAffineGeometry::SharedAcrossGroups &&
                        (payload->weight_mode != 1 || payload->reduction_extent == 0)) ||
            ...
            return op.semantic_version >= 7 && payload->weight_mode == 0 &&
                   payload->affine_geometry == RmsNormAffineGeometry::FullWidth &&
                   payload->reduction_extent == 0 && op.tensors.empty();
        }
...
bool valid_state(const SemanticState& state) {
    ...
    if (state.semantic_version >= 2 && state.semantic_version <= 8) {
...
bool valid_model(const SemanticModel& model) {
    ...
    const bool v8 = model.schema_major == 8 && model.schema_minor == 0 &&
                    model.opset_major == 8 && model.opset_minor == 0;
    if ((!v1 && !v2 && !v3 && !v4 && !v5 && !v6 && !v7 && !v8) || (v1 && model.maximum_context != 32768) ||
        ((v2 || v3 || v4 || v5 || v6 || v7 || v8) && (model.maximum_context == 0 || model.maximum_context > 262144)) ||
    ...
    for (const SemanticTensor& tensor : model.tensors)
        if (!valid_tensor(tensor, v7 || v8, v6 || v7 || v8)) return false;
    ...
    for (const SemanticOperator& op : model.operators) {
        if ((v1 && op.semantic_version != 1) || (v2 && op.semantic_version != 2) ||
            ...
            (v7 && op.semantic_version != 7) || (v8 && op.semantic_version != 8) || !valid_payload(op)) return false;
        ...
        if ((v2 || v3 || v4 || v5 || v6 || v7 || v8) && !valid_recurrent_bindings(model, op)) return false;
    }
    ...
    for (size_t index = 0; index != model.layers.size(); ++index) {
        ...
        if (layer.layer_index != index || layer.first_operator < operator_cursor ||
            (layer.flags & ~((v6 || v7 || v8) ? kSemanticLayerFlagSpeculative : 0u)) != 0 ||
    ...
    for (const SemanticState& state : model.states) {
        ...
        if (state.kind == StateKind::RecurrentDeltaMatrix) {
            const LayoutPolicy expected = (v3 || v4 || v5 || v6 || v7 || v8) ? LayoutPolicy::ValueHeadKeyRowOutputColumn
    ...
    for (const SemanticOperator& op : model.operators) {
        ...
        const auto& payload = std::get<GatedDeltaNetPayload>(op.payload);
        const DeltaStateLayout expected = (v3 || v4 || v5 || v6 || v7 || v8) ? DeltaStateLayout::ValueHeadKeyRowOutputColumn
                                             : DeltaStateLayout::ValueHeadOutputRowKeyColumn;
        if (payload.state_layout != expected) return false;
    }
    if (v7 || v8) for (const SemanticOperator& op : model.operators) if (!valid_v7_bindings(model, op)) return false;
...
void append_payload(std::vector<uint8_t>& output, const SemanticOperator& op) {
    ...
    case OperatorKind::RmsNorm: {
        ...
        append_u8(output, payload.weight_mode);
        if (op.semantic_version >= 8) {
            append_u8(output, static_cast<uint8_t>(payload.affine_geometry));
            append_u16(output, 0);
            append_u32(output, payload.reduction_extent);
        } else {
            append_zeros(output, 7);
        }
        break;
...
bool read_payload(const std::vector<uint8_t>& bytes, size_t& offset, OperatorKind kind,
    ...
    case OperatorKind::RmsNorm: {
        ...
        if (!read_u32(bytes, offset, value.epsilon_f32_bits) || !read_u32(bytes, offset, axis) ||
            !read_u8(bytes, offset, value.weight_mode)) return false;
        value.axis = static_cast<int32_t>(axis);
        if (semantic_version >= 8) {
            uint8_t geometry = 0;
            uint16_t reserved = 0;
            if (!read_u8(bytes, offset, geometry) || !read_u16(bytes, offset, reserved) ||
                reserved != 0 || !read_u32(bytes, offset, value.reduction_extent)) return false;
            value.affine_geometry = static_cast<RmsNormAffineGeometry>(geometry);
        } else {
            if (bytes.size() - offset < 7) return false;
            for (unsigned index = 0; index != 7; ++index) if (bytes[offset++] != 0) return false;
        }
        payload = value; return true;
    }
                    ...
                    : model.schema_major == 5
                        ...
                        : model.schema_major == 6
                            ...
                            : model.schema_major == 7
                                ...
                                : std::initializer_list<uint8_t>{'L', 'A', 'P', 'I', 'R', '0', '0', '8'});
...
SemanticDecodeResult decode_semantic_model(const std::vector<uint8_t>& bytes) {
    ...
    const bool v8_magic = bytes.size() >= 64 && std::memcmp(bytes.data(), "LAPIR008", 8) == 0;
    if (!v1_magic && !v2_magic && !v3_magic && !v4_magic && !v5_magic && !v6_magic && !v7_magic && !v8_magic) {
    ...
    if (!read_u16(bytes, offset, schema_major) || !read_u16(bytes, offset, schema_minor) ||
        ...
        schema_major != (v1_magic ? 1 : v2_magic ? 2 : v3_magic ? 3 : v4_magic ? 4 : v5_magic ? 5 : v6_magic ? 6 : v7_magic ? 7 : 8) ||
        schema_minor != 0 ||
        opset_major != (v1_magic ? 1 : v2_magic ? 2 : v3_magic ? 3 : v4_magic ? 4 : v5_magic ? 5 : v6_magic ? 6 : v7_magic ? 7 : 8) ||
        opset_minor != 0 ||

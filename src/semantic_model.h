...
enum class TensorRole : uint16_t {
    ...
    RouterScaleWeight = 32, ExpertNormWeight = 33, ReduceScaleWeight = 34,
    PostNormWeight = 35, OutputScaleWeight = 36,
};
...
enum class RmsNormAffineGeometry : uint8_t {
    FullWidth = 0,
    SharedAcrossGroups = 1,
};
...
struct RmsNormPayload {
    ...
    RmsNormAffineGeometry affine_geometry = RmsNormAffineGeometry::FullWidth;
    uint32_t reduction_extent = 0;
...
inline bool semantic_operator_signature_valid(const SemanticOperator& op) {
    ...
    } else if (op.semantic_version < 2 || op.semantic_version > 8) {
        return false;
    }
    switch (op.kind) {

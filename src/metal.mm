...
#else
#define LAPLACE_HAS_MTL_TENSOR_API 0
#endif
#include <cstdio>
#include <atomic>
#include <cstring>
...
static std::unordered_map<uint32_t, id<MTLComputePipelineState>>
    g_affine_u2_256_variant_pipes;
#if defined(LAPLACE_METAL_TESTING) || defined(LAPLACE_TESTING)
static std::unordered_map<uint32_t, id<MTLComputePipelineState>>
    g_quantized_variant_pipes;
...
static std::once_flag g_trellis_init;
#endif
static std::mutex g_dispatch_metrics_mutex;
static MetalDispatchMetrics g_dispatch_metrics;
static std::atomic<uint64_t> g_global_pipeline_lookup_attempts{0};
static std::atomic<uint64_t> g_global_pipeline_compilation_attempts{0};
static thread_local bool g_lease_pipeline_path = false;

struct MetalTokContext;
static id<MTLComputePipelineState> leased_pipeline(
    const char* function, std::span<const MetalFunctionConstant> constants);
static std::array<uint8_t, 32> metal_invocation_authority_digest(
    const MetalTokInvocationAuthority&) noexcept;
static bool leased_program_complete(const MetalTokContext&) noexcept;

// A lease-backed token scope is deliberately barred from the legacy global
// pipeline caches. Count an attempted crossing so tests catch a regression,
// then make the caller fail closed before it can touch g_lib/g_named/get_*.
static bool global_pipeline_access_blocked() {
    if (!g_lease_pipeline_path) return false;
    g_global_pipeline_lookup_attempts.fetch_add(1, std::memory_order_relaxed);
    return true;
}

static MetalTokContext& active_tok_context();
static id<MTLDevice> tok_device();
...
void metal_pipeline_global_lookup_metrics_reset_for_testing() {
    g_global_pipeline_lookup_attempts.store(0, std::memory_order_relaxed);
    g_global_pipeline_compilation_attempts.store(0, std::memory_order_relaxed);
}

MetalPipelineGlobalLookupMetrics metal_pipeline_global_lookup_metrics_for_testing() {
    MetalPipelineGlobalLookupMetrics metrics;
    metrics.lookup_attempts =
        g_global_pipeline_lookup_attempts.load(std::memory_order_relaxed);
    metrics.compilation_attempts =
        g_global_pipeline_compilation_attempts.load(std::memory_order_relaxed);
    return metrics;
}
#endif
...
#if defined(LAPLACE_TESTING)
static const char* src_tensorops_probe_uint2 = R"METAL(
...
using tensorops_probe_scales = tensor_blockwise<tensor_plane_scales,
                                                 device metal_fp8_ue8m0_format,
                                                 32, 1>;
using tensorops_probe_weights = tensor<device uint2b_format,
                                       dextents<int, 2>, tensor_inline,
                                       tensorops_probe_scales>;

kernel void laplace_tensorops_probe_uint2(
    device half* input [[buffer(0)]], device uchar* weights [[buffer(1)]],
    device uchar* scales [[buffer(2)]], device float* output [[buffer(3)]],
    uint tgid [[threadgroup_position_in_grid]])
{
    constexpr int K = 512;
    constexpr int M = 8;
    constexpr int N = 16;
    auto a = tensor<device half, dextents<int, 2>, tensor_inline>(
        input, dextents<int, 2>(K, M), array<int, 2>({1, K}));
    auto b = tensorops_probe_weights(
        weights, dextents<int, 2>(K, N), array<int, 2>({1, K}),
        tensorops_probe_scales(scales));
    auto c = tensor<device float, dextents<int, 2>, tensor_inline>(
        output, dextents<int, 2>(N, M), array<int, 2>({1, N}));
    constexpr auto desc = matmul2d_descriptor(
        M, N, K, false, true, false, matmul2d_descriptor::mode::multiply);
    matmul2d<desc, execution_simdgroups<2>> op;
    if (tgid == 0) op.run(a, b, c);
}
)METAL";

static const char* src_tensorops_probe_int2 = R"METAL(
#include <metal_stdlib>
#include <metal_tensor>
#include <MetalPerformancePrimitives/MPPTensorOpsMatMul2d.h>
using namespace metal;
...
using tensorops_probe_scales = tensor_blockwise<tensor_plane_scales,
                                                 device metal_fp8_ue8m0_format,
                                                 32, 1>;
using tensorops_probe_weights = tensor<device int2b_format,
                                       dextents<int, 2>, tensor_inline,
                                       tensorops_probe_scales>;

kernel void laplace_tensorops_probe_int2(
    device half* input [[buffer(0)]], device uchar* weights [[buffer(1)]],
    device uchar* scales [[buffer(2)]], device float* output [[buffer(3)]],
    uint tgid [[threadgroup_position_in_grid]])
{
...
#if defined(LAPLACE_METAL_TESTING) || defined(LAPLACE_TESTING)
static const char* src_trellis_decode_dot = R"METAL(
#include <metal_stdlib>
using namespace metal;

static inline uint trellis_logical_word(device const ushort* packed, uint index) {
    // Host packing applies SWAP16 to every uint32. The EXL3 funnel decoder
    // consumes the stored (already swapped) uint32 view directly.
    return uint(packed[index * 2u]) |
           (uint(packed[index * 2u + 1u]) << 16u);
}

static inline ushort trellis_state(device const ushort* packed, uint pair,
                                   uint parity, uint bits) {
    const uint word_count = 8u * bits;
    const int b0 = int(pair * 2u * bits + bits - 16u + 256u * bits);
    const int b2 = b0 + int(bits) + 16;
    const uint raw_i0 = uint(b0 / 32);
    const uint raw_i1 = uint((b2 - 1) / 32);
    const uint i0 = raw_i0 % word_count;
    const uint i1 = raw_i1 % word_count;
    const uint shift = uint((int(raw_i1) + 1) * 32 - b2);
    const ulong merged = (ulong(trellis_logical_word(packed, i0)) << 32u) |
                         ulong(trellis_logical_word(packed, i1));
    const uint window = uint(merged >> shift);
    const uint first = (window >> bits) & 0xffffu;
    const uint second = window & 0xffffu;
    return ushort(parity == 0u ? first : second);
}

static inline uint trellis_kernel_slot(uint row, uint col) {
    const uint group = col < 8u ? col : col - 8u;
    const uint pair = group * 4u + ((row & 7u) >> 1u);
    const uint slot = (row >= 8u ? 2u : 0u) + (row & 1u) +
                      (col >= 8u ? 4u : 0u);
    return pair * 8u + slot;
}

static inline float trellis_decode(uint cw, uint codebook) {
    if (codebook == 0u || codebook == 1u) {
        uint x = codebook == 0u ? cw * 89226354u + 64248484u
                                : cw * 0xcbac1fedu;
        x = (x & 0x8fff8fffu) ^ 0x3b603b60u;
        half2 pair = as_type<half2>(x);
        return float(pair.x + pair.y);
    }
    uint x = cw * 0x83dcd12du;
    uint sum = 0x6400u;
    for (uint lane = 0u; lane != 4u; ++lane)
        sum += (x >> (8u * lane)) & 0xffu;
    half h = as_type<half>(ushort(sum & 0xffffu));
    half k_inv = as_type<half>(ushort(0x1eeeu));
    half k_bias = as_type<half>(ushort(0xc931u));
    return float(h * k_inv + k_bias);
}

kernel void trellis_decode_dot(
    ...
    constant uint& codebook [[buffer(6)]],
    uint tid [[thread_index_in_threadgroup]],
    uint threads [[threads_per_threadgroup]],
    uint out_tile [[threadgroup_position_in_grid]]) {
    const uint in_tiles = logical_k / 16u;
    const uint out_tiles = logical_n / 16u;
    if (out_tile >= out_tiles) return;
    threadgroup ushort states[256];
    threadgroup_barrier(mem_flags::mem_threadgroup);
    const uint packed_words = 16u * bits;
    float sum = 0.0f;
    for (uint in_tile = 0u; in_tile != in_tiles; ++in_tile) {
        device const ushort* tile = packed +
            (ulong(in_tile) * ulong(out_tiles) + ulong(out_tile)) * packed_words;
        for (uint pair = tid; pair < 128u; pair += threads) {
            states[pair * 2u] = trellis_state(tile, pair, 0u, bits);
            states[pair * 2u + 1u] = trellis_state(tile, pair, 1u, bits);
        }
        threadgroup_barrier(mem_flags::mem_threadgroup);
        if (tid < 16u) {
            const uint col = tid;
            for (uint row = 0u; row != 16u; ++row) {
                ...
                const uint slot = (row >= 8u ? 2u : 0u) + (row & 1u) +
                                  ((col >> 3u) * 4u);
                const uint state_index = pair * 8u + slot;
                sum += input[in_tile * 16u + row] *
                       trellis_decode(uint(states[state_index]), codebook);
            }
        }
        threadgroup_barrier(mem_flags::mem_threadgroup);
    }
    if (tid < 16u) output[out_tile * 16u + tid] = sum;
}

// Direct packed/register candidate: a faithful port of PonyExl3's v12 M=1
// kernel. 128 threads (four SIMD groups) own one 16-column output tile. Each
// lane keeps eight accumulators in registers, obtains x through a SIMD
// shuffle, and the only threadgroup barriers are the final cross-SIMD-group
// reduction. The descriptor's bits select the physical funnel; no model or
// artifact property is consulted here.
kernel void trellis_decode_dot_direct(
    device const ushort* packed [[buffer(0)]],
    device const float* input [[buffer(1)]],
    device float* output [[buffer(2)]],
    constant uint& logical_k [[buffer(3)]],
    ...
    const uint in_tiles = logical_k / 16u;
    const uint out_tiles = logical_n / 16u;
    if (out_tile >= out_tiles || tid >= 128u || bits < 1u || bits > 8u) return;
    const uint lane = tid & 31u;
    const uint sg = tid >> 5u;
    const uint packed_words = 8u * bits;

    // tensor_core_perm() is affine in t. Keeping it in registers avoids a
    // second per-dispatch permutation buffer while preserving the upstream
    // physical order exactly. For t=lane*4+j, j selects the 2x2 row/column
    // quadrant owned by the pair; this is the same sequence as perm[t*2].
    uint pos_[8];
    uint row_[8];
    for (uint j = 0u; j < 4u; ++j) {
        const uint r0 = (lane & 3u) * 2u;
        const uint row_offset = (j & 1u) != 0u ? 8u : 0u;
        const uint col = (lane >> 2u) + (j >= 2u ? 8u : 0u);
        const uint row = r0 + row_offset;
        pos_[j * 2u] = row * 16u + col;
        pos_[j * 2u + 1u] = (row + 1u) * 16u + col;
        row_[j * 2u] = row;
        row_[j * 2u + 1u] = row + 1u;
    }
    ...
    for (uint g = 0u; g < 2u; ++g) {
        const uint c0 = lane * 8u + g * 4u;
        const int e_last = int(c0 + 4u) * int(bits) + 256 * int(bits);
        const int i_end = (e_last - 1) / 32;
        ig1_[g] = uint(i_end) % packed_words;
        ig0_[g] = uint(i_end - 1) % packed_words;
        s3_[g] = uint((i_end + 1) * 32 - e_last);
    }
    uint i0_[4] = {0u, 0u, 0u, 0u};
    uint i1_[4] = {0u, 0u, 0u, 0u};
    uint s1_[4] = {0u, 0u, 0u, 0u};
    if (bits == 7u) {
        // 3K+16 is 37 bits for k=7 and can straddle three words. Use the
        // upstream per-pair funnel instead of the dq4 two-word window.
        for (uint j = 0u; j < 4u; ++j) {
            const uint t = lane * 4u + j;
            const int b0 = int(t) * 2 * int(bits) + int(bits) - 16 +
                           256 * int(bits);
            const int b2 = b0 + int(bits) + 16;
            i0_[j] = uint(b0 / 32) % packed_words;
            i1_[j] = uint((b2 - 1) / 32) % packed_words;
            s1_[j] = uint(((b2 - 1) / 32 + 1) * 32 - b2);
        }
    }

    // Each SIMD group owns a disjoint stride of input tiles. The four partial
    // sums are combined once in threadgroup memory below.
    for (uint in_tile = sg; in_tile < in_tiles; in_tile += 4u) {
        device const ushort* tile = packed +
            (ulong(in_tile) * ulong(out_tiles) + ulong(out_tile)) * (16u * bits);
        const float x_lane = input[in_tile * 16u + (lane & 15u)];
        if (bits == 7u) {
            for (uint j = 0u; j < 4u; ++j) {
                const ulong merged = (ulong(trellis_logical_word(tile, i0_[j])) << 32u) |
                                     ulong(trellis_logical_word(tile, i1_[j]));
                const uint window = uint(merged >> s1_[j]);
                const uint cw0 = (window >> bits) & 0xffffu;
                const uint cw1 = window & 0xffffu;
                float dq_val = trellis_decode(cw0, codebook);
                acc[j * 2u] = fma(simd_shuffle(x_lane, ushort(row_[j * 2u])),
                                  dq_val, acc[j * 2u]);
                dq_val = trellis_decode(cw1, codebook);
                acc[j * 2u + 1u] = fma(simd_shuffle(x_lane, ushort(row_[j * 2u + 1u])),
                                      dq_val, acc[j * 2u + 1u]);
            }
        } else {
            for (uint g = 0u; g < 2u; ++g) {
                const ulong merged = (ulong(trellis_logical_word(tile, ig0_[g])) << 32u) |
                                     ulong(trellis_logical_word(tile, ig1_[g]));
                const uint s = s3_[g];
                const uint cw[4] = {
                    (uint(merged >> (s + 3u * bits)) & 0xffffu),
                    (uint(merged >> (s + 2u * bits)) & 0xffffu),
                    (uint(merged >> (s + 1u * bits)) & 0xffffu),
                    (uint(merged >> s) & 0xffffu)};
                const uint base = g * 4u;
                for (uint j = 0u; j < 4u; ++j) {
                    const float dq_val = trellis_decode(cw[j], codebook);
                    acc[base + j] = fma(
                        simd_shuffle(x_lane, ushort(row_[base + j])), dq_val,
                        acc[base + j]);
                }
            }
        }
    ...
    for (uint j = 0u; j < 8u; ++j) tg_w[sg][pos_[j]] = acc[j];
    threadgroup_barrier(mem_flags::mem_threadgroup);
    if (tid < 16u) {
        float sum = 0.0f;
        for (uint r = 0u; r < 16u; ++r) {
            const uint p = r * 16u + tid;
            for (uint g = 0u; g < 4u; ++g) sum += tg_w[g][p];
        }
        output[out_tile * 16u + tid] = sum;
    }
}
)METAL";
#endif
...
static bool column_grouped_affine_lowbit_v1_pipeline_ready() {
    if (g_lease_pipeline_path) return true;
    init();
...
static id<MTLComputePipelineState> column_grouped_affine_lowbit_v1_optimized_pipeline(
    ...
    if (g_lease_pipeline_path) {
        switch (bits) {
            case 2u: return leased_pipeline("column_grouped_affine_lowbit_v1_q2", {});
            case 3u: return leased_pipeline("column_grouped_affine_lowbit_v1_q3", {});
            case 4u: return leased_pipeline("column_grouped_affine_lowbit_v1_q4", {});
            default: return nil;
        }
    }
...
static void init() {
    if (g_lease_pipeline_path) {
        g_global_pipeline_lookup_attempts.fetch_add(1, std::memory_order_relaxed);
        g_global_pipeline_compilation_attempts.fetch_add(1, std::memory_order_relaxed);
        return;
    }
...
static id<MTLComputePipelineState> get_sampler_pipe() {
    if (g_lease_pipeline_path)
        return leased_pipeline("sampler_greedy_f32", {});
    if (global_pipeline_access_blocked()) return nil;
...
static id<MTLComputePipelineState> get_prefill_f16_pipe() {
    if (g_lease_pipeline_path)
        return leased_pipeline("prefill_f16_rows", {});
    if (global_pipeline_access_blocked()) return nil;
...
static id<MTLComputePipelineState> get_pipe(int type) {
    if (g_lease_pipeline_path) {
        const std::array<MetalFunctionConstant, 1> constants = {
            MetalFunctionConstant{0, MetalFunctionConstantType::Int32,
                                  static_cast<uint64_t>(static_cast<int64_t>(type))}};
        return leased_pipeline("gemv", constants);
    }
    if (global_pipeline_access_blocked()) return nil;
    {
...
static id<MTLComputePipelineState> get_sparse_pipe(
    ...
    if (g_lease_pipeline_path) {
        const std::array<MetalFunctionConstant, 2> constants = {
            MetalFunctionConstant{0, MetalFunctionConstantType::Int32,
                                  static_cast<uint64_t>(static_cast<int64_t>(type))},
            MetalFunctionConstant{1, MetalFunctionConstantType::Bool,
                                  affine ? 1u : 0u}};
        return leased_pipeline(name, constants);
    }
    if (global_pipeline_access_blocked()) return nil;
...
static id<MTLComputePipelineState> get_q4k_pipe() {
    if (g_lease_pipeline_path) return leased_pipeline("gemv_q4k", {});
    if (global_pipeline_access_blocked()) return nil;
...
static id<MTLComputePipelineState> get_q2k_pipe() {
    if (g_lease_pipeline_path) return leased_pipeline("gemv_q2k", {});
    if (global_pipeline_access_blocked()) return nil;
...
static id<MTLComputePipelineState> get_iq2_xxs_pipe() {
    if (g_lease_pipeline_path) return leased_pipeline("gemv_iq2_xxs", {});
    if (global_pipeline_access_blocked()) return nil;
...
static id<MTLComputePipelineState> get_iq1_s_pipe() {
    if (g_lease_pipeline_path) return leased_pipeline("gemv_iq1_s", {});
    if (global_pipeline_access_blocked()) return nil;
...
static id<MTLComputePipelineState> get_q2k_two_row_pipe() {
    if (g_lease_pipeline_path) return leased_pipeline("gemv_q2k_two_row", {});
    if (global_pipeline_access_blocked()) return nil;
...
static id<MTLComputePipelineState> get_q2k_streamed_pipe() {
    if (g_lease_pipeline_path) return leased_pipeline("gemv_q2k_streamed", {});
    if (global_pipeline_access_blocked()) return nil;
...
static id<MTLComputePipelineState> get_q4k_id_pipe() {
    if (g_lease_pipeline_path) return leased_pipeline("gemv_q4k_id", {});
    if (global_pipeline_access_blocked()) return nil;
...
static id<MTLComputePipelineState> get_q8_id_pipe() {
    if (g_lease_pipeline_path) return leased_pipeline("gemv_q8_id", {});
    if (global_pipeline_access_blocked()) return nil;
...
static id<MTLComputePipelineState> get_id_pipe(int type) {
    if (g_lease_pipeline_path) {
        const std::array<MetalFunctionConstant, 1> constants = {
            MetalFunctionConstant{0, MetalFunctionConstantType::Int32,
                                  static_cast<uint64_t>(static_cast<int64_t>(type))}};
        return leased_pipeline("gemv_id", constants);
    }
    if (global_pipeline_access_blocked()) return nil;
...
static id<MTLComputePipelineState> get_moe_down_reduce_pipe(int type) {
    if (g_lease_pipeline_path) {
        const std::array<MetalFunctionConstant, 1> constants = {
            MetalFunctionConstant{0, MetalFunctionConstantType::Int32,
                                  static_cast<uint64_t>(static_cast<int64_t>(type))}};
        return leased_pipeline("moe_down_reduce", constants);
    }
    if (global_pipeline_access_blocked()) return nil;
...
static id<MTLComputePipelineState> get_q8_pipe() {
    if (g_lease_pipeline_path) return leased_pipeline("gemv_q8", {});
    if (global_pipeline_access_blocked()) return nil;
...
static id<MTLComputePipelineState> get_gemm_pipe(int type) {
    if (g_lease_pipeline_path) {
        const std::array<MetalFunctionConstant, 1> constants = {
            MetalFunctionConstant{0, MetalFunctionConstantType::Int32,
                                  static_cast<uint64_t>(static_cast<int64_t>(type))}};
        return leased_pipeline("gemm_simdgroup", constants);
    }
    if (global_pipeline_access_blocked()) return nil;
    {
        std::lock_guard<std::mutex> lk(g_pipe_mtx);
...
#if defined(LAPLACE_TESTING)
static bool tensorops_probe_descriptor_valid(const MetalTensorOpsProbeDescriptor& descriptor) {
    if (descriptor.version != 1 || descriptor.rank != 2 ||
        (descriptor.storage != MetalTensorOpsProbeStorage::UInt2 &&
         descriptor.storage != MetalTensorOpsProbeStorage::Int2) ||
        descriptor.dimensions[0] == 0 || descriptor.dimensions[1] == 0 ||
        descriptor.dimensions[0] % 32 != 0 || descriptor.strides[0] != 1 ||
        descriptor.strides[1] % 512 != 0 ||
        descriptor.block_factors != std::array<uint64_t, 2>{32, 1} ||
        descriptor.data_plane.byte_offset % 128 != 0 ||
        descriptor.scale_plane.byte_offset % 128 != 0 ||
        !descriptor.attachments.data || !descriptor.attachments.scales)
        return false;
    if (descriptor.dimensions[0] > UINT64_MAX / descriptor.dimensions[1]) return false;
    const uint64_t elements = descriptor.dimensions[0] * descriptor.dimensions[1];
    const uint64_t data_bytes = elements / 4;
    const uint64_t scale_rows = descriptor.dimensions[0] / descriptor.block_factors[0];
    if (scale_rows > UINT64_MAX / descriptor.dimensions[1]) return false;
    const uint64_t scale_bytes = scale_rows * descriptor.dimensions[1];
    const auto contains = [](uint64_t attachment_bytes, const MetalTensorOpsProbePlane& plane,
                             uint64_t required_bytes) {
        return plane.byte_length >= required_bytes && plane.byte_offset <= attachment_bytes &&
               plane.byte_length <= attachment_bytes - plane.byte_offset;
    };
    return contains(descriptor.attachments.data_bytes, descriptor.data_plane, data_bytes) &&
           contains(descriptor.attachments.scale_bytes, descriptor.scale_plane, scale_bytes);
}

MetalTensorOpsProbeResult metal_test_tensorops_descriptor_probe(
    const MetalTensorOpsProbeDescriptor& descriptor) {
    MetalTensorOpsProbeResult result;
    if (!tensorops_probe_descriptor_valid(descriptor)) return result;

#if !LAPLACE_HAS_MTL_TENSOR_API
    result.status = MetalTensorOpsProbeStatus::SkippedSdkUnavailable;
    return result;
#else
    constexpr uint64_t probe_k = 512;
    constexpr uint64_t probe_n = 16;
    constexpr uint64_t probe_m = 8;
    if (descriptor.dimensions != std::array<uint64_t, 2>{probe_k, probe_n} ||
        descriptor.strides != std::array<uint64_t, 2>{1, probe_k}) {
        result.status = MetalTensorOpsProbeStatus::SkippedSdkUnavailable;
        return result;
    }
    if (!@available(macOS 27.0, *)) {
    ...
    init();
    if (!g_dev || !g_q || ![g_dev supportsFamily:MTLGPUFamilyApple10]) {
        result.status = MetalTensorOpsProbeStatus::SkippedSdkUnavailable;
        return result;
    }
    const char* headers = mpp_header_dir();
    if (read_text(std::string(headers) + "/MPPTensorOpsMatMul2d.h").empty()) {
        result.status = MetalTensorOpsProbeStatus::SkippedSdkUnavailable;
        return result;
    }
    ...
    @autoreleasepool {
        NSInteger dimensions[] = {
            static_cast<NSInteger>(descriptor.dimensions[0]),
            static_cast<NSInteger>(descriptor.dimensions[1]),
        };
        NSInteger strides[] = {
            static_cast<NSInteger>(descriptor.strides[0]),
            static_cast<NSInteger>(descriptor.strides[1]),
        };
        NSInteger block_factors[] = {
            static_cast<NSInteger>(descriptor.block_factors[0]),
            static_cast<NSInteger>(descriptor.block_factors[1]),
        };
        MTLTensorExtents* tensor_dimensions =
            [[[MTLTensorExtents alloc] initWithRank:2 values:dimensions] autorelease];
        MTLTensorExtents* tensor_strides =
            [[[MTLTensorExtents alloc] initWithRank:2 values:strides] autorelease];
        MTLTensorExtents* tensor_block_factors =
            [[[MTLTensorExtents alloc] initWithRank:2 values:block_factors] autorelease];
        if (!tensor_dimensions || !tensor_strides || !tensor_block_factors) {
            result.status = MetalTensorOpsProbeStatus::Failed;
            return result;
        }
        MTLTensorDescriptor* tensor_descriptor = [[MTLTensorDescriptor new] autorelease];
        tensor_descriptor.dataType = descriptor.storage == MetalTensorOpsProbeStorage::UInt2
            ? MTLTensorDataTypeUInt2 : MTLTensorDataTypeInt2;
        tensor_descriptor.dimensions = tensor_dimensions;
        tensor_descriptor.strides = tensor_strides;
        tensor_descriptor.usage = MTLTensorUsageCompute;

        MTLTensorAuxiliaryPlaneDescriptor* scale_descriptor =
            [[MTLTensorAuxiliaryPlaneDescriptor new] autorelease];
        scale_descriptor.dataType = MTLTensorDataTypeMetalFloat8UE8M0;
        scale_descriptor.blockFactors = tensor_block_factors;
        MTLTensorAuxiliaryPlaneDescriptorMap* auxiliary_planes =
            [[MTLTensorAuxiliaryPlaneDescriptorMap new] autorelease];
        [auxiliary_planes setDescriptor:scale_descriptor forPlane:MTLTensorPlaneTypeScales];
        tensor_descriptor.auxiliaryPlanes = auxiliary_planes;

        id<MTLBuffer> data = [[g_dev newBufferWithBytesNoCopy:
            const_cast<void*>(descriptor.attachments.data)
            length:descriptor.attachments.data_bytes
            options:MTLResourceStorageModeShared deallocator:nil] autorelease];
        id<MTLBuffer> scales = [[g_dev newBufferWithBytesNoCopy:
            const_cast<void*>(descriptor.attachments.scales)
            length:descriptor.attachments.scale_bytes
            options:MTLResourceStorageModeShared deallocator:nil] autorelease];
        id<MTLBuffer> input = [[g_dev newBufferWithLength:probe_k * probe_m * sizeof(uint16_t)
                                                   options:MTLResourceStorageModeShared] autorelease];
        id<MTLBuffer> output = [[g_dev newBufferWithLength:probe_n * probe_m * sizeof(float)
                                                    options:MTLResourceStorageModeShared] autorelease];
        if (!data || !scales || !input || !output) {
            result.status = MetalTensorOpsProbeStatus::Failed;
            return result;
        }
        std::memset(input.contents, 0, probe_k * probe_m * sizeof(uint16_t));
        MTLTensorBufferAttachments* attachments = [[MTLTensorBufferAttachments new] autorelease];
        [attachments setBuffer:data offset:descriptor.data_plane.byte_offset
                       forPlane:MTLTensorPlaneTypeData];
        [attachments setBuffer:scales offset:descriptor.scale_plane.byte_offset
                       forPlane:MTLTensorPlaneTypeScales];
        NSError* error = nil;
        id<MTLTensor> tensor = [[g_dev newTensorWithDescriptor:tensor_descriptor
                                                    attachments:attachments error:&error] autorelease];
        if (!tensor) {
            result.status = MetalTensorOpsProbeStatus::Failed;
            return result;
        }
        const MTLTensorDataType expected_data_type =
            descriptor.storage == MetalTensorOpsProbeStorage::UInt2
                ? MTLTensorDataTypeUInt2
                : MTLTensorDataTypeInt2;
        if (tensor.dataType != expected_data_type || tensor.buffer != data ||
            tensor.bufferOffset != descriptor.data_plane.byte_offset ||
            tensor.auxiliaryPlanes.count != 1) {
            result.status = MetalTensorOpsProbeStatus::Failed;
            return result;
        }
        id<MTLTensorAuxiliaryPlane> scale_plane = [tensor.auxiliaryPlanes objectAtIndex:0];
        if (scale_plane.planeType != MTLTensorPlaneTypeScales ||
            scale_plane.dataType != MTLTensorDataTypeMetalFloat8UE8M0 ||
            scale_plane.buffer != scales ||
            scale_plane.bufferOffset != descriptor.scale_plane.byte_offset) {
            result.status = MetalTensorOpsProbeStatus::Failed;
            return result;
        }
        result.attached_planes = 2;

        const char* source = descriptor.storage == MetalTensorOpsProbeStorage::UInt2
            ? src_tensorops_probe_uint2 : src_tensorops_probe_int2;
        const char* entry = descriptor.storage == MetalTensorOpsProbeStorage::UInt2
            ? "laplace_tensorops_probe_uint2" : "laplace_tensorops_probe_int2";
        std::string flat;
        std::unordered_set<std::string> seen;
        flatten_metal(source, headers, headers, flat, seen);
        MTLCompileOptions* options = [[MTLCompileOptions new] autorelease];
        options.languageVersion = MTLLanguageVersion4_1;
        options.maxTotalThreadsPerThreadgroup = 64;
        id<MTLLibrary> library = [[g_dev newLibraryWithSource:
            [NSString stringWithUTF8String:flat.c_str()] options:options error:&error] autorelease];
        id<MTLFunction> function = library ? [[library newFunctionWithName:
            [NSString stringWithUTF8String:entry]] autorelease] : nil;
        id<MTLComputePipelineState> pipeline = function ? [[g_dev
            newComputePipelineStateWithFunction:function error:&error] autorelease] : nil;
        if (!pipeline) {
            result.status = MetalTensorOpsProbeStatus::Failed;
            return result;
        }
        ...
        [encoder setBuffer:data offset:descriptor.data_plane.byte_offset atIndex:1];
        [encoder setBuffer:scales offset:descriptor.scale_plane.byte_offset atIndex:2];
        ...
        result.status = MetalTensorOpsProbeStatus::Ran;
...
static bool metal_register_mmap_impl(const void* base, size_t size,
    ...
    if (!device || !base || size == 0) return false;
    ...
    size_t max_buf = static_cast<size_t>(device.maxBufferLength);
...
static id<MTLBuffer> get_weight_buf(const void* ptr, size_t len, size_t& offset) {
    offset = 0;
    {
        ...
        id<MTLBuffer> buf = [tok_device() newBufferWithBytes:ptr length:len
                                             options:MTLResourceStorageModeShared];
...
static NativePack* get_or_pack(const Tensor& w, int K, int N) {
    ...
    id<MTLDevice> device = tok_device();
    id<MTLBuffer> qb = [device newBufferWithLength:qbytes options:MTLResourceStorageModeShared];
    id<MTLBuffer> sb = [device newBufferWithLength:sbytes options:MTLResourceStorageModeShared];
    id<MTLBuffer> mb = [device newBufferWithLength:sbytes options:MTLResourceStorageModeShared];
...
static bool bind_rows(id<MTLComputeCommandEncoder> enc, const Tensor& w,
    ...
    if (g_lease_pipeline_path) {
        pipeline = leased_pipeline("gemv_m4_rows", {});
    } else {
...
static id<MTLComputePipelineState> get_affine_u2_256_variant_pipe(
    uint32_t rows_per_simd, uint32_t simdgroups, bool masked_qdot,
    bool packed_word_load, uint32_t values_per_lane) {
    if (g_lease_pipeline_path) {
        const std::array<MetalFunctionConstant, 5> constants = {
            MetalFunctionConstant{2, MetalFunctionConstantType::UInt32, rows_per_simd},
            MetalFunctionConstant{3, MetalFunctionConstantType::UInt32, simdgroups},
            MetalFunctionConstant{4, MetalFunctionConstantType::Bool, masked_qdot ? 1u : 0u},
            MetalFunctionConstant{5, MetalFunctionConstantType::Bool, packed_word_load ? 1u : 0u},
            MetalFunctionConstant{6, MetalFunctionConstantType::UInt32, values_per_lane}};
        return leased_pipeline("gemv_affine_u2_256", constants);
    }
    if (global_pipeline_access_blocked()) return nil;
    if ((rows_per_simd != 1 && rows_per_simd != 2 && rows_per_simd != 4 &&
         rows_per_simd != 8) ||
        (simdgroups != 1 && simdgroups != 2 && simdgroups != 4) ||
        (values_per_lane != 8 && values_per_lane != 16))
        return nil;
    const uint32_t key = rows_per_simd | (simdgroups << 8) |
                         (masked_qdot ? 1u << 16 : 0u) |
                         (packed_word_load ? 1u << 17 : 0u) |
                         (values_per_lane << 20);
    {
        std::lock_guard<std::mutex> lock(g_pipe_mtx);
        const auto found = g_affine_u2_256_variant_pipes.find(key);
        if (found != g_affine_u2_256_variant_pipes.end()) return found->second;
    }
    if (!g_lib) return nil;
    MTLFunctionConstantValues* values = [MTLFunctionConstantValues new];
    [values setConstantValue:&rows_per_simd type:MTLDataTypeUInt atIndex:2];
    [values setConstantValue:&simdgroups type:MTLDataTypeUInt atIndex:3];
    [values setConstantValue:&masked_qdot type:MTLDataTypeBool atIndex:4];
    [values setConstantValue:&packed_word_load type:MTLDataTypeBool atIndex:5];
    [values setConstantValue:&values_per_lane type:MTLDataTypeUInt atIndex:6];
    NSError* error = nil;
    id<MTLFunction> function =
        [g_lib newFunctionWithName:@"gemv_affine_u2_256"
                    constantValues:values error:&error];
    [values release];
    if (!function) {
        std::fprintf(stderr,
                     "[metal] affine UInt2/256 variant rows=%u simdgroups=%u masked=%d word=%d values=%u: %s\n",
                     rows_per_simd, simdgroups, masked_qdot, packed_word_load,
                     values_per_lane,
                     error ? error.localizedDescription.UTF8String : "function unavailable");
        return nil;
    }
    id<MTLComputePipelineState> pipeline =
        [g_dev newComputePipelineStateWithFunction:function error:&error];
    ...
    if (!pipeline) {
        std::fprintf(stderr,
                     "[metal] affine UInt2/256 variant pipeline rows=%u simdgroups=%u masked=%d word=%d values=%u: %s\n",
                     rows_per_simd, simdgroups, masked_qdot, packed_word_load,
                     values_per_lane,
                     error ? error.localizedDescription.UTF8String : "unknown");
        return nil;
    ...
    std::lock_guard<std::mutex> lock(g_pipe_mtx);
    const auto [inserted, fresh] =
        g_affine_u2_256_variant_pipes.emplace(key, pipeline);
    if (!fresh) {
        [pipeline release];
...
static id<MTLComputePipelineState> get_affine_u2_256_pipe() {
    return get_affine_u2_256_variant_pipe(4, 2, false, false, 16);
}

#if defined(LAPLACE_METAL_TESTING) || defined(LAPLACE_TESTING)

static id<MTLComputePipelineState> get_trellis_decode_dot_pipe() {
    if (g_lease_pipeline_path)
        return leased_pipeline("trellis_decode_dot_direct", {});
    if (global_pipeline_access_blocked()) return nil;
    std::call_once(g_trellis_init, [] {
        if (!g_dev) return;
        NSError* error = nil;
        g_trellis_lib = [g_dev newLibraryWithSource:
            [NSString stringWithUTF8String:src_trellis_decode_dot]
            options:nil error:&error];
        if (!g_trellis_lib) {
            std::fprintf(stderr, "[metal] trellis decode-dot library: %s\n",
                         error ? error.localizedDescription.UTF8String : "unknown");
            return;
        }
        id<MTLFunction> function = [g_trellis_lib newFunctionWithName:@"trellis_decode_dot_direct"];
        if (!function) {
            std::fprintf(stderr, "[metal] trellis decode-dot function unavailable\n");
            return;
        }
        g_trellis_pipe = [g_dev newComputePipelineStateWithFunction:function error:&error];
        [function release];
        if (!g_trellis_pipe)
            std::fprintf(stderr, "[metal] trellis decode-dot pipeline: %s\n",
                         error ? error.localizedDescription.UTF8String : "unknown");
    });
    return g_trellis_pipe;
}
...
bool metal_test_trellis_decode_dot(
    ...
    init();
    if (!trellis_validate_descriptor(descriptor) || !packed || !input || !output ||
        !sample_gpu_ms || !requested_bytes || !command_buffers || !implicit_copies ||
        samples == 0 || descriptor.bits < 1 || descriptor.bits > 8 ||
        static_cast<uint8_t>(codebook) > static_cast<uint8_t>(TrellisCodebook::Mul1) ||
        codebook != descriptor.codebook)
        return false;
    id<MTLComputePipelineState> pipeline = get_trellis_decode_dot_pipe();
    if (!g_dev || !g_q || !pipeline ||
        pipeline.threadExecutionWidth < 16 ||
        pipeline.maxTotalThreadsPerThreadgroup < 128)
        return false;
    const size_t packed_bytes = static_cast<size_t>(descriptor.packed_plane_bytes);
    const size_t input_bytes = static_cast<size_t>(descriptor.logical_k) * sizeof(float);
    const size_t output_bytes = static_cast<size_t>(descriptor.logical_n) * sizeof(float);
    if (packed_bytes == 0 || input_bytes / sizeof(float) != descriptor.logical_k ||
        output_bytes / sizeof(float) != descriptor.logical_n)
        return false;
    @autoreleasepool {
        id<MTLBuffer> packed_buffer = [g_dev newBufferWithBytes:packed
                                                           length:packed_bytes
                                                          options:MTLResourceStorageModeShared];
        ...
        id<MTLBuffer> output_buffer = [g_dev newBufferWithLength:output_bytes
                                                           options:MTLResourceStorageModeShared];
        if (!packed_buffer || !input_buffer || !output_buffer) {
            [packed_buffer release];
            [input_buffer release];
            [output_buffer release];
            return false;
        }
        const uint32_t logical_k = descriptor.logical_k;
        const uint32_t logical_n = descriptor.logical_n;
        const uint32_t bits = descriptor.bits;
        const uint32_t cb = static_cast<uint32_t>(codebook);
        const NSUInteger groups = (static_cast<NSUInteger>(logical_n) + 15u) / 16u;
        auto run = [&](double* elapsed_ms) -> bool {
            id<MTLCommandBuffer> command = [g_q commandBuffer];
            id<MTLComputeCommandEncoder> encoder = command ? [command computeCommandEncoder] : nil;
            if (!command || !encoder) {
            ...
            [encoder setBytes:&bits length:sizeof(bits) atIndex:5];
            [encoder setBytes:&cb length:sizeof(cb) atIndex:6];
            ...
            const auto wait_end = std::chrono::steady_clock::now();
            if (command.status != MTLCommandBufferStatusCompleted) {
                std::fprintf(stderr, "[metal] trellis decode-dot command: %s\n",
                             command_buffer_failure_detail(command).c_str());
                return false;
            }
            ...
            const CFTimeInterval start = command.GPUStartTime;
            const CFTimeInterval end = command.GPUEndTime;
            *elapsed_ms = end >= start ? 1000.0 * (end - start) :
        ...
        for (uint32_t warmup = 0; warmup != warmups; ++warmup) {
            double elapsed = 0.0;
            if (!run(&elapsed)) {
                [packed_buffer release]; [input_buffer release]; [output_buffer release];
                return false;
            }
        }
        for (uint32_t sample = 0; sample != samples; ++sample) {
            if (!run(&sample_gpu_ms[sample])) {
                [packed_buffer release]; [input_buffer release]; [output_buffer release];
                return false;
            }
        }
        std::memcpy(output, output_buffer.contents, output_bytes);
        *requested_bytes = packed_bytes;
        *command_buffers = warmups + samples;
        *implicit_copies = 0;
        [packed_buffer release];
        [input_buffer release];
        [output_buffer release];
...
bool metal_test_affine_u2_block256_variant(
    ...
    if (!g_dev || !g_q || !g_lib || !input || !packed_weights || !scales ||
        ...
        !maximum_threads || K <= 0 || N <= 0 ||
        (rows_per_simd != 1 && rows_per_simd != 2 && rows_per_simd != 4 &&
         rows_per_simd != 8) ||
        (simdgroups != 1 && simdgroups != 2 && simdgroups != 4) ||
        (values_per_lane != 8 && values_per_lane != 16) ||
        K % static_cast<int>(values_per_lane * 32u) != 0)
        return false;
    id<MTLComputePipelineState> pipeline =
        get_affine_u2_256_variant_pipe(
            rows_per_simd, simdgroups, masked_qdot, packed_word_load,
            values_per_lane);
    if (!pipeline || pipeline.threadExecutionWidth != 32 ||
        pipeline.maxTotalThreadsPerThreadgroup < 32 * simdgroups)
        return false;
    ...
    @autoreleasepool {
        ...
        for (uint32_t sample = 0; sample != samples; ++sample) {
            ...
            for (uint32_t dispatch = 0; dispatch != dispatches_per_command; ++dispatch) {
                ...
                const NSUInteger rows_per_threadgroup = rows_per_simd * simdgroups;
                [encoder dispatchThreadgroups:
                    MTLSizeMake((static_cast<NSUInteger>(N) + rows_per_threadgroup - 1) /
                                    rows_per_threadgroup,
                                1, 1)
                    threadsPerThreadgroup:MTLSizeMake(32 * simdgroups, 1, 1)];
            }
            [encoder endEncoding];
            const auto wait_start = std::chrono::steady_clock::now();
            [command commit];
...
bool metal_test_affine_u2_block256_paired(
    const float* input, const uint8_t* packed_weights,
    ...
    init();
    if (!g_dev || !g_q || !g_lib || !input || !packed_weights || !scales ||
        !biases || !control_output || !candidate_output || !control_gpu_ms ||
        !candidate_gpu_ms || measured_pairs == 0 || dispatches_per_command == 0 ||
        !requested_bytes || !command_buffers || K <= 0 || N <= 0 || K % 512 != 0)
        return false;
    id<MTLComputePipelineState> control =
        get_affine_u2_256_variant_pipe(4, 2, false, false, 16);
    id<MTLComputePipelineState> candidate =
        get_affine_u2_256_variant_pipe(4, 1, true, false, 16);
    if (!control || !candidate || control.threadExecutionWidth != 32 ||
        candidate.threadExecutionWidth != 32 ||
        control.maxTotalThreadsPerThreadgroup < 64 ||
        candidate.maxTotalThreadsPerThreadgroup < 32)
        return false;
    ...
    @autoreleasepool {
        ...
        id<MTLBuffer> bias_buffer = [g_dev newBufferWithLength:plane_length * copies
                                                      options:MTLResourceStorageModeShared];
        const size_t output_length = static_cast<size_t>(N) * copies * sizeof(float);
        id<MTLBuffer> control_buffer = [g_dev newBufferWithLength:output_length
                                                         options:MTLResourceStorageModeShared];
        id<MTLBuffer> candidate_buffer = [g_dev newBufferWithLength:output_length
                                                           options:MTLResourceStorageModeShared];
        auto release_buffers = [&] {
            [input_buffer release];
            [packed_buffer release];
        ...
        for (uint32_t copy = 0; copy != copies; ++copy) {
            std::memcpy(static_cast<uint8_t*>(packed_buffer.contents) + copy * packed_length,
                        packed_weights + copy * packed_length, packed_length);
            std::memcpy(static_cast<uint8_t*>(scale_buffer.contents) + copy * plane_length,
                        scales + copy * plane_values, plane_length);
            std::memcpy(static_cast<uint8_t*>(bias_buffer.contents) + copy * plane_length,
                        biases + copy * plane_values, plane_length);
        }

        auto run = [&](id<MTLComputePipelineState> pipeline, uint32_t simdgroups,
                       id<MTLBuffer> output_buffer, double& elapsed_ms) {
            id<MTLCommandBuffer> command = [g_q commandBuffer];
            id<MTLComputeCommandEncoder> encoder = command ? [command computeCommandEncoder] : nil;
            if (!command || !encoder) {
                if (encoder) [encoder endEncoding];
            ...
            for (uint32_t dispatch = 0; dispatch != dispatches_per_command; ++dispatch) {
                ...
                [encoder setBuffer:output_buffer
                             offset:static_cast<NSUInteger>(copy) * N * sizeof(float)
                            atIndex:4];
                const NSUInteger rows_per_threadgroup = 4u * simdgroups;
                [encoder dispatchThreadgroups:
                    MTLSizeMake((static_cast<NSUInteger>(N) + rows_per_threadgroup - 1) /
                                    rows_per_threadgroup,
                                1, 1)
                    threadsPerThreadgroup:MTLSizeMake(32u * simdgroups, 1, 1)];
            }
            [encoder endEncoding];
            const auto wait_start = std::chrono::steady_clock::now();
            [command commit];
            ...
            const auto wait_end = std::chrono::steady_clock::now();
            if (command.status != MTLCommandBufferStatusCompleted) {
                std::fprintf(stderr, "[metal] affine UInt2/256 paired command: %s\n",
                             command_buffer_failure_detail(command).c_str());
                return false;
            }
            record_completed_command(command, wait_start, wait_end);
            ...
            const CFTimeInterval end = command.GPUEndTime;
            if (!(end >= start)) return false;
            elapsed_ms = 1000.0 * (end - start) / dispatches_per_command;
            return std::isfinite(elapsed_ms) && elapsed_ms > 0.0;
        };

        const MetalDispatchMetrics before = metal_dispatch_metrics();
        const uint32_t total_pairs = warmup_pairs + measured_pairs;
        for (uint32_t pair = 0; pair != total_pairs; ++pair) {
            double control_ms = 0.0;
            double candidate_ms = 0.0;
            const bool candidate_first = (pair & 1u) != 0;
            const bool first_ok = candidate_first
                ? run(candidate, 1, candidate_buffer, candidate_ms)
                : run(control, 2, control_buffer, control_ms);
            const bool second_ok = candidate_first
                ? run(control, 2, control_buffer, control_ms)
                : run(candidate, 1, candidate_buffer, candidate_ms);
            ...
            if (pair >= warmup_pairs) {
                control_gpu_ms[pair - warmup_pairs] = control_ms;
                candidate_gpu_ms[pair - warmup_pairs] = candidate_ms;
            }
        }
        std::memcpy(control_output, control_buffer.contents, output_length);
        std::memcpy(candidate_output, candidate_buffer.contents, output_length);
        *requested_bytes = packed_length + 2 * plane_length;
        const MetalDispatchMetrics after = metal_dispatch_metrics();
        if (after.command_buffers < before.command_buffers ||
            after.command_buffers - before.command_buffers > UINT32_MAX) {
            release_buffers();
            return false;
        }
        *command_buffers = static_cast<uint32_t>(
...
bool metal_test_affine_u2_block256(const float* input,
                                   ...
                                   uint32_t* width, uint32_t* maximum_threads) {
    return metal_test_affine_u2_block256_variant(
        input, packed_weights, scales, biases, output, K, N, 4, 2, false, false, 16,
        gpu_ms, samples, dispatches_per_command, distinct_weights,
...
static bool grow_shared(id<MTLBuffer>& buf, size_t need) {
    ...
    id<MTLBuffer> next = [tok_device() newBufferWithLength:need
                                           options:MTLResourceStorageModeShared];
...
static id<MTLComputePipelineState> get_glu_pipe() {
    if (g_lease_pipeline_path) return leased_pipeline("act_glu", {});
    if (global_pipeline_access_blocked()) return nil;
...
static id<MTLComputePipelineState> get_glu_experts_pipe() {
    if (g_lease_pipeline_path) return leased_pipeline("act_glu_experts", {});
    if (global_pipeline_access_blocked()) return nil;
...
static id<MTLComputePipelineState> get_q6k_pipe() {
    if (g_lease_pipeline_path) return leased_pipeline("gemv_q6k", {});
    if (global_pipeline_access_blocked()) return nil;
...
#if defined(LAPLACE_METAL_TESTING) || defined(LAPLACE_TESTING)
static id<MTLComputePipelineState> get_quantized_variant_pipe(
    GGMLType type, uint32_t rows_per_simd, uint32_t simdgroups) {
    if (g_lease_pipeline_path || global_pipeline_access_blocked() || !g_lib)
        return nil;
    if ((type != GGMLType::Q4_K && type != GGMLType::Q6_K) ||
        (rows_per_simd != 1u && rows_per_simd != 2u && rows_per_simd != 4u) ||
        (simdgroups != 1u && simdgroups != 2u && simdgroups != 4u))
        return nil;
    const uint32_t key = static_cast<uint32_t>(type) |
                         (rows_per_simd << 8) | (simdgroups << 16);
    {
        std::lock_guard<std::mutex> lock(g_pipe_mtx);
        const auto found = g_quantized_variant_pipes.find(key);
        if (found != g_quantized_variant_pipes.end()) return found->second;
    }
    const int quant_type = static_cast<int>(type);
    const uint32_t rows_constant = rows_per_simd;
    const uint32_t groups_constant = simdgroups;
    MTLFunctionConstantValues* values = [MTLFunctionConstantValues new];
    [values setConstantValue:&quant_type type:MTLDataTypeInt atIndex:0];
    [values setConstantValue:&rows_constant type:MTLDataTypeUInt atIndex:7];
    [values setConstantValue:&groups_constant type:MTLDataTypeUInt atIndex:8];
    NSError* error = nil;
    id<MTLFunction> function =
        [g_lib newFunctionWithName:@"gemv_quantized_variant"
                    constantValues:values error:&error];
    [values release];
    if (!function) {
        std::fprintf(stderr,
                     "[metal] quantized variant function rows=%u groups=%u: %s\n",
                     rows_per_simd, simdgroups,
    ...
    if (!pipeline) {
        std::fprintf(stderr,
                     "[metal] quantized variant pipeline rows=%u groups=%u: %s\n",
                     rows_per_simd, simdgroups,
                     error ? error.localizedDescription.UTF8String : "unavailable");
        return nil;
    }
    if (pipeline.threadExecutionWidth != 32 ||
        pipeline.maxTotalThreadsPerThreadgroup < 32u * simdgroups) {
        [pipeline release];
        return nil;
    }
    std::lock_guard<std::mutex> lock(g_pipe_mtx);
    const auto [it, inserted] = g_quantized_variant_pipes.emplace(key, pipeline);
    return inserted ? pipeline : it->second;
}

static id<MTLComputePipelineState> get_q6k_llama_variant_pipe() {
    if (g_lease_pipeline_path || global_pipeline_access_blocked() || !g_lib)
        return nil;
    ...
    NSError* error = nil;
    id<MTLFunction> function =
        [g_lib newFunctionWithName:@"gemv_q6k_llama_variant"];
    if (!function) return nil;
    pipeline = [g_dev newComputePipelineStateWithFunction:function error:&error];
    [function release];
    if (!pipeline || pipeline.threadExecutionWidth != 32 ||
...
bool metal_test_quantized_gemv_variant(
    ...
    if (requested_bytes) *requested_bytes = 0;
    if (command_buffers) *command_buffers = 0;
    if (thread_execution_width) *thread_execution_width = 0;
    if (max_threads_per_threadgroup) *max_threads_per_threadgroup = 0;
    init();
    if (!g_dev || !g_q || !input || !output || !sample_gpu_ms ||
        !requested_bytes || !command_buffers || !thread_execution_width ||
        !max_threads_per_threadgroup || samples == 0u ||
        (weight.type != GGMLType::Q4_K && weight.type != GGMLType::Q6_K) ||
        (llama_q6_variant && (weight.type != GGMLType::Q6_K ||
                              rows_per_simd != 2u || simdgroups != 2u)) ||
        (q6_reference && (weight.type != GGMLType::Q6_K ||
                          rows_per_simd != 2u || simdgroups != 2u)) ||
        (llama_q6_variant && q6_reference) ||
        weight.n_dims != 2 || weight.dims[0] == 0 || weight.dims[1] == 0 ||
        weight.dims[0] % 256u != 0 || rows_per_simd == 0u || simdgroups == 0u ||
        dispatches_per_command == 0u || dispatches_per_command > 1024u ||
        samples > 1024u || warmups > 1024u)
        return false;
    std::fill(sample_gpu_ms, sample_gpu_ms + samples, 0.0);
    const uint64_t K64 = weight.dims[0];
    const uint64_t N64 = weight.dims[1];
    if (K64 > static_cast<uint64_t>(INT_MAX) ||
        N64 > static_cast<uint64_t>(INT_MAX)) return false;
    const size_t row_bytes = static_cast<size_t>(K64 / 256u) *
                             bytes_per_block(weight.type);
    if (row_bytes == 0 || N64 > SIZE_MAX / row_bytes) return false;
    const size_t weight_bytes = static_cast<size_t>(N64) * row_bytes;
    if (!weight.data || !strict_tensor_data_span(weight, weight_bytes)) return false;
    id<MTLComputePipelineState> pipeline = q6_reference
        ? get_q6k_pipe()
        : llama_q6_variant
        ? get_q6k_llama_variant_pipe()
        : get_quantized_variant_pipe(weight.type, rows_per_simd, simdgroups);
    if (!pipeline) return false;

    const int K = static_cast<int>(K64);
    const int N = static_cast<int>(N64);
    const uint64_t rb = static_cast<uint64_t>(row_bytes);
    const int M = 1;
    const NSUInteger threads = static_cast<NSUInteger>(32u * simdgroups);
    const NSUInteger rows_per_threadgroup =
        static_cast<NSUInteger>(rows_per_simd) * simdgroups;
    const NSUInteger threadgroups =
        (static_cast<NSUInteger>(N) + rows_per_threadgroup - 1u) /
        rows_per_threadgroup;
    @autoreleasepool {
        id<MTLBuffer> weights = [g_dev newBufferWithBytes:weight.data
                                                     length:weight_bytes
                                                    options:MTLResourceStorageModeShared];
        id<MTLBuffer> input_buffer = [g_dev newBufferWithBytes:input
                                                         length:static_cast<size_t>(K) * sizeof(float)
                                                        options:MTLResourceStorageModeShared];
        id<MTLBuffer> output_buffer = [g_dev newBufferWithLength:
        ...
        std::memset([output_buffer contents], 0,
                    static_cast<size_t>(N) * sizeof(float));
        const auto run_sample = [&](double* elapsed_ms) -> bool {
            ...
            [encoder setBytes:&K length:sizeof(K) atIndex:3];
            [encoder setBytes:&N length:sizeof(N) atIndex:4];
            [encoder setBytes:&rb length:sizeof(rb) atIndex:5];
            [encoder setBytes:&M length:sizeof(M) atIndex:6];
            for (uint32_t dispatch = 0; dispatch != dispatches_per_command; ++dispatch)
            ...
            const auto wait_end = std::chrono::steady_clock::now();
            if (command.status != MTLCommandBufferStatusCompleted) {
                std::fprintf(stderr, "[metal] quantized variant command: %s\n",
                             command_buffer_failure_detail(command).c_str());
                return false;
            }
            record_completed_command(command, wait_start, wait_end);
            ...
            *elapsed_ms = gpu_end >= gpu_start ?
                1000.0 * (gpu_end - gpu_start) : host_ms;
            return std::isfinite(*elapsed_ms) && *elapsed_ms > 0.0;
        };
        for (uint32_t warmup = 0; warmup != warmups; ++warmup) {
        ...
        std::memcpy(output, [output_buffer contents],
                    static_cast<size_t>(N) * sizeof(float));
        *requested_bytes = weight_bytes;
        *command_buffers = warmups + samples;
        *thread_execution_width = static_cast<uint32_t>(pipeline.threadExecutionWidth);
...
bool metal_test_q6k_alias_paired(
    ...
    if (max_threads_per_threadgroup) *max_threads_per_threadgroup = 0;
    init();
    if (!g_dev || !g_q || !input || !reference_output || !candidate_output ||
        !reference_gpu_ms || !candidate_gpu_ms || !reference_wall_ms ||
        !candidate_wall_ms || !requested_bytes || !command_buffers ||
        !thread_execution_width || !max_threads_per_threadgroup ||
        weight.type != GGMLType::Q6_K || weight.n_dims != 2 ||
        weight.dims[0] == 0 || weight.dims[1] == 0 ||
        weight.dims[0] % 256u != 0 || warmups < 10u || samples < 20u)
        return false;
    ...
    if (K64 > static_cast<uint64_t>(INT_MAX) ||
        N64 > static_cast<uint64_t>(INT_MAX)) return false;
    const uint64_t blocks_per_row = K64 / 256u;
    if (blocks_per_row > UINT64_MAX / bytes_per_block(GGMLType::Q6_K))
        return false;
    const uint64_t row_bytes64 = blocks_per_row * bytes_per_block(GGMLType::Q6_K);
    if (row_bytes64 == 0 || N64 > UINT64_MAX / row_bytes64)
        return false;
    const uint64_t weight_bytes64 = N64 * row_bytes64;
    if (weight_bytes64 > SIZE_MAX ||
        !weight.data || !strict_tensor_data_span(weight, weight_bytes64))
        return false;

    // Construct both pipeline states before timing. The two arms share one
    // persistent input and weight allocation and use equal-sized output
    // allocations; only the typed physical decomposition differs.
    id<MTLComputePipelineState> reference = get_q6k_pipe();
    id<MTLComputePipelineState> candidate = get_q6k_llama_variant_pipe();
    if (!reference || !candidate || reference.threadExecutionWidth != 32u ||
        candidate.threadExecutionWidth != 32u ||
        reference.maxTotalThreadsPerThreadgroup < 64u ||
        candidate.maxTotalThreadsPerThreadgroup < 64u)
        return false;

    const int K = static_cast<int>(K64);
    const int N = static_cast<int>(N64);
    const uint64_t row_bytes = row_bytes64;
    const int M = 1;
    const NSUInteger threadgroups = (static_cast<NSUInteger>(N) + 3u) / 4u;
    const MTLSize threads = MTLSizeMake(64, 1, 1);
    const MTLSize groups = MTLSizeMake(threadgroups, 1, 1);
    const size_t weight_bytes = static_cast<size_t>(weight_bytes64);
    const size_t input_bytes = static_cast<size_t>(K) * sizeof(float);
    const size_t output_bytes = static_cast<size_t>(N) * sizeof(float);
    std::fill(reference_gpu_ms, reference_gpu_ms + samples, 0.0);
    std::fill(candidate_gpu_ms, candidate_gpu_ms + samples, 0.0);
    std::fill(reference_wall_ms, reference_wall_ms + samples, 0.0);
    std::fill(candidate_wall_ms, candidate_wall_ms + samples, 0.0);
    ...
    @autoreleasepool {
        ...
        id<MTLBuffer> input_buffer = [g_dev newBufferWithBytes:input
                                                         length:input_bytes
                                                        options:MTLResourceStorageModeShared];
        id<MTLBuffer> reference_buffer = [g_dev newBufferWithLength:output_bytes
                                                              options:MTLResourceStorageModeShared];
        id<MTLBuffer> candidate_buffer = [g_dev newBufferWithLength:output_bytes
                                                              options:MTLResourceStorageModeShared];
        auto release_buffers = [&] {
            [weights release];
            [input_buffer release];
        ...
        if (!weights || !input_buffer || !reference_buffer || !candidate_buffer) {
            release_buffers();
            return false;
        }
        std::memset(reference_buffer.contents, 0, output_bytes);
        std::memset(candidate_buffer.contents, 0, output_bytes);

        auto run = [&](id<MTLComputePipelineState> pipeline,
                       id<MTLBuffer> output_buffer, double& gpu_ms,
                       double& wall_ms) -> bool {
            id<MTLCommandBuffer> command = [g_q commandBuffer];
            id<MTLComputeCommandEncoder> encoder =
                command ? [command computeCommandEncoder] : nil;
            ...
            const auto wait_end = std::chrono::steady_clock::now();
            if (command.status != MTLCommandBufferStatusCompleted) {
                std::fprintf(stderr, "[metal] q6k alias command: %s\n",
                             command_buffer_failure_detail(command).c_str());
                return false;
            }
            record_completed_command(command, wait_start, wait_end);
            ...
            gpu_ms = gpu_end >= gpu_start ?
                1000.0 * (gpu_end - gpu_start) : wall_ms;
        ...
        for (uint32_t warmup = 0; warmup != warmups; ++warmup) {
            if (!run(reference, reference_buffer, ignored_gpu, ignored_wall) ||
                !run(candidate, candidate_buffer, ignored_gpu, ignored_wall)) {
                release_buffers();
                return false;
            }
        }
        ...
        for (uint32_t sample = 0; sample != samples; ++sample) {
            double first_gpu = 0.0, first_wall = 0.0;
            double second_gpu = 0.0, second_wall = 0.0;
            double third_gpu = 0.0, third_wall = 0.0;
            double fourth_gpu = 0.0, fourth_wall = 0.0;
            const bool baab = (sample & 1u) != 0u;
            const bool first_ok = baab
                ? run(candidate, candidate_buffer, first_gpu, first_wall)
                : run(reference, reference_buffer, first_gpu, first_wall);
            const bool second_ok = baab
                ? run(reference, reference_buffer, second_gpu, second_wall)
                : run(candidate, candidate_buffer, second_gpu, second_wall);
            const bool third_ok = baab
                ? run(reference, reference_buffer, third_gpu, third_wall)
                : run(candidate, candidate_buffer, third_gpu, third_wall);
            const bool fourth_ok = baab
                ? run(candidate, candidate_buffer, fourth_gpu, fourth_wall)
                : run(reference, reference_buffer, fourth_gpu, fourth_wall);
            if (!first_ok || !second_ok || !third_ok || !fourth_ok) {
                release_buffers();
                return false;
            }
            if (baab) {
                candidate_gpu_ms[sample] = (first_gpu + fourth_gpu) * 0.5;
                candidate_wall_ms[sample] = (first_wall + fourth_wall) * 0.5;
                reference_gpu_ms[sample] = (second_gpu + third_gpu) * 0.5;
                reference_wall_ms[sample] = (second_wall + third_wall) * 0.5;
            } else {
                reference_gpu_ms[sample] = (first_gpu + fourth_gpu) * 0.5;
                reference_wall_ms[sample] = (first_wall + fourth_wall) * 0.5;
                candidate_gpu_ms[sample] = (second_gpu + third_gpu) * 0.5;
                candidate_wall_ms[sample] = (second_wall + third_wall) * 0.5;
            }
        }
        std::memcpy(reference_output, reference_buffer.contents, output_bytes);
        std::memcpy(candidate_output, candidate_buffer.contents, output_bytes);
        *requested_bytes = weight_bytes64;
        *command_buffers = 2u * warmups + 4u * samples;
        *thread_execution_width = static_cast<uint32_t>(reference.threadExecutionWidth);
        *max_threads_per_threadgroup = static_cast<uint32_t>(
            std::min(reference.maxTotalThreadsPerThreadgroup,
                     candidate.maxTotalThreadsPerThreadgroup));
        std::fprintf(stderr,
                     "quantized-q6k alias pipelines reference=gemv_q6k "
                     "candidate=gemv_q6k_llama_variant warmups=%u samples=%u "
                     "order=ABBA/BAAB shared_buffers=1\n",
                     warmups, samples);
        release_buffers();
        return true;
    }
...
static id<MTLComputePipelineState> named_pipe(const char* name) {
    if (g_lease_pipeline_path) return leased_pipeline(name, {});
    if (global_pipeline_access_blocked()) return nil;
...
static id<MTLComputePipelineState> get_router_topk_pipe() {
    if (g_lease_pipeline_path) return leased_pipeline("router_topk_f32", {});
    if (global_pipeline_access_blocked()) return nil;
...
struct MetalTokContext {
    ...
    // The queue and all session-owned resources must be created on this
    // device. Legacy sessions continue to use the process device; leased
    // sessions never consult g_dev for device selection.
    ...
    bool lease_backed = false;
    const MetalPipelineLease* leased_lease = nullptr;
    const std::vector<const void*>* leased_pipeline_tokens = nullptr;
    const std::vector<MetalPipelineRecipe>* leased_recipes = nullptr;
    const std::vector<uint32_t>* leased_invocations = nullptr;
    const std::vector<MetalTokInvocationAuthority>* leased_invocation_authorities = nullptr;
    const std::vector<MetalTokProgramRange>* leased_program_ranges = nullptr;
    MetalPipelineDigest leased_generation{};
    uint32_t leased_program_id = UINT32_MAX;
    uint32_t leased_program_begin = 0;
    uint32_t leased_program_end = 0;
    uint32_t leased_program_cursor = 0;
    bool leased_program_selected = false;
    bool require_invocation_authority = false;
    uint32_t leased_program_batch_rows = 1;
    uint32_t leased_selected_output_row = UINT32_MAX;
    uint32_t leased_dispatch_row_index = 0;
    uint32_t leased_dispatch_row_count = 1;
...
public:
    explicit MetalTokScope(MetalTokContext& context)
        : previous_tok_(g_active_tok_context), previous_weights_(g_active_weight_context),
          previous_lease_path_(g_lease_pipeline_path) {
        g_active_tok_context = &context;
        g_active_weight_context = context.weights;
        g_lease_pipeline_path = context.lease_backed;
    }

    ~MetalTokScope() {
        g_lease_pipeline_path = previous_lease_path_;
...
private:
    ...
    bool previous_lease_path_;
};

static bool leased_constants_match(
    std::span<const MetalFunctionConstant> expected,
    std::span<const MetalFunctionConstant> actual) noexcept {
    if (expected.size() != actual.size()) return false;
    for (const MetalFunctionConstant& constant : expected) {
        bool found = false;
        for (const MetalFunctionConstant& candidate : actual) {
...
static id<MTLComputePipelineState> leased_pipeline(
    ...
    if (!context.lease_backed || !function || !context.leased_lease ||
        !context.leased_pipeline_tokens || !context.leased_recipes ||
        !context.leased_invocations || !context.leased_program_selected ||
        context.leased_program_cursor >= context.leased_program_end ||
        context.leased_program_cursor >= context.leased_invocations->size() ||
        (context.require_invocation_authority &&
         (!context.leased_invocation_authorities ||
          context.leased_program_cursor >=
              context.leased_invocation_authorities->size())))
        return nil;

    const uint32_t recipe_index =
        (*context.leased_invocations)[context.leased_program_cursor];
    if (context.require_invocation_authority) {
        const MetalTokInvocationAuthority& authority =
            (*context.leased_invocation_authorities)[
                context.leased_program_cursor];
        if (authority.program_id != context.leased_program_id ||
            authority.invocation_ordinal != context.leased_program_cursor ||
            authority.pipeline_slot != recipe_index ||
            authority.batch_rows != context.leased_program_batch_rows ||
            authority.row_index != context.leased_dispatch_row_index ||
            authority.row_count != context.leased_dispatch_row_count ||
            authority.invocation_digest !=
                metal_invocation_authority_digest(authority)) {
            context.failure_detail = "leased invocation authority mismatch";
            return nil;
        }
    }
    if (recipe_index >= context.leased_recipes->size() ||
        recipe_index >= context.leased_pipeline_tokens->size())
        return nil;
    const MetalPipelineRecipe& recipe = (*context.leased_recipes)[recipe_index];
    if (recipe.function_name != function ||
        !leased_constants_match(recipe.function_constants, constants))
        return nil;

    uint32_t resolved_slot = UINT32_MAX;
    const void* resolved_pipeline = nullptr;
    const void* pipeline_token = (*context.leased_pipeline_tokens)[recipe_index];
    if (!pipeline_token ||
        !context.leased_lease->resolve_recipe(recipe, &resolved_slot,
                                              &resolved_pipeline) ||
        resolved_slot != recipe_index || resolved_pipeline != pipeline_token ||
        context.leased_lease->generation_digest() != context.leased_generation ||
        context.leased_lease->device_token() != context.device ||
        !context.queue || [context.queue device] != context.device)
        return nil;

    id<MTLComputePipelineState> pipeline =
        (id<MTLComputePipelineState>)const_cast<void*>(pipeline_token);
    if (!pipeline || [pipeline device] != context.device ||
        pipeline.maxTotalThreadsPerThreadgroup < 1)
        return nil;
    ++context.leased_program_cursor;
    return pipeline;
}
...
static bool tok_fail(const char* detail) {
    MetalTokContext& context = active_tok_context();
    if (context.failure_detail.empty())
        context.failure_detail = detail ? detail : "Metal token transaction failed";
    return false;
}

struct TokLeaseDispatchScope {
    MetalTokContext& context;
    const uint32_t previous_index;
    const uint32_t previous_count;

    TokLeaseDispatchScope(uint32_t index, uint32_t count)
        : context(active_tok_context()),
          previous_index(context.leased_dispatch_row_index),
          previous_count(context.leased_dispatch_row_count) {
        context.leased_dispatch_row_index = index;
        context.leased_dispatch_row_count = count;
    }

    ~TokLeaseDispatchScope() {
        context.leased_dispatch_row_index = previous_index;
        context.leased_dispatch_row_count = previous_count;
    }
};

static id<MTLDevice> tok_device() {
    MetalTokContext& context = active_tok_context();
    return context.device ? context.device : g_dev;
}
...
static void tok_profile_prepare(int n_layers, uint32_t token_count) {
    ...
    if (!device || ![device supportsCounterSampling:MTLCounterSamplingPointAtDispatchBoundary]) {
    ...
    if (@available(macOS 26.0, *)) frequency = [device queryTimestampFrequency];
    if (frequency == 0) {
    ...
    NSError* error = nil;
    id<MTLCounterSampleBuffer> samples = [device newCounterSampleBufferWithDescriptor:descriptor error:&error];
    [descriptor release];
    if (!samples) {
...
static bool tok_begin(int H, int inter, int exp_inter, int n_used, int n_experts,
    ...
    MetalTokContext& context = active_tok_context();
    if (!continue_command_buffer) context.failure_detail.clear();
    if (!context.lease_backed) init();
    id<MTLDevice> device = tok_device();
    if (!device || H < 1 || max_seq < 1 || n_layers < 1)
        return tok_fail("token begin has invalid device or geometry");
    if (n_used < 0 || n_experts < 0 || n_experts > 512)
        return tok_fail("token begin has invalid MoE geometry");
    ...
    if (context.lease_backed &&
        (!context.leased_program_selected ||
         context.leased_program_batch_rows != batch_rows)) {
        return tok_fail(
            "leased program batch rows do not match the requested token transaction");
    }
    if ((query_capacity == 0) != (kv_stride == 0) || query_capacity < 0 || kv_stride < 0 ||
        (has_attention && (Hq > INT_MAX / Dh || Hk > INT_MAX / Dh)))
        return tok_fail("token begin has invalid attention capacity");
    ...
    if ((has_attention && (query_capacity < layer_query_width || kv_stride < layer_kv_width ||
        ...
        (!has_attention && (query_capacity != 0 || kv_stride != 0 || kv_width_sum != 0)))
        return tok_fail("token begin attention capacity is below the layer geometry");
    ...
    if (continue_command_buffer) {
        tok_split();
        keep_cmd = g_tok.cmd;
        if (!keep_cmd) return tok_fail("continuing token transaction has no command buffer");
    }
    g_tok = TokWS{};
    g_tok.ws = keep_ws;
    ...
    if (n_used != 0 && (n_used > INT_MAX / 2 ||
                          ...
                          n_used > INT_MAX / H))
        return tok_fail("token begin has overflowing MoE workspace geometry");
    ...
    const size_t required_workspace_bytes = o;
    if (!workspace_valid || o == 0) {
        g_tok.ws = keep_ws;
        g_tok.bytes = keep_bytes;
        g_tok.cmd = keep_cmd;
        return tok_fail("token begin workspace geometry is invalid");
    }
    id<MTLBuffer> replacement_workspace = nil;
    if (!keep_ws || keep_bytes < required_workspace_bytes) {
        replacement_workspace =
            [device newBufferWithLength:required_workspace_bytes
                                options:MTLResourceStorageModeShared];
        if (!replacement_workspace) {
            g_tok.ws = keep_ws;
            g_tok.bytes = keep_bytes;
            g_tok.cmd = keep_cmd;
            return tok_fail("token begin workspace allocation failed");
        }
    }
    if (has_attention) {
        ...
        if (widths == 0 || widths > SIZE_MAX || sequence > SIZE_MAX / static_cast<size_t>(widths) ||
            ...
            [replacement_workspace release];
            g_tok.ws = keep_ws;
            g_tok.bytes = keep_bytes;
            g_tok.cmd = keep_cmd;
            return tok_fail("token begin KV cache geometry overflowed");
        }
        ...
        if (kvb > UINT64_MAX / 2u) {
            ...
            g_tok.bytes = keep_bytes;
            g_tok.cmd = keep_cmd;
            return tok_fail("token begin KV cache size overflowed");
        }
        if (!g_kcache || g_kv_bytes != kvb) {
            id<MTLBuffer> replacement_kcache =
                [device newBufferWithLength:kvb options:MTLResourceStorageModeShared];
            id<MTLBuffer> replacement_vcache =
                [device newBufferWithLength:kvb options:MTLResourceStorageModeShared];
            if (!replacement_kcache || !replacement_vcache) {
                [replacement_kcache release];
                [replacement_vcache release];
                [replacement_workspace release];
                g_tok.ws = keep_ws;
                g_tok.bytes = keep_bytes;
                g_tok.cmd = keep_cmd;
                return tok_fail("token begin KV cache allocation failed");
            }
            [g_kcache release];
            [g_vcache release];
            g_kcache = replacement_kcache;
            g_vcache = replacement_vcache;
            g_kv_bytes = kvb;
            g_kv_seeded_to = -1;
        }
    ...
    if (replacement_workspace) {
        [keep_ws release];
        g_tok.ws = replacement_workspace;
        g_tok.bytes = required_workspace_bytes;
    } else {
        g_tok.ws = keep_ws;
        g_tok.bytes = keep_bytes;
    }
    if (!g_tok.ws || (has_attention && (!g_tok.kcache || !g_tok.vcache))) {
        return tok_fail("token begin did not retain required buffers");
    }
...
static bool tok_rms(const Tensor& w, size_t src, size_t dst, int n, float eps) {
    if (!tok_enc() || !w.data || n < 1 || !std::isfinite(eps) || eps < 0.0f ||
        (w.type != GGMLType::F32 && w.type != GGMLType::Q8_0)) return false;
    if (w.type == GGMLType::Q8_0 &&
        (w.n_dims != 1 || w.dims[0] != static_cast<uint64_t>(n) || (n % 32) != 0))
        return false;
    const uint64_t block_count = static_cast<uint64_t>(n / 32);
    uint64_t weight_bytes = 0;
    if (w.type == GGMLType::Q8_0) {
        if (block_count > UINT64_MAX / bytes_per_block(GGMLType::Q8_0)) return false;
        weight_bytes = block_count * bytes_per_block(GGMLType::Q8_0);
    } else {
        if (static_cast<uint64_t>(n) > UINT64_MAX / sizeof(float)) return false;
        weight_bytes = static_cast<uint64_t>(n) * sizeof(float);
    }
    if (weight_bytes == 0 || weight_bytes > SIZE_MAX ||
        !strict_tensor_data_span(w, weight_bytes)) return false;
    size_t woff = 0;
    id<MTLBuffer> wb = get_weight_buf(w.data, static_cast<size_t>(weight_bytes), woff);
    if (!wb) return false;
    auto p = named_pipe(w.type == GGMLType::Q8_0 ? "rmsnorm_q8_0" : "rmsnorm_f32");
    if (!p) return false;
...
#define TOKFAIL(s) do { fprintf(stderr, "[metal] tok fail %s Hq=%d Hk=%d Dh=%d inter=%d\n", \
    ...
    if (L.q_norm && L.k_norm) {
        ...
        [g_tok.enc dispatchThreadgroups:MTLSizeMake(Hq, 1, 1)
           threadsPerThreadgroup:MTLSizeMake(64, 1, 1)];
        if (g_lease_pipeline_path) {
            pr = named_pipe("rmsnorm_rows_f32");
            if (!pr) TOKFAIL("rmsnorm_rows");
        }
        [g_tok.enc setBuffer:g_tok.ws offset:g_tok.k atIndex:0];
    ...
    if (g_lease_pipeline_path) {
        rp = named_pipe(L.rope_multi_section ? "rope_multisection_f32" :
                        L.rope_interleaved ? "rope_interleaved_f32" : "rope_f32");
        if (!rp) TOKFAIL("rope");
    }
    [g_tok.enc setBuffer:g_tok.ws offset:g_tok.k atIndex:0];
    ...
    if (L.out_scale && L.out_scale->data) {
        const float s = std::bit_cast<float>(L.moe_output_scale_bits);
        if (!std::isfinite(s)) TOKFAIL("output_scale");
        auto ps = named_pipe("vec_scale");
...
static bool metal_tok_dense_prefill_batch_layer(const MetalTokLayer& L) {
    ...
    for (uint32_t row = 0; row != rows; ++row) {
        ...
        if (!tok_rms(*L.attn_norm, tok_batch_row(g_tok.x, H, row),
                     tok_batch_row(g_tok.xn, H, row), H, L.rms_eps))
            return tok_fail("prefill batch attention norm encoding failed");
    }
    tok_barrier();
    {
        TokLeaseDispatchScope dispatch(UINT32_MAX, rows);
        if (!tok_bind_f16_rows(*L.attn_q, g_tok.xn, g_tok.q, rows, H, qn) ||
            ...
            !tok_profile_mark(TokProfileSegment::Qkv))
            return tok_fail("prefill batch QKV encoding failed");
    }
    tok_barrier();
    for (uint32_t row = 0; row != rows; ++row) {
        TokLeaseDispatchScope dispatch(row, 1);
        if (!tok_prefill_f16_rope_and_kv(L, row, g_tok.pos + static_cast<int>(row)))
            return tok_fail("prefill batch RoPE/KV encoding failed");
    }
    tok_barrier();
    for (uint32_t row = 0; row != rows; ++row) {
        TokLeaseDispatchScope dispatch(row, 1);
        if (!tok_prefill_f16_attention(L, row, g_tok.pos + static_cast<int>(row)))
            return tok_fail("prefill batch attention encoding failed");
    }
    tok_barrier();
    {
        TokLeaseDispatchScope dispatch(UINT32_MAX, rows);
        if (!tok_bind_f16_rows(*L.attn_o, g_tok.ao, g_tok.xb, rows, qn, H))
            return tok_fail("prefill batch attention output encoding failed");
    }
    tok_barrier();
    for (uint32_t row = 0; row != rows; ++row) {
        TokLeaseDispatchScope dispatch(row, 1);
        if (!tok_add(tok_batch_row(g_tok.x, H, row), tok_batch_row(g_tok.xb, H, row),
            ...
            !tok_rms(*L.ffn_norm, tok_batch_row(g_tok.x, H, row),
                     tok_batch_row(g_tok.xn, H, row), H, L.rms_eps))
            return tok_fail("prefill batch attention residual encoding failed");
    }
    if (!tok_profile_mark(TokProfileSegment::Attention))
        return tok_fail("prefill batch attention profiling failed");
    tok_barrier();
    {
        TokLeaseDispatchScope dispatch(UINT32_MAX, rows);
        if (!tok_bind_f16_rows(*L.ffn_gate, g_tok.xn, g_tok.fg, rows, H, L.inter) ||
            !tok_bind_f16_rows(*L.ffn_up, g_tok.xn, g_tok.fu, rows, H, L.inter))
            return tok_fail("prefill batch FFN input encoding failed");
    }
    tok_barrier();
    for (uint32_t row = 0; row != rows; ++row) {
        TokLeaseDispatchScope dispatch(row, 1);
        if (!tok_enc() || !enqueue_glu(g_tok.enc, g_tok.ws,
                                       ...
                                       tok_batch_row(g_tok.fh, L.inter, row), L.inter, 1))
            return tok_fail("prefill batch GLU encoding failed");
    }
    tok_barrier();
    {
        TokLeaseDispatchScope dispatch(UINT32_MAX, rows);
        if (!tok_bind_f16_rows(*L.ffn_down, g_tok.fh, g_tok.xb, rows, L.inter, H))
            return tok_fail("prefill batch FFN output encoding failed");
    }
    tok_barrier();
    for (uint32_t row = 0; row != rows; ++row) {
        TokLeaseDispatchScope dispatch(row, 1);
        if (!tok_add(tok_batch_row(g_tok.x, H, row), tok_batch_row(g_tok.xb, H, row),
                     tok_batch_row(g_tok.x, H, row), H))
            return tok_fail("prefill batch FFN residual encoding failed");
    }
    if (!tok_profile_mark(TokProfileSegment::Ffn))
        return tok_fail("prefill batch FFN profiling failed");
    return true;
}
...
static bool metal_tok_final_impl(const Tensor& norm, const Tensor& lm, float* logits,
    ...
    MetalTokContext& context = active_tok_context();
    if (context.lease_backed &&
        (context.leased_selected_output_row == UINT32_MAX ||
         context.leased_selected_output_row >= g_tok.batch_rows ||
         g_tok.selected_batch_row != context.leased_selected_output_row)) {
        context.failure_detail =
            "selected output row does not match the leased semantic program";
        return false;
    }
    ...
    if (!g_logits_buf || g_logits_bytes < need) {
        g_logits_buf = [tok_device() newBufferWithLength:need
                                          options:MTLResourceStorageModeShared];
        g_logits_bytes = need;
    }
    ...
    if (sample_logits) {
        ...
        if (!active_tok_context().sampler_result) {
            active_tok_context().sampler_result =
                [tok_device() newBufferWithLength:sizeof(MetalSamplerResult)
                                   options:MTLResourceStorageModeShared];
        }
        const NSUInteger threads = std::min<NSUInteger>(
    ...
    if (!leased_program_complete(active_tok_context())) {
        active_tok_context().failure_detail =
            "leased program ended before all invocations were consumed";
        return false;
    }
    tok_split();
    if (!g_tok.cmd) return false;
...
public:
    ...
    MetalTokSession(id<MTLCommandQueue> queue, MetalPipelineLease&& lease,
                    std::vector<const void*>&& pipeline_tokens,
                    std::vector<MetalPipelineRecipe>&& recipes,
                    std::vector<uint32_t>&& invocations,
                    std::vector<MetalTokProgramRange>&& program_ranges,
                    std::vector<MetalTokInvocationAuthority>&& authorities) noexcept
        : pipeline_lease_(std::move(lease)),
          leased_pipeline_tokens_(std::move(pipeline_tokens)),
          leased_recipes_(std::move(recipes)),
          leased_invocations_(std::move(invocations)),
          leased_program_ranges_(std::move(program_ranges)),
          leased_invocation_authorities_(std::move(authorities)) {
        token_.weights = &weights_;
        token_.device = (id<MTLDevice>)const_cast<void*>(pipeline_lease_->device_token());
        token_.single_command_buffer = true;
        token_.lease_backed = true;
        token_.leased_lease = &*pipeline_lease_;
        token_.leased_pipeline_tokens = &leased_pipeline_tokens_;
        token_.leased_recipes = &leased_recipes_;
        token_.leased_invocations = &leased_invocations_;
        token_.leased_invocation_authorities =
            &leased_invocation_authorities_;
        token_.leased_program_ranges = &leased_program_ranges_;
        token_.require_invocation_authority =
            !leased_invocation_authorities_.empty();
        token_.leased_generation = pipeline_lease_->generation_digest();
        token_.queue = queue;
    }
...
private:
    ...
    friend std::shared_ptr<MetalTokSession>
    metal_tok_session_create_from_pipeline_lease(
        MetalPipelineLease&&, std::span<const MetalPipelineRecipe>);
    friend std::shared_ptr<MetalTokSession>
    metal_tok_session_create_from_pipeline_lease(
        MetalPipelineLease&&, std::span<const MetalPipelineRecipe>,
        std::span<const uint32_t>, std::span<const MetalTokProgramRange>);
    friend std::shared_ptr<MetalTokSession>
    metal_tok_session_create_from_pipeline_lease(
        MetalPipelineLease&&, std::span<const MetalPipelineRecipe>,
        std::span<const uint32_t>, std::span<const MetalTokProgramRange>,
    ...
    friend bool metal_tok_session_begin_program(MetalTokSession&, uint32_t);
    friend bool metal_tok_session_select_program(MetalTokSession&, uint32_t);
    ...
    friend uintptr_t metal_tok_session_queue_device_identity_for_testing(const MetalTokSession&);
    friend uint32_t metal_tok_session_pipeline_lease_slot_count_for_testing(
        const MetalTokSession&);
    friend bool metal_tok_session_dispatch_leased_slot_for_testing(MetalTokSession&, uint32_t);
    friend bool metal_tok_session_probe_global_pipeline_for_testing(MetalTokSession&);
    friend bool metal_tok_session_dispatch_program_invocation_for_testing(
        MetalTokSession&, const char*, std::span<const MetalFunctionConstant>);
    friend uintptr_t metal_tok_session_pipeline_device_identity_for_testing(
        const MetalTokSession&, uint32_t);
    ...
    bool select_leased_program(uint32_t program_id) noexcept {
        if (token_.tok.live) return false;
        token_.leased_program_id = UINT32_MAX;
        token_.leased_program_begin = 0;
        token_.leased_program_end = 0;
        token_.leased_program_cursor = 0;
        token_.leased_program_selected = false;
        token_.leased_program_batch_rows = 1;
        token_.leased_selected_output_row = UINT32_MAX;
        for (const MetalTokProgramRange& range : leased_program_ranges_) {
            if (range.id != program_id) continue;
            token_.leased_program_id = range.id;
            token_.leased_program_begin = range.first_invocation;
            token_.leased_program_end =
                range.first_invocation + range.invocation_count;
            token_.leased_program_cursor = range.first_invocation;
            token_.leased_program_batch_rows = range.batch_rows;
            token_.leased_selected_output_row = range.selected_output_row;
            token_.leased_dispatch_row_index = 0;
            token_.leased_dispatch_row_count = 1;
            token_.leased_program_selected = true;
            return true;
        }
        return false;
    }
    ...
    std::optional<MetalPipelineLease> pipeline_lease_;
    // Retained by pipeline_lease_. This direct index is the only pipeline
    // selection operation available to a leased session hot path.
    std::vector<const void*> leased_pipeline_tokens_;
    std::vector<MetalPipelineRecipe> leased_recipes_;
    std::vector<uint32_t> leased_invocations_;
    std::vector<MetalTokProgramRange> leased_program_ranges_;
    std::vector<MetalTokInvocationAuthority> leased_invocation_authorities_;
...
static bool metal_pipeline_recipe_set_complete(
    const MetalPipelineLease& lease,
    std::span<const MetalPipelineRecipe> ordered_recipes) noexcept {
    const uint32_t slot_count = lease.slot_count();
    if (!lease.valid() || slot_count == 0 ||
        ordered_recipes.size() != static_cast<size_t>(slot_count))
        return false;

    for (size_t index = 0; index != ordered_recipes.size(); ++index) {
        uint32_t resolved_slot = UINT32_MAX;
        const void* resolved_pipeline = nullptr;
        if (!lease.resolve_recipe(ordered_recipes[index], &resolved_slot,
                                  &resolved_pipeline) ||
            resolved_slot != index ||
            resolved_pipeline != lease.pipeline_token(resolved_slot))
            return false;
    }
    return true;
}
...
static std::array<uint8_t, 32> metal_invocation_authority_digest(
    const MetalTokInvocationAuthority& authority) noexcept {
    std::array<uint8_t, 112> bytes{};
    size_t offset = 0;
    const auto append_u8 = [&](uint8_t value) { bytes[offset++] = value; };
    const auto append_u16 = [&](uint16_t value) {
        append_u8(static_cast<uint8_t>(value));
        append_u8(static_cast<uint8_t>(value >> 8u));
    };
    const auto append_u32 = [&](uint32_t value) {
        for (unsigned shift = 0; shift != 32; shift += 8)
            append_u8(static_cast<uint8_t>(value >> shift));
    };
    for (const uint8_t value :
         std::array<uint8_t, 8>{'L', 'P', 'M', 'I', 'N', 'V', 1, 0})
        append_u8(value);
    append_u16(authority.version);
    append_u32(authority.program_id);
    append_u32(authority.invocation_ordinal);
    append_u32(authority.group_ordinal);
    append_u16(authority.group_kind);
    append_u16(authority.group_shape);
    append_u16(authority.primitive);
    append_u32(authority.primitive_order);
    append_u32(authority.recipe_index);
    append_u32(authority.batch_rows);
    append_u32(authority.row_index);
    append_u32(authority.row_count);
    for (const uint8_t value : authority.semantic_program_digest)
        append_u8(value);
    for (const uint8_t value : authority.bound_program_digest)
        append_u8(value);
...
static bool zero_authority_digest(
    const std::array<uint8_t, 32>& digest) noexcept {
    return std::all_of(digest.begin(), digest.end(),
                       [](uint8_t value) { return value == 0; });
}

static const MetalTokProgramRange* invocation_program_range(
    uint32_t ordinal,
    std::span<const MetalTokProgramRange> program_ranges) noexcept {
    const MetalTokProgramRange* result = nullptr;
    for (const MetalTokProgramRange& range : program_ranges) {
        if (ordinal < range.first_invocation ||
            ordinal - range.first_invocation >= range.invocation_count)
            continue;
...
static bool metal_pipeline_invocation_authority_set_complete(
    std::span<const MetalPipelineRecipe> ordered_recipes,
    std::span<const uint32_t> flattened_invocations,
    std::span<const MetalTokProgramRange> program_ranges,
    std::span<const MetalTokInvocationAuthority> authorities) noexcept {
    if (authorities.empty() ||
        authorities.size() != flattened_invocations.size())
        return false;
    for (size_t index = 0; index != authorities.size(); ++index) {
        const MetalTokInvocationAuthority& authority = authorities[index];
        const MetalTokProgramRange* range = invocation_program_range(
            static_cast<uint32_t>(index), program_ranges);
        if (!range || authority.version != 1 ||
            authority.invocation_ordinal != index ||
            authority.program_id != range->id ||
            authority.group_kind < 1 || authority.group_kind > 4 ||
            authority.group_shape < 1 || authority.group_shape > 6 ||
            authority.primitive < 1 || authority.primitive > 35 ||
            authority.group_ordinal == UINT32_MAX ||
            authority.primitive_order == UINT32_MAX ||
            authority.recipe_index >= ordered_recipes.size() ||
            authority.pipeline_slot >= ordered_recipes.size() ||
            authority.pipeline_slot != flattened_invocations[index] ||
            authority.batch_rows != range->batch_rows ||
            authority.semantic_program_digest !=
                range->semantic_program_digest ||
            zero_authority_digest(authority.semantic_program_digest) ||
            zero_authority_digest(authority.bound_program_digest) ||
            authority.row_count == 0 ||
            (authority.row_index == UINT32_MAX
                 ? (authority.batch_rows <= 1 ||
                    authority.row_count != authority.batch_rows)
                 : (authority.row_count != 1 ||
                    authority.row_index >= authority.batch_rows)) ||
            authority.invocation_digest !=
                metal_invocation_authority_digest(authority) ||
            zero_authority_digest(authority.invocation_digest))
            return false;
    }
    return true;
}

static bool metal_pipeline_program_set_complete(
    const MetalPipelineLease& lease,
    std::span<const MetalPipelineRecipe> ordered_recipes,
    std::span<const uint32_t> flattened_invocations,
    std::span<const MetalTokProgramRange> program_ranges) noexcept {
    constexpr size_t kMaximumInvocations = 1u << 20;
    constexpr size_t kMaximumPrograms = 64;
    if (!metal_pipeline_recipe_set_complete(lease, ordered_recipes) ||
        flattened_invocations.empty() ||
        flattened_invocations.size() > kMaximumInvocations ||
        program_ranges.empty() || program_ranges.size() > kMaximumPrograms)
        return false;
    for (const uint32_t recipe_index : flattened_invocations)
        if (recipe_index >= ordered_recipes.size()) return false;
    size_t covered_invocations = 0;
            ...
            (range.selected_output_row != UINT32_MAX &&
             (range.selected_output_row >= range.batch_rows ||
              std::all_of(range.semantic_program_digest.begin(),
                          range.semantic_program_digest.end(),
                          [](uint8_t byte) { return byte == 0; }))) ||
            range.first_invocation > flattened_invocations.size() ||
            range.invocation_count >
                flattened_invocations.size() - range.first_invocation)
            return false;
        for (size_t prior = 0; prior != index; ++prior) {
            const MetalTokProgramRange& previous = program_ranges[prior];
            if (range.id == previous.id ||
                (range.first_invocation <
                     previous.first_invocation + previous.invocation_count &&
                 previous.first_invocation <
                 range.first_invocation + range.invocation_count))
                return false;
        }
        covered_invocations += range.invocation_count;
    }
    return covered_invocations == flattened_invocations.size();
}

static bool leased_program_complete(const MetalTokContext& context) noexcept {
    return !context.lease_backed ||
           (context.leased_program_selected &&
            context.leased_program_cursor == context.leased_program_end);
}

static void reset_leased_program_cursor(MetalTokContext& context) noexcept {
    if (context.lease_backed && context.leased_program_selected) {
        context.leased_program_cursor = context.leased_program_begin;
        context.leased_dispatch_row_index = 0;
        context.leased_dispatch_row_count = 1;
    }
}

static bool prepare_leased_program(MetalTokContext& context) noexcept {
    if (!context.lease_backed) return true;
    if (!context.leased_program_selected) return false;
    reset_leased_program_cursor(context);
    return true;
}
...
std::shared_ptr<MetalTokSession> metal_tok_session_create_from_pipeline_lease(
    ...
    std::vector<MetalTokProgramRange> program_ranges;
    try {
        flattened_invocations.resize(ordered_recipes.size());
        for (uint32_t index = 0; index != flattened_invocations.size(); ++index)
            flattened_invocations[index] = index;
        program_ranges.push_back(
            MetalTokProgramRange{0, 0, static_cast<uint32_t>(ordered_recipes.size())});
    } catch (...) {
        return {};
    }
    std::shared_ptr<MetalTokSession> session =
        metal_tok_session_create_from_pipeline_lease(
        std::move(lease), ordered_recipes,
        std::span<const uint32_t>(flattened_invocations),
        std::span<const MetalTokProgramRange>(program_ranges));
    if (session) session->select_leased_program(0);
    return session;
}

static std::shared_ptr<MetalTokSession>
metal_tok_session_create_from_pipeline_lease_impl(
    MetalPipelineLease&& lease,
    std::span<const MetalPipelineRecipe> ordered_recipes,
    std::span<const uint32_t> flattened_invocations,
    std::span<const MetalTokProgramRange> program_ranges,
    std::span<const MetalTokInvocationAuthority> invocation_authorities,
    bool require_invocation_authority) {
    if (!metal_pipeline_program_set_complete(
            lease, ordered_recipes, flattened_invocations, program_ranges) ||
        (require_invocation_authority
             ? !metal_pipeline_invocation_authority_set_complete(
                   ordered_recipes, flattened_invocations, program_ranges,
                   invocation_authorities)
             : !invocation_authorities.empty()))
        return {};

    id<MTLDevice> device =
        (id<MTLDevice>)const_cast<void*>(lease.device_token());
    if (!device) return {};

    // Resolve the complete ordered table before publishing a session. The
    // lease owns the pipeline objects; the session retains only direct slot
    // pointers, so dispatch cannot fall back to a process-global cache.
    std::vector<const void*> pipeline_tokens;
    std::vector<MetalPipelineRecipe> recipes;
    std::vector<uint32_t> invocations;
    std::vector<MetalTokProgramRange> ranges;
    std::vector<MetalTokInvocationAuthority> authorities;
    try {
        pipeline_tokens.reserve(ordered_recipes.size());
        recipes.assign(ordered_recipes.begin(), ordered_recipes.end());
        invocations.assign(flattened_invocations.begin(), flattened_invocations.end());
        ranges.assign(program_ranges.begin(), program_ranges.end());
        authorities.assign(invocation_authorities.begin(),
                           invocation_authorities.end());
        for (size_t index = 0; index != ordered_recipes.size(); ++index) {
            const void* pipeline_token = lease.pipeline_token(static_cast<uint32_t>(index));
            id<MTLComputePipelineState> pipeline =
                (id<MTLComputePipelineState>)const_cast<void*>(pipeline_token);
            if (!pipeline || [pipeline device] != device ||
                pipeline_token != lease.pipeline_token(static_cast<uint32_t>(index)) ||
                pipeline.maxTotalThreadsPerThreadgroup <
                    ordered_recipes[index].dispatch.min_threads_per_threadgroup ||
                pipeline.threadExecutionWidth <
                    ordered_recipes[index].dispatch.min_thread_execution_width ||
                (ordered_recipes[index].dispatch.max_thread_execution_width != 0 &&
                 pipeline.threadExecutionWidth >
                     ordered_recipes[index].dispatch.max_thread_execution_width))
                return {};
            for (const void* prior : pipeline_tokens)
                if (prior == pipeline_token) return {};
            pipeline_tokens.push_back(pipeline_token);
        }
    } catch (...) {
        // No command buffer has been created. Keep the lease with the caller
        // on an allocation failure just as the pre-queue validation does.
        return {};
    }
    ...
    try {
        auto session = std::make_shared<MetalTokSession>(
            queue, std::move(lease), std::move(pipeline_tokens),
            std::move(recipes), std::move(invocations), std::move(ranges),
...
std::shared_ptr<MetalTokSession> metal_tok_session_create_from_pipeline_lease(
    ...
    std::span<const uint32_t> flattened_invocations,
    std::span<const MetalTokProgramRange> program_ranges) {
    return metal_tok_session_create_from_pipeline_lease_impl(
        std::move(lease), ordered_recipes, flattened_invocations,
        program_ranges, {}, false);
}
...
std::shared_ptr<MetalTokSession> metal_tok_session_create_from_pipeline_lease(
    ...
    return metal_tok_session_create_from_pipeline_lease_impl(
        std::move(lease), ordered_recipes, flattened_invocations,
        program_ranges, invocation_authorities, true);
}

bool metal_tok_session_begin_program(MetalTokSession& session,
                                     uint32_t program_id) {
    MetalTokScope scope(session.token_);
    return session.token_.lease_backed &&
           session.select_leased_program(program_id);
}

bool metal_tok_session_select_program(MetalTokSession& session,
                                      uint32_t program_id) {
    MetalTokScope scope(session.token_);
    return session.token_.lease_backed &&
           session.select_leased_program(program_id);
...
uintptr_t metal_tok_session_queue_device_identity_for_testing(
    const MetalTokSession& session) {
    return session.token_.queue
        ? reinterpret_cast<uintptr_t>([session.token_.queue device])
        : 0;
}

uint32_t metal_tok_session_pipeline_lease_slot_count_for_testing(
    const MetalTokSession& session) {
    return session.pipeline_lease_ ? session.pipeline_lease_->slot_count() : 0;
}

uintptr_t metal_tok_session_pipeline_device_identity_for_testing(
    const MetalTokSession& session, uint32_t slot) {
    if (!session.pipeline_lease_ ||
        slot >= session.leased_pipeline_tokens_.size()) return 0;
    const void* token = session.leased_pipeline_tokens_[slot];
    id<MTLComputePipelineState> pipeline =
        (id<MTLComputePipelineState>)const_cast<void*>(token);
    return pipeline ? reinterpret_cast<uintptr_t>([pipeline device]) : 0;
}

bool metal_tok_session_dispatch_leased_slot_for_testing(
    MetalTokSession& session, uint32_t slot) {
    MetalTokScope scope(session.token_);
    if (!session.pipeline_lease_ || !session.token_.lease_backed ||
        !session.token_.device || !session.token_.queue ||
        slot >= session.leased_pipeline_tokens_.size()) return false;

    const void* token = session.leased_pipeline_tokens_[slot];
    if (!token || token != session.pipeline_lease_->pipeline_token(slot)) return false;
    id<MTLDevice> device = session.token_.device;
    id<MTLComputePipelineState> pipeline =
        (id<MTLComputePipelineState>)const_cast<void*>(token);
    if (!pipeline || [pipeline device] != device ||
        [session.token_.queue device] != device ||
        pipeline.maxTotalThreadsPerThreadgroup < 1)
        return false;

    id<MTLBuffer> scratch = [device newBufferWithLength:sizeof(uint32_t)
                                                 options:MTLResourceStorageModeShared];
    if (!scratch || [scratch device] != device) {
        [scratch release];
        return false;
    }
    *static_cast<uint32_t*>(scratch.contents) = 0;
    ...
    if (completed) {
        ...
        session.token_.failure_detail = command_buffer_failure_detail(command);
    }
    [scratch release];
    return completed;
}

bool metal_tok_session_probe_global_pipeline_for_testing(
    MetalTokSession& session) {
    MetalTokScope scope(session.token_);
    if (!session.pipeline_lease_ || !session.token_.lease_backed) return false;
    if (global_pipeline_access_blocked()) return false;
    return named_pipe("vec_add") != nil;
}
...
bool metal_tok_session_dispatch_program_invocation_for_testing(
    ...
    MetalTokScope scope(session.token_);
    if (!session.token_.lease_backed || !g_tok.live) return false;
    id<MTLComputePipelineState> pipeline = leased_pipeline(function, constants);
    if (!pipeline || !tok_enc()) return false;
    [g_tok.enc setComputePipelineState:pipeline];
    [g_tok.enc dispatchThreads:MTLSizeMake(1, 1, 1)
             threadsPerThreadgroup:MTLSizeMake(1, 1, 1)];
    return true;
}
#endif
...
MetalResourceSnapshot metal_tok_session_resource_snapshot(const MetalTokSession& session) {
    ...
    id<MTLDevice> device = tok_device();
    snapshot.current_allocated_size = device ? device.currentAllocatedSize : 0;
...
bool metal_tok_session_dense_ready(MetalTokSession& session) {
    MetalTokScope scope(session.token_);
    if (session.token_.lease_backed) return false;
    init();
    ...
    return g_dev && session.token_.queue && g_lib && sparse_proxy_ready && sparse_oracle_ready &&
           ...
           suitable(named_pipe("rmsnorm_q8_0")) &&
...
bool metal_tok_session_register_column_grouped_affine_u2_skip_256(
    ...
    try {
        ...
        candidate.metadata.value = [tok_device() newBufferWithLength:metadata_bytes
                                                         options:MTLResourceStorageModeShared];
        ...
        if (candidate.new_max_k != session.token_.column_grouped_affine_u2_max_k)
            candidate.active_columns.value = [tok_device() newBufferWithLength:
        ...
        if (candidate.new_max_n != session.token_.column_grouped_affine_u2_max_n)
            candidate.partial.value = [tok_device() newBufferWithLength:
                static_cast<size_t>(candidate.new_max_n) * 16u * sizeof(float)
                                                           options:MTLResourceStorageModePrivate];
        if (!session.token_.column_grouped_affine_u2_active_count)
            candidate.active_count.value = [tok_device() newBufferWithLength:sizeof(uint32_t)
                                                               options:MTLResourceStorageModeShared];
        if (!session.token_.column_grouped_affine_u2_numerical_error)
            candidate.numerical_error.value = [tok_device() newBufferWithLength:sizeof(uint32_t)
                                                                    options:MTLResourceStorageModeShared];
        if (!session.token_.column_grouped_affine_u2_selected_bytes)
            candidate.selected_bytes.value = [tok_device() newBufferWithLength:sizeof(uint64_t)
                                                                  options:MTLResourceStorageModeShared];
...
MetalTokMoeCapabilities metal_tok_session_moe_capabilities(MetalTokSession& session) {
    MetalTokScope scope(session.token_);
    if (session.token_.lease_backed) return {};
    init();
...
bool metal_tok_session_recurrent_ready(MetalTokSession& session) {
    ...
    return g_dev && session.token_.queue && g_lib && q2 && q2.threadExecutionWidth == 32 &&
           ...
           suitable(named_pipe("rmsnorm_f32")) &&
           suitable(named_pipe("rmsnorm_q8_0")) && suitable(named_pipe("vec_add")) &&
...
bool metal_test_rmsnorm_q8_0(
    ...
    if (max_threads_per_threadgroup) *max_threads_per_threadgroup = 0;
    init();
    if (!g_dev || !g_q || !g_lib || !input || !output || !gpu_ms ||
        !requested_bytes || !command_buffers || !thread_execution_width ||
        !max_threads_per_threadgroup || weight.type != GGMLType::Q8_0 ||
        weight.n_dims != 1 || n < 1 || (n % 32) != 0 ||
        weight.dims[0] != static_cast<uint64_t>(n) || !weight.data ||
    ...
    if (block_count > std::numeric_limits<uint64_t>::max() /
        ...
        return false;
    const uint64_t weight_bytes = block_count * bytes_per_block(GGMLType::Q8_0);
    if (weight_bytes == 0 || weight_bytes > SIZE_MAX ||
        weight.data_bytes != weight_bytes ||
        static_cast<uint64_t>(n) > SIZE_MAX / sizeof(float)) return false;
    id<MTLComputePipelineState> pipeline = named_pipe("rmsnorm_q8_0");
    if (!pipeline || pipeline.threadExecutionWidth != 32 ||
        pipeline.maxTotalThreadsPerThreadgroup < 256u) return false;
    const size_t input_bytes = static_cast<size_t>(n) * sizeof(float);
    id<MTLBuffer> weights = [g_dev newBufferWithBytes:weight.data
                                                length:static_cast<size_t>(weight_bytes)
                                               options:MTLResourceStorageModeShared];
    id<MTLBuffer> input_buffer = [g_dev newBufferWithBytes:input
                                                     length:input_bytes
                                                    options:MTLResourceStorageModeShared];
    id<MTLBuffer> output_buffer = [g_dev newBufferWithLength:input_bytes
                                                      options:MTLResourceStorageModeShared];
    auto release_buffers = [&] {
        [weights release];
        [input_buffer release];
    ...
    if (completed) {
        ...
        const double host_ms = std::chrono::duration<double, std::milli>(
            wait_end - wait_start).count();
        *gpu_ms = gpu_end >= gpu_start ? 1000.0 * (gpu_end - gpu_start) : host_ms;
        std::memcpy(output, [output_buffer contents], input_bytes);
    } else {
        std::fprintf(stderr, "[metal] rmsnorm_q8_0 command: %s\n",
                     command_buffer_failure_detail(command).c_str());
    }
    *requested_bytes = weight_bytes;
    *command_buffers = 1u;
    ...
    *max_threads_per_threadgroup =
        static_cast<uint32_t>(pipeline.maxTotalThreadsPerThreadgroup);
    release_buffers();
    return completed && std::isfinite(*gpu_ms) && *gpu_ms > 0.0;
}
#endif
...
static bool metal_test_column_grouped_affine_uint2_skip_v1_run(
    ...
    if (split_count == 0u) return false;
...
bool metal_tok_session_set_sparse_ffn_runs(MetalTokSession& session,
    ...
    id<MTLBuffer> table = [tok_device() newBufferWithBytes:block_ids.data()
...
bool metal_tok_session_set_sparse_ffn_layer_ids(MetalTokSession& session,
    ...
    id<MTLBuffer> table = [tok_device() newBufferWithBytes:ids
...
bool metal_tok_session_set_sparse_ffn_proxy(MetalTokSession& session, uint32_t slot,
    ...
    id<MTLBuffer> coefficient_buffer = [tok_device() newBufferWithBytes:coefficients
    ...
    if (!session.token_.sparse_scores || session.token_.sparse_scores.length < score_bytes) {
        id<MTLBuffer> scores = [tok_device() newBufferWithLength:score_bytes
                                                  options:MTLResourceStorageModeShared];
    ...
    if (!session.token_.sparse_block_ids || session.token_.sparse_block_ids.length < 2 * sizeof(uint32_t)) {
        ...
        id<MTLBuffer> table = [tok_device() newBufferWithBytes:placeholder length:sizeof(placeholder)
                                             options:MTLResourceStorageModeShared];
        if (!table) return false;
        [session.token_.sparse_block_ids release];
        session.token_.sparse_block_ids = table;
...
bool metal_tok_session_set_importance_slots(MetalTokSession& session, const uint32_t* widths,
    ...
    for (uint32_t slot = 0; slot != slot_count; ++slot) {
        ...
        candidate[slot].values = [tok_device() newBufferWithLength:bytes options:MTLResourceStorageModeShared];
...
bool metal_tok_session_begin(MetalTokSession& session, int H, int inter, int exp_inter, int n_used, int n_experts,
    ...
    if (!prepare_leased_program(session.token_))
        return tok_fail("leased program was not selected before token begin");
...
bool metal_tok_session_begin_with_attention_capacity(
    ...
    if (capacity.query_width < 1 || capacity.key_value_width < 1)
        return tok_fail("token begin received invalid attention capacity");
...
bool metal_tok_session_begin_continuing(MetalTokSession& session, int H, int inter, int exp_inter,
    ...
    MetalTokScope scope(session.token_);
    if (!prepare_leased_program(session.token_)) return false;
...
bool metal_tok_session_upload_embeddings_batch(MetalTokSession& session, const Tensor& embedding,
    ...
    for (uint32_t row = 0; row != rows; ++row) {
        ...
        if (!tok_upload_embedding_to(embedding, tokens[row], H, vocab, scale,
                                     tok_batch_row(g_tok.x, H, row), row == 0))
            return tok_fail("prefill batch embedding upload failed");
    }
    return true;
}
...
bool metal_tok_session_select_prefill_batch_row(MetalTokSession& session, uint32_t row) {
    ...
    if (session.token_.lease_backed &&
        ...
        session.token_.failure_detail =
            "prefill output row does not match the leased semantic program";
        return false;
    }
    g_tok.x = tok_batch_row(g_tok.x, g_tok.H, row);
    g_tok.xn = tok_batch_row(g_tok.xn, g_tok.H, row);
    g_tok.selected_batch_row = row;
    session.token_.leased_dispatch_row_index = row;
    session.token_.leased_dispatch_row_count = 1;
    return true;
}
...
bool metal_tok_session_seal_token(MetalTokSession& session) {
    ...
    if (!leased_program_complete(session.token_)) {
        session.token_.failure_detail =
            "leased program cannot seal before its exact range is consumed";
        return false;
    }
    tok_split();
    g_tok.live = false;
...
void metal_tok_session_abort(MetalTokSession& session) {
    ...
    session.discard_recurrent();
    metal_tok_abort();
    reset_leased_program_cursor(session.token_);
}
...
bool recurrent_buffer(id<MTLBuffer>& buffer, size_t bytes) {
    ...
    [buffer release];
    buffer = [tok_device() newBufferWithLength:bytes options:MTLResourceStorageModeShared];
    return buffer != nil;
}
...
bool metal_tok_session_recurrent_layer(MetalTokSession& session, const MetalTokRecurrentLayer& layer) {
    MetalTokScope scope(session.token_);
    const bool leased = session.token_.lease_backed;
    if (!leased) init();
    RecurrentSizes sizes;
    if ((!leased && (!g_dev || !session.token_.queue || !g_lib)) ||
        (leased && (!session.token_.device || !session.token_.queue)) ||
        !g_tok.live) {
...
bool metal_tok_session_commit_token(MetalTokSession& session) {
    ...
    if (!leased_program_complete(session.token_)) {
        session.token_.failure_detail =
            "leased program cannot commit before its exact range is consumed";
        session.discard_recurrent();
        return false;
    }
    tok_split();
    ...
    if (completed) ++metrics.command_buffers;
    metrics.cpu_wait_ms = std::chrono::duration<double, std::milli>(wait_end - wait_start).count();
    const CFTimeInterval gpu_start = [g_tok.cmd GPUStartTime];
    const CFTimeInterval gpu_end = [g_tok.cmd GPUEndTime];
    metrics.gpu_time_ms = gpu_end >= gpu_start ? 1000.0 * (gpu_end - gpu_start) : 0.0;

...
static bool bind_q4k(id<MTLComputeCommandEncoder> enc, const Tensor& w,
    ...
    if (w.type != GGMLType::Q4_K || (K % 256) != 0 || N < 1) return false;
...
#endif
    if (w.type != GGMLType::Q2_K || (K % 256) != 0 || N < 1) return false;
...
static bool bind_iq2_xxs(id<MTLComputeCommandEncoder> encoder, const Tensor& weight,
    ...
    if (weight.type != GGMLType::IQ2_XXS || (K % 256) != 0 || N < 1) return false;
    ...
    const uint64_t row_bytes = static_cast<uint64_t>(K / 256) * 66;
...
static bool bind_iq1_s(id<MTLComputeCommandEncoder> encoder, const Tensor& weight,
                       int K, int N, id<MTLBuffer> input, size_t input_offset,
                       id<MTLBuffer> output, size_t output_offset) {
    if (weight.type != GGMLType::IQ1_S || (K % 256) != 0 || N < 1) return false;
    ...
    const uint64_t row_bytes = static_cast<uint64_t>(K / 256) * 50;
    size_t weight_offset = 0;
    id<MTLBuffer> weight_buffer =
        get_weight_buf(weight.data, static_cast<size_t>(N) * row_bytes, weight_offset);
    if (!weight_buffer) return false;
...
static bool bind_q2k_two_row(id<MTLComputeCommandEncoder> encoder, const Tensor& weight,
                             int K, int N, id<MTLBuffer> input, size_t input_offset,
                             id<MTLBuffer> output, size_t output_offset) {
    if (weight.type != GGMLType::Q2_K || (K % 256) != 0 || N < 1) return false;
...
static bool bind_q8(id<MTLComputeCommandEncoder> enc, const Tensor& w,
                    int K, int N, id<MTLBuffer> xbuf, size_t xoff,
                    id<MTLBuffer> ybuf, size_t yoff) {
    if (w.type != GGMLType::Q8_0 || (K % 32) != 0 || N < 1) return false;
...
static bool metal_register_mmap(const void* base, size_t size) {
    ...
    if (page_size <= 0 || size > SIZE_MAX - (static_cast<size_t>(page_size) - 1)) return false;
    ...
    while (off < aligned) {
        ...
        if (chunk > (size_t)max_buf) chunk = (size_t)max_buf;
        ...
        if (buf) {
            ...
            context.mmap_bufs.push_back({buf, ptr, chunk});
            n_bufs++;
        } else {
            fprintf(stderr, "[metal] mmap buffer failed at %p size %zu\n", ptr, chunk);
            complete = false;
        }
        if (off + chunk >= aligned) break;
        size_t step = (chunk > overlap) ? (chunk - overlap) : chunk;
        off += step;
    }
    fprintf(stderr, "[metal] model %.1f GB, %d zero-copy buffer(s)\n", size/1e9, n_bufs);
    return complete && n_bufs != 0;
}
...
void metal_unregister_weights(const void* base) {
    ...
    auto first = std::remove_if(context.mmap_bufs.begin(), context.mmap_bufs.end(), [&](const MmapBuf& buffer) {
        if (buffer.base != base) return false;
        [buffer.buf release];
        return true;
    });
...
static bool bind_rows(id<MTLComputeCommandEncoder> enc, const Tensor& w,
    ...
    // This experimental TensorOps path is only valid for medium-width rows.
...
static bool bind_q6k(id<MTLComputeCommandEncoder> enc, const Tensor& w,
                     int K, int N, id<MTLBuffer> xbuf, size_t xoff,
                     id<MTLBuffer> ybuf, size_t yoff) {
    if (w.type != GGMLType::Q6_K || (K % 256) != 0 || N < 1) return false;
...
struct TokWS {
    ...
    int H = 0, inter = 0;
    int Hq = 0, Hk = 0, Dh = 0, max_seq = 0, n_layers = 0, pos = 0;
    uint32_t batch_rows = 1;
    size_t x = 0, xn = 0, xb = 0, q = 0, k = 0, v = 0, ao = 0;
    size_t fg = 0, fu = 0, fh = 0, tmp = 0, ones = 0;
    size_t bytes = 0;
    bool live = false;
    bool async = false;
};
...
static bool tok_enc() {
    if (!g_tok.cmd) {
        if (active_tok_context().detailed_errors) {
            ...
            g_tok.cmd = [g_q commandBufferWithDescriptor:descriptor];
            [descriptor release];
        } else {
            g_tok.cmd = [g_q commandBuffer];
        }
...
static bool tok_begin(int H, int inter, int exp_inter, int n_used, int n_experts,
    ...
    if (Hq < 1 || Hk < 1 || Dh < 1 || (batch_rows != 1 && batch_rows != 2)) return false;
    ...
    if (Hq > INT_MAX / Dh || Hk > INT_MAX / Dh) return false;
    int qn = Hq * Dh, kn = Hk * Dh;
    size_t o = 0;
    ...
    const size_t layers = static_cast<size_t>(n_layers);
    const size_t sequence = static_cast<size_t>(max_seq);
    const size_t key_values = static_cast<size_t>(kn);
    if (layers > SIZE_MAX / sequence || layers * sequence > SIZE_MAX / key_values ||
        layers * sequence * key_values > SIZE_MAX / sizeof(float)) {
        g_tok = TokWS{};
        return false;
    }
    size_t kvb = layers * sequence * key_values * sizeof(float);
    ...
    if (!g_tok.ws || !g_tok.kcache || !g_tok.vcache) {
        g_tok = TokWS{};
        return false;
    }
    if (pos == 0) {
    ...
    for (int i = 0; i < Dh; i++) ones[i] = 1.0f;
...
static bool tok_upload_embedding_to(const Tensor& embedding, uint32_t token, int H, int vocab,
                                    size_t destination, bool profile_start) {
    if (!g_tok.live || !embedding.data || H < 1 || vocab < 1 || token >= static_cast<uint32_t>(vocab)) return false;
...
bool metal_tok_upload_embedding(const Tensor& embedding, uint32_t token, int H, int vocab) {
    return tok_upload_embedding_to(embedding, token, H, vocab, g_tok.x, true);
}
...
static void tok_record_projection_bytes(const Tensor& w, int K, int N) {
    ...
    if (K > 0 && N > 0 && block_elements > 0 && K % block_elements == 0 && block_bytes != 0) {
        ...
        active_tok_context().metrics.projection_weight_bytes += static_cast<uint64_t>(N) * row_bytes;
    }
}
...
static bool tok_bind_sparse_rows(const Tensor& w, size_t xoff, size_t yoff,
    ...
    if (static_cast<uint64_t>(full_N) > SIZE_MAX / rb) return false;
    size_t weight_offset = 0;
...
static bool tok_bind_sparse_columns_to(const Tensor& w, id<MTLBuffer> input, size_t input_offset,
    ...
    if (static_cast<uint64_t>(N) > SIZE_MAX / rb) return false;
    size_t weight_offset = 0;
...
static bool tok_bind_sparse_columns(const Tensor& w, size_t xoff, size_t yoff,
    ...
    context.metrics.projection_weight_bytes += physical;
    tok_record_projection_dispatch(w.type);
    return true;
}

static bool tok_rms(const Tensor& w, size_t src, size_t dst, int n, float eps) {
    if (!tok_enc() || !w.data) return false;
    size_t woff = 0;
...
#define TOKFAIL(s) do { fprintf(stderr, "[metal] tok fail %s Hq=%d Hk=%d Dh=%d inter=%d\n", \
    ...
    if (L.owns_kv) {
        ...
        [g_tok.enc setBytes:&L.cache_id length:4 atIndex:4];
        [g_tok.enc setBytes:&kn length:4 atIndex:5];
        int kn_stride = g_tok.Hk * g_tok.Dh;
        [g_tok.enc setBytes:&kn_stride length:4 atIndex:6];
    ...
    [g_tok.enc setBytes:&L.cache_id length:4 atIndex:10];
    [g_tok.enc setBytes:&scale length:4 atIndex:11];
    int kn_stride = g_tok.Hk * g_tok.Dh;
    [g_tok.enc setBytes:&kn_stride length:4 atIndex:12];
...
static bool tok_prefill_f16_layer_supported(const MetalTokLayer& L) {
    ...
    return g_tok.live && g_tok.batch_rows == 2 && L.H == g_tok.H && L.Hq == g_tok.Hq &&
           L.Hk == g_tok.Hk && L.Dh == g_tok.Dh && L.inter == g_tok.inter &&
           L.owns_kv &&
           ...
           std::isfinite(L.rope_base) && L.rope_base > 0.0f && std::isfinite(L.rms_eps) &&
...
static bool tok_prefill_f16_rope_and_kv(const MetalTokLayer& L, uint32_t row, int position) {
    ...
    [g_tok.enc setBytes:&L.cache_id length:sizeof(L.cache_id) atIndex:4];
    [g_tok.enc setBytes:&kn length:sizeof(kn) atIndex:5];
    const int stride = g_tok.Hk * g_tok.Dh;
    [g_tok.enc setBytes:&stride length:sizeof(stride) atIndex:6];
...
static bool tok_prefill_f16_attention(const MetalTokLayer& L, uint32_t row, int position) {
    ...
    const float scale = 1.0f / sqrtf(static_cast<float>(L.Dh));
    ...
    [g_tok.enc setBytes:&L.cache_id length:sizeof(L.cache_id) atIndex:10];
    [g_tok.enc setBytes:&scale length:sizeof(scale) atIndex:11];
    [g_tok.enc setBytes:&stride length:sizeof(stride) atIndex:12];
...
bool metal_tok_final(const Tensor& norm, const Tensor& lm, float* logits,
    ...
    if (!g_tok.live || !logits || vocab < 1 || !lm.data) return false;
...
private:
    ...
    friend bool metal_tok_session_upload_embedding(MetalTokSession&, const Tensor&, uint32_t, int, int);
...
std::shared_ptr<MetalTokSession> metal_tok_session_create() {
    init();
    if (!g_dev || !g_q || !g_lib) return {};
    return std::make_shared<MetalTokSession>();
}
...
#endif
    return g_dev && g_q && g_lib && sparse_proxy_ready && sparse_oracle_ready &&
...
bool metal_tok_session_recurrent_ready(MetalTokSession& session) {
    ...
    return g_dev && g_q && g_lib && q2 && q2.threadExecutionWidth == 32 &&
...
bool metal_test_rope_interleaved(float* query, float* key, int query_heads, int key_heads,
                                 int head_dimension, int rotary_dimension,
    ...
    if (!g_dev || !g_q || !g_lib || !query || !key || query_heads < 1 || key_heads < 1 ||
        ...
        rotary_dimension % 2 != 0 || !std::isfinite(rope_base) || rope_base <= 0) return false;
...
bool metal_tok_session_upload_embedding(MetalTokSession& session, const Tensor& embedding, uint32_t token,
                                         int H, int vocab) {
    MetalTokScope scope(session.token_);
    return metal_tok_upload_embedding(embedding, token, H, vocab);
}
...
bool metal_tok_session_upload_embeddings_batch(MetalTokSession& session, const Tensor& embedding,
    ...
    for (uint32_t row = 0; row != rows; ++row) {
        if (!tok_upload_embedding_to(embedding, tokens[row], H, vocab,
...
bool recurrent_matrix(const Tensor* tensor, int K, int N) {
    return tensor && tensor->data && tensor->n_dims == 2 && tensor->dims[0] == static_cast<uint64_t>(K) &&
           ...
           (tensor->type == GGMLType::IQ2_XXS || tensor->type == GGMLType::Q2_K ||
            ...
            tensor->type == GGMLType::Q6_K) && K % 256 == 0;
}
...
bool metal_tok_session_recurrent_layer(MetalTokSession& session, const MetalTokRecurrentLayer& layer) {
    ...
    init();
    RecurrentSizes sizes;
    if (!g_dev || !g_q || !g_lib || !g_tok.live ||
        recurrent_layer_preflight(layer, g_tok.H, g_tok.inter, g_tok.Dh) != nullptr ||
        !recurrent_sizes(layer.qk_heads, layer.value_heads, layer.head_dimension, layer.kernel, sizes)) return false;
    const int channels = static_cast<int>(sizes.channels);
    const int output_width = static_cast<int>(sizes.output);
...
bool metal_tok_session_recurrent_step(MetalTokSession& session, const float* qkv, const float* conv_weight,
    ...
    init();
    RecurrentSizes sizes;
    if (!g_dev || !g_q || !g_lib || !qkv || !conv_weight || !gate || !beta || !alpha || !dt_bias || !decay ||
...
bool metal_test_attn(const float* Q, const float* Kc, const float* Vc,
    ...
    [enc setBytes:&cache_row length:4 atIndex:10];

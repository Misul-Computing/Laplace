...
public:
    ...
    static SemanticManifestResult compile(const ArtifactIndex& physical,
                                          ...
                                          const TokenContract& token_contract,
                                          const PhysicalCodecRegistry& codec_registry) {
        return build(physical, model, token_contract, codec_registry);
    }
    ...
    // Product authority requires an explicit, complete codec registry whose
    // digest is bound to this immutable semantic model.
    bool has_physical_codec_authority() const;

...
SessionCreateResult create_product_runtime_session_internal(
    ...
    if (const auto& physical = package->physical_program_package(); physical) {
        auto bound = validate_physical_program_package_for_runtime(*package, *physical);
        if (const auto* report = std::get_if<CompatibilityReport>(&bound)) return *report;
    ...
    if (!metal->plan().codec_aware || metal->plan().dispatch_programs.empty())
        return session_error(CompatibilityError::KERNEL_UNAVAILABLE);
    const auto physical_package = package->physical_program_package();
    RuntimeSession result(std::move(package), metal->plan(), SessionResources{}, StateStore(SemanticModel{}),
                          std::move(metal), true, request, physical_package);
...
RuntimeRunResult RuntimeSession::execute(std::span<const uint32_t> token_ids,
    ...
    if (product_route_) {
        ...
        if (phase == ExecutionPhase::Prefill && token_ids.size() > 1) {
            ...
            while (index < token_ids.size()) {
                ...
                if (final_chunk) final_output = std::move(sample);
                index += rows;
            }
